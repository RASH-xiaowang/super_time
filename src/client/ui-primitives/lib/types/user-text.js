...
export function projectUserText(text, sessionLabels, slashNames = [], slashKind = 'skill') {
    ...
    for (const range of ranges) {
        ...
        parts.push(_jsxs("span", { className: clsx(css.refChip, referenceKind === undefined && css.slashChip), "data-ref-chip": referenceKind ?? slashKind, title: label, children: [referenceKind !== undefined && (_jsx(ReferenceIcon, { kind: referenceKind, size: 16, className: css.refIcon })), displayLabel] }, tokenStart));
        cursor = end;
    }
    if (parts.length === 0)
