/**
 * 微信内置表情表：表情代码 → `public/wxemoji/` 下的图片文件名。
 *
 * 同一枚表情有**三种**写法（中文名 `[微笑]`、英文名 `[Smile]`、颜文字 `/::)`），
 * 微信在文本消息里存的就是这些字面量，因此三套 key 都要能匹配。
 * 表与图片资源对齐 WeChatDataAnalysis 的 `frontend/lib/wechat-emojis.ts` +
 * `frontend/public/wxemoji/`（149 个 PNG，约 0.9 MB）。
 *
 * 图片放在 `src/client/ui-app/public/wxemoji/`，由 Vite 原样拷进构建产物。
 */
 ...
 * 表情图片的资源前缀。
 *
 * ⚠️ 必须是**相对**路径：渲染进程用 `mainWindow.loadFile()` 加载
 * `src/client/ui-dist/index.html`（`file://` 源）。绝对路径 `/wxemoji/…` 在
 * `file://` 下会被解析成盘符根目录 `file:///wxemoji/…`，图片全部 404。
 * `vite.config.js` 的 `base: './'` 与 public 目录（`src/client/ui-app/public`）
 * 共同保证构建后 `ui-dist/wxemoji/*.png` 与本文件同级可寻址。
 */
const EMOJI_BASE = './wxemoji/';
 ...
 * 把一段文本切成「普通文本 / 表情」分段。
 *
 * 微信的文本消息里 `[微笑]`、`/::)`、`[Smile]` 都是表情代码（同一枚表情有
 * 中文名/英文名/颜文字三种写法，表里 886 行覆盖）。不转换就会在气泡里显示
 * 字面量 `[微笑]` —— 这是聊天界面可读性最直接的损失。
 * @param text - 原始文本。
 * @returns 分段数组（至少一段，除非入参为空）。
 */
export function parseTextWithEmoji(text: string): TextSegment[] {
  ...
  while ((match = emojiRegex.exec(text)) !== null) {
    ...
    segments.push({
      type: 'emoji',
      content: emojiKey,
      emojiSrc: `${EMOJI_BASE}${WechatEmojiTable[emojiKey] ?? ''}`
    });
