...
export type WechatTab =
  | 'overview' | 'ask' | 'chats' | 'graph' | 'knowledge' | 'kb' | 'monitor' | 'contacts' | 'moments'
  | 'favorites' | 'emoticons' | 'files' | 'records' | 'ledger' | 'storage' | 'bizchats'
  | 'servicechats' | 'kefu' | 'annual' | 'period' | 'dailysummary' | 'hook' | 'privacy'
  | 'revoked' | 'backup' | 'settings' | 'oplog' | 'tasks' | 'groupinsights' | 'health' | 'momentsinsights' | 'privacytrust' | 'assetinsights' | 'officialassets' | 'mediaassets' | 'calls' | 'kbfiles'
...
export const NAV_GROUPS: ReadonlyArray<{ label: string; items: ReadonlyArray<NavItem> }> = [
  ...
  },
  {
    ...
    items: [
      ...
      // ── 社交图谱 2026-09-17 并入「通讯录」─────────────────────────────
      // 两者是**同一批人的两种看法**（联系人列表 / 关系图），所以收进同一个导航项、
      // 由面板顶部段条切换（见 WechatDataPanel 里 contacts|graph 的合并 case）。
      // 因此本条目标 hidden：不再占侧栏条目，但**仍然可路由** —— 段条的第二段
      //（key: 'graph'）与深链 #graph 都走它，`TAB_LABELS.graph` 也照旧是「社交图谱」。
      // 图标（人像 + 三个散点）刻意留在配置里：hidden 条目不渲染，但它是逐条对照
      // 微信官方字形画过的，将来若把这枚图标挪去别处复用，不必重画。
      // 人像与散点之间必须留出可见空隙：17px 下两者的最近点若只隔不到 2 个单位，
      // 缩到 12px 就会糊成一团（首版实测就是这样），看起来像「人身上长了疙瘩」。
      // 社交图谱：人像 + 三个散点（我的关系网络）。官方「通讯录」只给一个人像，
      // 这枚靠右侧三个散点读成「网络」，17px 下两者也能一眼分开。
      { tab: 'graph', label: '社交图谱', icon: filled('<path d="M7.6 10.4a3.2 3.2 0 1 0 0-6.4 3.2 3.2 0 0 0 0 6.4zM7.6 12c-3.3 0-5.9 1.9-5.9 4.4v1.5h11.8v-1.5c0-2.5-2.6-4.4-5.9-4.4zM17.2 4.4a1.8 1.8 0 1 0 0 3.6 1.8 1.8 0 0 0 0-3.6zM21 10a1.4 1.4 0 1 0 0 2.8 1.4 1.4 0 0 0 0-2.8zM17.4 14.2a1.6 1.6 0 1 0 0 3.2 1.6 1.6 0 0 0 0-3.2z"/>'), hidden: true },
      // ⚠️ 这条是 hidden，但**绝不能整行删掉**：`TAB_LABELS` 由 NAV_GROUPS 构建，
      // 删条目等于删掉 `TAB_LABELS.graph`，深链 `#graph` 会静默落回「数据总览」
      // （`WechatDataPanel` 用 `initialHash in TAB_LABELS` 判初始页签）。
      // 2026-09-18 的一次合并重构真的删过它：typecheck / build / 全部旧断言都是绿的，
      // 只有功能坏了。现在由 contacts-graph-merge.wiring.spec.ts 钉住。
      // ── 知识图谱 2026-09-18 并入「知识库」─────────────────────────────
      // 它与知识库是**同一批数据的两种看法**（同一个 `wechat_notes.db` 的 notes 表）：
      // 图谱把笔记当「网」看（力导向画布 / [[链接]] 边 / 待补节点），
      // 知识库把笔记当「文档」看（列表 / 详情 / 新建编辑 / 标签 / 关键词搜索）。
      // 既然同源，就收进同一个导航项、由面板顶部段条切换
      // （见 WechatDataPanel 里 knowledge|kb 的合并 case）。
      // 因此本条目标 hidden：不再占侧栏条目，但**仍然可路由** —— 段条第二段
      //（key: 'knowledge'）与深链 #knowledge 都走它，
      // `TAB_LABELS.knowledge` 也照旧是「知识图谱」。
      // 图标（带书脊 + 两行正文的书）留在配置里：hidden 条目不渲染，但它是逐条
      // 对照微信官方字形画过的，将来若挪去别处复用，不必重画。
      // 注意它与下面「知识库」那枚**必须能分开**：这枚是**一本书**（= 单篇笔记），
      // 那枚是**一排书立在架上**（= 一个库）。
      { tab: 'knowledge', label: '知识图谱', icon: filled('<path fill-rule="evenodd" d="M4.6 3.4A2.2 2.2 0 0 1 6.8 1.2h12.6a.9.9 0 0 1 .9.9v19.8a.9.9 0 0 1-.9.9H6.8a2.2 2.2 0 0 1-2.2-2.2zM8.2 5.6h1.8v12.8H8.2zM12 7.2h5.4v1.9H12zM12 11.2h5.4v1.9H12z"/>'), hidden: true },
      // 知识库：三本竖立的书 + 一条底座（书架）。**它是这一主题在侧栏的唯一入口**
      //（2026-09-18 起吸收知识图谱，2026-09-19 再吸收文件，见上面与下面两条注释）：
      // 点进来后由段条在「文件 / 笔记库 / 知识图谱」之间切换，默认落在**文件**那一段
      //（落点写在 `KbShell.tsx` 的 `landing` 里，不在本表 —— 本表只管入口与标签）。
      // 图标刻意与图谱那枚「带书脊的竖书」不同：那枚是**一本书**（= 单篇笔记），
      // 这枚是**一排书立在架上**（= 一个库），17px 下也能一眼分开。
      // 三本书高度**非单调**（7.0 / 5.0 / 6.2）—— 递增的三根柱是「数据总览」的形，
      // 同排两个图标读成同一个就是形制事故。书脚插进底座 0.9 单位，避免抗锯齿接缝。
      // ⚠️ 必须包 `<path d="…"/>`：裸 path 数据塞进 `filled()` 只会产出
      // `<g>M4.3 17.7…</g>` —— SVG 里 <g> 不渲染文本子节点，图标直接变空白，
      // 而 typecheck / build / 全部静态断言都不报（这个缺陷真发生过）。
      // 用默认 nonzero 填充规则：三本书的脚插进底座 0.9 单位，重叠区靠同向绕组取并集；
      // 换成 evenodd 会把重叠那条缝挖空，底座与书之间出现一道发丝缝。
      { tab: 'kb', label: '知识库', icon: filled('<path d="M4.3 17.7H19.7A1.4 1.4 0 0 1 21.1 19.1A1.4 1.4 0 0 1 19.7 20.5H4.3A1.4 1.4 0 0 1 2.9 19.1A1.4 1.4 0 0 1 4.3 17.7ZM4.9 7H7.5A0.8 0.8 0 0 1 8.3 7.8V17.8A0.8 0.8 0 0 1 7.5 18.6H4.9A0.8 0.8 0 0 1 4.1 17.8V7.8A0.8 0.8 0 0 1 4.9 7ZM10.7 5H13.3A0.8 0.8 0 0 1 14.1 5.8V17.8A0.8 0.8 0 0 1 13.3 18.6H10.7A0.8 0.8 0 0 1 9.9 17.8V5.8A0.8 0.8 0 0 1 10.7 5ZM16.5 6.2H19.1A0.8 0.8 0 0 1 19.9 7V17.8A0.8 0.8 0 0 1 19.1 18.6H16.5A0.8 0.8 0 0 1 15.7 17.8V7A0.8 0.8 0 0 1 16.5 6.2Z"/>') },
      // ── 文件 2026-09-18 并入「知识库」（本计划的 T2）─────────────────────
      // 文件是知识库的**第三类内容**（笔记 / 图谱 / 文件）：它们同属一个作用域
      // （当前选中的那个库），都由分段条切换、都由右侧的库切换器决定「看哪个库」。
      // 因此本条目标 hidden：不再占侧栏条目（侧栏只留「知识库」一个入口），
      // 但**仍然可路由** —— 段条第三段（key: 'kbfiles'）与深链 #kbfiles 都走它，
      // `TAB_LABELS.kbfiles` 也照常是「文件」。
      // ⚠️ 与上面两条同理：这条 hidden 条目**绝不能整行删掉**。`TAB_LABELS` 由
      // NAV_GROUPS 构建（含 hidden 项），删了它就等于删掉 TAB_LABELS.kbfiles，
      // 深链 #kbfiles 会静默落回「数据总览」（WechatDataPanel 用
      // `initialHash in TAB_LABELS` 判初始页签），而 typecheck 与构建都不报。
      // 图标（文件 + 两行正文）沿用旧的描边字形：hidden 条目不渲染，没有绘制点，
      // 因此不参与本轮「按微信官方字形重画」的口径。
      { tab: 'kbfiles', label: '文件', icon: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="13" y2="17"/>', hidden: true },
    ],
  },
  {
    label: '内容资产',
