...
function communityColumns(ratio: PosterRatio, count: number): number {
  if (ratio === '3:4') return 2
  if (ratio === '16:9') return 2
  // 1:1 方图:最多 4 列,至少随数量走
  return Math.max(1, Math.min(count, 4))
}

export function getPosterLayout(ratio: PosterRatio, communityCount = 0): PosterLayout {
  let base: PosterLayout
  if (ratio === '3:4') {
    base = {
      width: 1080,
      height: 1440,
  ...
  // 无圈子时保持原布局
  if (communityCount <= 0) return base
  ...
  const layout: PosterLayout = { ...base }
...
export const POSTER_THEMES: Record<PosterStyle, PosterTheme> = {
  light: {
...
/** 文本截断(中英文按字符计数,超长省略)。 */
function ellipsis(text: string, maxPx: number, font: string, ctx: CanvasRenderingContext2D): string {
  if (maxPx <= 0) return ''
  ctx.save()
  ctx.font = font
  let t = text
...
export function buildPoster(input: PosterInput): HTMLCanvasElement {
  ...
  const scale = Math.max(
    1,
    Math.min(input.scale ?? 7, 8192 / Math.max(layout.width, layout.height)),
  )
  ...
  if (rels.length > 0) {
    ...
    rels.slice(0, n).forEach((rel, i) => {
      ...
      ctx.save()
      if ((input.blurNodes ?? 0) > 0) ctx.filter = `blur(${input.blurNodes}px)`
      if (rel.sprite) {
  ...
  if (comms.length > 0) {
    ...
    comms.forEach((c, i) => {
      ...
      ctx.fillText(
        ellipsis(`${c.count} 人 · ${c.names}${suffix}`, itemW - 52, `500 14px ${FONT}`, ctx),
        cx + 16,
        cy + 4,
      )
    })
...
export function posterToDataUrl(canvas: HTMLCanvasElement, format: 'png' | 'jpeg' = 'png'): Promise<string> {
  return new Promise((res, rej) => {
    canvas.toBlob(
      (b) => {
        if (!b) { rej(new Error('海报生成失败')); return }
