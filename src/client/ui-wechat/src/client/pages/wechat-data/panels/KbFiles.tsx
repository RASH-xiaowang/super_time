 ...
 */
function cachedFiles(kbId: number): KbFilesCache | null {
 ...
 * @returns 文案 + 色调。
 */
function indexLabel(view: KbVectorIndexView | null): { text: string; tone: 'ok' | 'warn' | 'muted' } {
...
type BadgeTone = 'default' | 'cyan' | 'green' | 'red' | 'amber' | 'purple' | 'blue'
 ...
 *     画一根假的进度条比不画更糟。
 */
const PARSE_LOOK: Record<KbFileParseState, ParseLook> = {
 ...
 * @returns 要不要渲染这一行元信息。
 */
function headingWorthShowing(heading: string): boolean {
...
const REGISTER_REASON: Record<NonNullable<KbFileRegisterResult['code']>, string> = {
  'bad-kb': '知识库不存在',
  'bad-path': '路径读不到',
 ...
 * @returns 一句给用户看的原因。
 */
function reasonText(r: KbFileRegisterResult): string {
  if (r.code === 'duplicate') {
 ...
 * @returns 带 `<mark>` 的片段。
 */
function SnippetText({ text, marks }: { text: string; marks: ReadonlyArray<{ start: number; end: number }> }): React.JSX.Element {
...
type StateFilter = 'all' | 'pending' | 'attention' | 'ready'

const FILTERS: ReadonlyArray<{ key: StateFilter; label: string }> = [
  { key: 'all', label: '全部' },
  { key: 'ready', label: '已就绪' },
              ...
              <div className={css.detail}>
                <div className={css.detailHead}>
                  ...
                  <div className={css.detailActions}>
                    ...
                    <Button
                      variant="pill"
                      onClick={() => { void runSummary(selected) }}
                    ...
                    <Button
                      variant="pill"
                      onClick={() => { if (reader === null) void loadChunks(selected.id); else setReader(null) }}
                    >
                      {reader === null ? '查看正文' : '收起正文'}
                    </Button>
                    <Button variant="pill" onClick={() => { void reveal(selected) }}>在资源管理器中定位</Button>
                    <Button
                      variant="danger"
                ...
                <div className={css.switchRow}>
                  <input
                    ...
                    disabled={busyId === selected.id}
                    onChange={(e) => { void toggleRag(selected, e.target.checked) }}
                  />
                ...
                {reader !== null && reader.fileId === selected.id && (
                  <div className={css.reader}>
                    ...
                    {reader.items.length < reader.total && (
                      <Button variant="pill" onClick={() => { void loadMoreReader() }} disabled={reader.loading}>
