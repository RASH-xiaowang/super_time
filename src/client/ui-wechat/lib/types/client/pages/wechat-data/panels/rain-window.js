...
function blurCopyFrom(sharp, px) {
...
function makeTextMask(w, h, label) {
...
function glGetUniform(gl, program, name) {
...
function getScene() {
    if (!sceneCache)
        sceneCache = makeScene(900, 840);
    return sceneCache;
}
export function RainWindow({ className, label = '选择左侧会话查看消息', active = true }) {
    ...
    useEffect(() => {
        ...
        let textTex = makeTexture(makeTextMask(2, 2, ''), 3);
        let raf = 0;
        ...
        const resize = () => {
            ...
            if (!textTex)
                textTex = makeTexture(makeTextMask(canvas.width, canvas.height, labelRef.current), 3);
        ...
        const draw = (now) => {
            if (Math.random() < 0.0012)
                flashTarget = 0.35 + Math.random() * 0.65;
            flashTarget *= 0.995;
    ...
    return _jsx("canvas", { ref: ref, className: className ?? '', "aria-hidden": "true" });
}
//# sourceMappingURL=rain-window.js.map