...
export function squarify(items, box) {
    const out = items.map(() => ({ x: box.x, y: box.y, w: 0, h: 0 }));
    ...
    for (const { v } of posIdx) {
        if (row.length > 0 && worst([...row, v], w, h) > worst(row, w, h))
            layoutRow();
        row.push(v);
    }
    layoutRow();
 ...
 */
export function centeredSquarify(items, box, centerIndex) {
    ...
    if (cWidth > maxW) {
        cWidth = maxW;
        cHeight = cArea / cWidth;
    }
    if (cHeight > maxH) {
        cHeight = maxH;
        cWidth = cArea / cHeight;
    }
    ...
    for (const { o, i } of order) {
        ...
        for (let s = 0; s < bucketArea.length; s += 1) {
            if (best === -1 || (bucketArea[s] ?? 0) > (bucketArea[best] ?? 0))
                best = s;
        }
        if (best === -1)
            continue;
    ...
    for (let s = 0; s < strips.length; s += 1) {
        ...
        for (let k = 0; k < bucket.length; k += 1) {
            ...
            if (src !== undefined && rect && targetIdx !== undefined)
                out[targetIdx] = rect;
        }
    }
    return out;
