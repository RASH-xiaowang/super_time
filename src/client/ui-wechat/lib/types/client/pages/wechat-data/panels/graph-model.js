...
function personWeight(isFriend, msgCount) {
...
function radiusByIntimacy(weight) {
    const MIN_R = 5;
    const MAX_R = 34;
...
function radiusBySqrt(value, divisor, min, max) {
...
export function detectCommunities(nodes, edges) {
    ...
    const idx = new Map(nodes.map((g, i) => [g.id, i]));
    const adj = new Map();
    for (let i = 0; i < n; i++)
        adj.set(i, new Map());
    let m2 = 0;
    ...
    for (let i = 0; i < n; i++) {
        comm.set(i, i);
        let w = 0;
        for (const ew of adj.get(i)?.values() ?? [])
            w += ew;
        nodeW.set(i, w);
        commW.set(i, w);
    }
    ...
    const dq = (i, target) => {
        ...
        if (from === target || m2 <= 0)
            return 0;
        let kIn = 0;
        for (const [j, ew] of adj.get(i) ?? new Map()) {
            if (comm.get(j) === target)
                kIn += ew;
        }
...
export function buildGraph(env, s) {
    if (!env)
    ...
    const rawNodes = env.nodes.filter((n) => {
        if (s.mode === 'people') {
            ...
            if (n.kind === 'group' || n.kind === 'self')
                return false;
            if (s.friendsOnly)
                return n.kind !== 'official' && n.is_friend === true;
            return true;
        }
        return n.kind === 'group';
    });
    // 排序:好友优先(内部按共同群数),其后群友;再整体截前 nodeLimit(参考实现语义)
    const weightOf = (n) => n.msg_count ?? n.shared_count ?? n.group_count ?? 1;
    const top = [...rawNodes]
        .sort((a, b) => {
    ...
    const nodes = [
        ...
        ...top.map(n => ({
            id: n.id,
            label: n.label,
            kind: (n.kind === 'group' ? 'group' : 'person'),
...
export function localGraph(graph, focusId, depth) {
    if (depth <= 0)
        return graph;
...
export function groupCommunities(graph) {
    const map = new Map();
    for (const n of graph.nodes) {
        if (n.kind === 'self' || n.community < 0)
            continue;
...
export function connectedEdgesOf(graph, nodeId, limit = 12) {
    return graph.edges
...
export function sharedGroupNames(n, groupNames, limit = 6) {
    return (n.groupCodes ?? [])
...
export function communityOf(graph) {
    const map = new Map();
    for (const n of graph.nodes)
        map.set(n.id, n.community);
    return map;
}
