...
## 功能界面（16 个侧栏入口 / 37 个可路由页签，158 个 Remote 方法）
...
> **2026-09-17**：社交图谱并入「通讯录」（同一批人的两种看法 —— 联系人列表 / 关系图，
> 由面板顶部段条切换），侧栏入口 17 → **16** 项。
> **2026-09-18**：知识图谱并入「知识库」（同一批笔记的两种看法 —— 文档列表 / 关系网络，
> 由面板顶部段条切换），`knowledge` 转隐藏，侧栏入口回到 **16** 项。
> **2026-09-18（续）**：「知识库」面板加入**第三段「文件」**（`kbfiles`，同样 `hidden: true`、
> 同样可路由），三段合用同一个 `MergedSections` 外壳，各子面板内部**零改动**；
> 侧栏入口仍 **16** 项（合并导航项不新增侧栏条目，这是既有约定）。
> **可路由页签数 37（并入的视图不会消失）**：`graph` / `knowledge` / `kbfiles`
> 都只是转为「隐藏但可路由」，深链 `#graph` / `#knowledge` / `#kbfiles`
> 与段条各段照旧走它。**隐藏项条目本身绝不能从 `NAV_GROUPS` 删掉** ——
> `TAB_LABELS` 由它构建，删了就静默丢深链（2026-09-18 踩过）。
> **计数口径（本节数字错过两次，务请照此复算）**：「侧栏入口」= `nav-config.ts` 里**未标 `hidden`**
> 且**不是 `settings`** 的条目数 —— `settings` 固定渲染在侧栏底部，分组循环显式排除它
> （`WechatDataPanel` 的 `!it.hidden && it.tab !== 'settings'`），不参与计数。
> 2026-09-15 那次重写写的是 16、而当时实际已是 17（「通话记录」`calls` 在计数之后加入，无人更新）；
> 本次按实测改写为 16，并把 `OnboardingShell.tsx` 的 `MODULES` 由 17 同步为 16。
