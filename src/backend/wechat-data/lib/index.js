...
// src/backend/llm-retry.js
var require_llm_retry = __commonJS({
  "src/backend/llm-retry.js"(exports, module) {
    "use strict";
    var RETRYABLE_STATUS = /* @__PURE__ */ new Set([408, 429, 500, 502, 503, 504]);
    var DEFAULT_MAX_ATTEMPTS = 3;
    var BASE_DELAY_MS = 500;
    var MAX_DELAY_MS = 5e3;
    var TIMEOUT_ERROR_NAME2 = "TimeoutError";
    function isRetryableStatus(status) {
      return RETRYABLE_STATUS.has(Number(status));
    }
    function parseRetryAfterMs(headerValue, now = Date.now()) {
      if (headerValue === null || headerValue === void 0) return null;
      const raw = String(headerValue).trim();
      if (raw === "") return null;
      if (/^\d+$/.test(raw)) return Math.min(MAX_DELAY_MS, Number(raw) * 1e3);
      const at = Date.parse(raw);
      if (Number.isNaN(at)) return null;
      return Math.min(MAX_DELAY_MS, Math.max(0, at - now));
    }
    function timeoutError(timeoutMs) {
      const shown = timeoutMs >= 1e3 ? `${Math.round(timeoutMs / 1e3)}s` : `${timeoutMs}ms`;
      const err = new Error(`\u8BF7\u6C42\u8D85\u65F6\uFF08${shown}\uFF09`);
      err.name = TIMEOUT_ERROR_NAME2;
    ...
    async function fetchWithRetry5(doFetch, url, init, opts = {}) {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        ...
        const attemptInit = armed ? { ...init, signal: signal ? AbortSignal.any([signal, armed.signal]) : armed.signal } : init;
        try {
    ...
    module.exports = {
      ...
      backoffDelayMs,
      fetchWithRetry: fetchWithRetry5,
      isRetryableStatus,
      parseRetryAfterMs
    };
  }
...
// src/backend/wechat-data/src/query/meta.ts
import { DatabaseSync } from "node:sqlite";
import { readdirSync, statSync } from "node:fs";
import { join } from "node:path";
var MAX_AGE_MS = 5e3;
var entries = /* @__PURE__ */ new Map();
var METADATA_KEY_FAMILY_LIMITS = [
  { prefix: "msg-by-sid:", cap: 200 }
];
function enforceKeyFamilyLimit(key) {
  ...
  try {
    ...
    try {
      ...
      for (const t of ["Name2Id", "name2id"]) {
        try {
          const rows2 = db.prepare("SELECT rowid AS id, user_name AS u FROM " + t).all();
...
function shardMetaOf(dbFile) {
  return get("shard-meta:" + dbFile, fileSig(dbFile), () => loadShardMeta(dbFile));
}
function shardCatalogDirs(decryptedDir, dirs) {
  ...
  for (const dirName of dirs) {
    ...
    for (const f of files) {
      ...
      const fs2 = fileSig(full);
      sigParts.push(`${f}:${fs2}`);
      entries2.push({ file: full });
    }
  }
  return get(key, sigParts.join("|"), () => entries2.map((e) => shardMetaOf(e.file)));
}
...
function bumpDataGeneration() {
  dataGeneration += 1;
}
function dataGenerationSig() {
  return "datagen:" + dataGeneration;
}

// src/backend/wechat-data/src/query/messages.ts
...
var RE_CACHE_MAX = 128;
var RE_CACHE = /* @__PURE__ */ new Map();
function cachedRe(key, build) {
...
function xmlAttr(xml, tag, attr) {
  const m = xml.match(cachedRe("attr:" + tag + "\0" + attr, () => new RegExp(`<\\s*${tag}[^>]*\\b${attr}\\s*=\\s*["']([^"']*)["']`)));
  return m ? m[1] ?? "" : "";
}
...
function parseSolitaire(app) {
  ...
  while (m = re.exec(body)) {
    ...
    const ts2 = Number(xmlTagText(item, "t")) || 0;
    if (username || content) members.push({ idx: members.length + 1, username, content, ts: ts2 });
  }
...
function applyMpNews(link, app, firstTitle, firstUrl, fallbackCover) {
...
function parseAppmsg(xml) {
  ...
  const thumb = xmlTagText(app, "thumburl") || xmlTagText(app, "tpthumburl") || xmlAttr(app, "img", "cdnthumburl") || xmlTagOrAttr(app, "cdnthumburl") || xmlTagOrAttr(app, "coverurl") || xmlTagOrAttr(app, "cover") || "";
  const source = unescapeXmlEntities(
    xmlTagText(app, "sourcedisplayname") || xmlTagText(app, "sourcedisplaynick") || xmlTagText(app, "appname") || ""
  );
  ...
  switch (appType) {
    ...
    case 3: {
      ...
      const music = { type: "music", title, desc: des, url: url || xmlTagText(app, "dataurl") };
    ...
    case 33: {
      ...
      const pageCover = (weapp ? xmlTagOrAttr(weapp, "cdnthumburl") || xmlTagOrAttr(weapp, "thumburl") || xmlTagOrAttr(weapp, "coverurl") : "") || xmlTagOrAttr(app, "cdnthumburl") || xmlTagOrAttr(app, "coverurl") || "";
      const miniTitle = cleanRichText(title) || cleanRichText(des) || "[\u5C0F\u7A0B\u5E8F]";
      const miniDesc = cleanRichText(des);
      const mini = { type: "miniapp", title: miniTitle, url, ...base };
    ...
    case 8: {
      const sticker = { type: "sticker", title: title || xmlTagText(app, "md5"), md5: xmlTagText(app, "md5") || xmlTagText(app, "emoticonmd5") || "", ...base };
...
var SYS_NOISE_TAGS_UNIQUE = [...new Set(SYS_NOISE_TAGS)];
...
function parseEmbeddedPayload(body) {
  ...
  if (/<voicemsg\b/.test(body)) {
    ...
    const rich = { type: "voice", title: xmlTagText(body, "voicemsg"), md5: xmlAttr(body, "voicemsg", "md5") };
  ...
  switch (msgType) {
    case 1: {
      ...
      if (text.includes("<mmreader>")) {
        const rich = { type: "newsfeed", title: xmlTagText(text, "title"), desc: xmlTagText(text, "digest"), url: xmlTagText(text, "url") };
        return { text: "", rich };
      }
      if (text.startsWith("<")) {
    ...
    case 34: {
      ...
      const rich = { type: "voice", title: xmlTagText(body, "voicemsg"), md5 };
      if (ms > 0) rich.durationMs = ms;
      return { text: "", rich };
    }
    case 42:
    ...
    case 47: {
      ...
      const rich = { type: "emoji", title: md5 || xmlTagText(body, "emoji"), md5 };
...
function senderFromContent(content, talker) {
...
function findShards(decryptedDir, table) {
  ...
  for (const shard of shardCatalog(decryptedDir)) {
    ...
    if (!meta) continue;
    try {
      const db = new DatabaseSync2(shard.file, { readOnly: true });
...
function queryShardRowsWith(shard, table, cursor, after, limit, ascending, cursorLocalId) {
  ...
  let whereClause2 = "";
  if (after !== void 0) {
    whereClause2 = has("sort_seq") ? "WHERE sort_seq > ?" : "WHERE local_id > ?";
    params.push(after);
  } else if (cursor !== void 0) {
    if (useComposite) {
      whereClause2 = "WHERE sort_seq < ? OR (sort_seq = ? AND local_id < ?)";
  ...
  return rows.map((r) => {
    ...
    const nameVal = hasName ? shard.name2id.get(realSenderId) ?? "" : void 0;
    return {
...
function toWechatMessages(rows, talker, isGroup, contactNames, selfUsername) {
  return rows.map((r) => {
    const normType2 = normalizeMsgType(r.localType);
    const content = decodeBlobText(r.content);
    const parsed = parseMessageContent(normType2, content, isGroup);
    ...
    const render = classifyRender(normType2, parsed.rich, parsed.sysKind);
    msg.renderType = render;
    msg.renderLabel = RENDER_LABEL[render] ?? "\u672A\u77E5\u6D88\u606F";
    if (parsed.sysKind) msg.sysKind = parsed.sysKind;
    if (needsAtUsers(talker, normType2, content) && r.source) {
...
function queryMessageByServerIdUncached(decryptedDir, serverId) {
  ...
  const tryLookup = (predicate, param) => {
    for (const shard of shardCatalog(decryptedDir)) {
      let db;
      try {
        db = new DatabaseSync2(shard.file, { readOnly: true, readBigInts: true });
      } catch {
        continue;
      }
      try {
        for (const [table, meta] of shard.tables) {
          ...
          try {
            ...
            if (!row) continue;
            const normType2 = normalizeMsgType(Number(row["local_type"] ?? 0));
            const content = decodeBlobText(row["message_content"]);
            const parsed = parseMessageContent(normType2, content, false);
            const msg = {
              ...
              msgContent: content,
              strContent: content,
              typeLabel: msgTypeLabel(normType2)
            };
...
function querySessions(decryptedDir, keyword, limit, offset) {
  ...
  const sig = [
    fileSigOf(join2(decryptedDir, "session", "session.db")),
    fileSigOf(join2(decryptedDir, "contact", "contact.db")),
...
function computeSessions(decryptedDir, keyword, limit, offset = 0) {
  const dbPath6 = join2(decryptedDir, "session", "session.db");
  const db = new DatabaseSync3(dbPath6, { readOnly: true });
  try {
    ...
    const srvCol = cols.has("unread_first_msg_srv_id") ? "CAST(unread_first_msg_srv_id AS TEXT) AS unread_srv" : "'' AS unread_srv";
    const sql = [
      "SELECT",
      [
        sel("username", "''"),
...
// src/backend/wechat-data/src/query/group-info.ts
import { DatabaseSync as DatabaseSync4 } from "node:sqlite";
import { join as join4 } from "node:path";

// src/backend/wechat-data/src/query/region.ts
...
function readVarint(buf, idx) {
  ...
  for (let i = idx; i < buf.length; i += 1) {
    ...
    if ((b & 128) === 0) return { value, next: i + 1 };
    shift += 7;
  }
  return null;
}
...
function regionData() {
  ...
  const data = fromFile([
    join3(dir, "..", "..", "resources", "contact-region-lookup.json"),
    join3(dir, "..", "..", "..", "resources", "contact-region-lookup.json"),
    join3(dir, "..", "resources", "contact-region-lookup.json")
  ]);
  _regionData = data ?? {};
    ...
    try {
      const obj = JSON.parse(json);
      {
        ...
        if (Array.isArray(ci) && ci.length > 0) {
          ...
          return parts.join(" \xB7 ").slice(0, 120);
        }
...
function queryGroupInfo(decryptedDir, username, selfUsername) {
  const dbPath6 = join4(decryptedDir, "contact", "contact.db");
  let db = null;
  try {
    db = new DatabaseSync4(dbPath6, { readOnly: true });
    ...
    const pushMember = (mUsername, r) => {
      ...
      if (selfClean && cleanWxid(mUsername) === selfClean) member.isSelf = true;
      members.push(member);
    };
    if (roomId !== void 0) {
      try {
        const rows = db.prepare(
          "SELECT c.username, c.nick_name, c.remark, c.small_head_url, c.big_head_url, c.extra_buffer FROM chatroom_member m LEFT JOIN contact c ON c.id = m.member_id WHERE m.room_id = ? ORDER BY m.rowid"
        ).all(roomId);
        for (const r of rows) {
...
// src/backend/wechat-data/src/query/payments.ts
...
function queryPaymentStatus(decryptedDir, serverId) {
  const dbPath6 = join5(decryptedDir, "general", "general.db");
  if (!existsSync2(dbPath6)) return { found: false };
  const db = new DatabaseSync5(dbPath6, { readOnly: true });
  try {
    const sid = serverId;
...
// src/backend/wechat-data/src/query/contacts.ts
import { DatabaseSync as DatabaseSync6 } from "node:sqlite";
import { join as join6 } from "node:path";
function tableColumns3(db, table) {
  const rows = db.prepare(`PRAGMA table_info(${table})`).all();
  return new Set(rows.map((r) => r.name));
}
...
function queryContacts(decryptedDir, options) {
  ...
  return cachedBySig(
    ...
    fileSigOf(join6(decryptedDir, "contact", "contact.db")),
    () => computeContacts(decryptedDir, limit, offset)
  );
}
function computeContacts(decryptedDir, limit, offset = 0) {
  const db = new DatabaseSync6(join6(decryptedDir, "contact", "contact.db"), { readOnly: true });
  try {
    const cols = tableColumns3(db, "contact");
    ...
    try {
      const crCols = tableColumns3(db, "chat_room");
      ...
      const cmCols = tableColumns3(db, "chatroom_member");
    ...
    const bizTypes = /* @__PURE__ */ new Map();
    try {
      const biCols = tableColumns3(db, "biz_info");
      if (biCols.has("username") && biCols.has("type")) {
    ...
    const page = limit !== void 0 ? contacts.slice(offset, offset + limit) : contacts;
...
// src/backend/wechat-data/src/query/region-map.ts
...
function queryRegionMap(decryptedDir) {
  const dbPath6 = join7(decryptedDir, "contact", "contact.db");
  const rows = [];
  let unknown = 0;
  try {
    const db = new DatabaseSync7(dbPath6, { readOnly: true });
    try {
      ...
      for (const r of raw) {
        ...
        if (deleteFlag !== 0 || localType !== 1) continue;
...
function snsDb(decryptedDir) {
  for (const p of [join8(decryptedDir, "sns", "db_sns", "sns.db"), join8(decryptedDir, "sns", "sns.db")]) {
...
function xmlNestedText(xml, parent, child) {
  ...
  if (start < 0) return null;
  const close = xml.indexOf("</" + parent + ">", start + open2.length);
  if (close < 0) return null;
  return xmlTagText2(xml.slice(start, close + open2.length + parent.length + 3), child);
}
function tagTextFirst(xml, tags) {
  for (const t of tags) {
    const v = xmlTagText2(xml, t);
    if (v != null && v.trim() !== "") return unescapeXml(v).trim();
  }
  return "";
}
var MEDIA_OPEN = "<media";
...
function parseSnsXml(xml) {
  ...
  const mediaDesc = mediaCount === 0 ? "" : hasVideo ? "\u89C6\u9891" : `\u56FE\u7247\xD7${mediaCount}`;
  const images = [];
  const videos = [];
  ...
  const linkUrl = unescapeXml(xmlTagText2(xml, "contentUrl") ?? "") || unescapeXml(xmlTagText2(xml, "ContentUrl") ?? "") || "";
...
function parseSnsLikesComments(xml) {
  ...
  for (; ; ) {
    ...
    raws.push({
      id: xmlTagText2(b, "comment_id") ?? "",
      username: xmlTagText2(b, "username") ?? "",
      nickname: unescapeXml(xmlTagText2(b, "nickname") ?? ""),
      content: unescapeXml(xmlTagText2(b, "content") ?? ""),
      ts: Number.parseInt(xmlTagText2(b, "create_time") ?? "0", 10) || 0,
      ref: xmlTagText2(b, "ref_comment_id") ?? "0",
      type: Number.parseInt(xmlTagText2(b, "type") ?? "0", 10) || 0,
      block: b
    });
    pos = bc + 15;
  }
  ...
  for (const r of raws) {
    ...
    if (ii >= 0) {
      ...
      const image = {};
      const iu = unescapeXml(xmlTagText2(ib, "url") ?? "");
      if (iu) image.url = iu;
      const it = unescapeXml(xmlTagText2(ib, "thumb_url") ?? "");
      if (it) image.thumb = it;
      const imd = xmlTagText2(ib, "md5") ?? "";
      if (imd) image.md5 = imd;
      const imid = xmlTagText2(ib, "media_id") ?? "";
...
function queryMoments(decryptedDir, offset, limit, authorUsername, selfUsername) {
  const dbPath6 = snsDb(decryptedDir);
  if (dbPath6 === null) return { moments: [], total: 0 };
  const db = new DatabaseSync8(dbPath6, { readOnly: true });
  try {
    ...
    const moments = rows.map((r) => {
      const username = cellString2(r.u);
      const xml = cellString2(r.c);
...
function queryMomentsAuthors(decryptedDir) {
  ...
  try {
    ...
    const names = contactMeta(decryptedDir).names;
    return rows.map((r) => ({ name: names.get(cellString2(r.u)) || cellString2(r.u) || "\u672A\u77E5", count: r.n ?? 0 }));
  } finally {
    db.close();
  }
}
...
function xmlTagAttr2(xml, tag, attr) {
  const openExact = "<" + tag + ">";
  const openAttr = "<" + tag + " ";
  const start = xml.indexOf(openExact) >= 0 ? xml.indexOf(openExact) : xml.indexOf(openAttr);
  if (start < 0) return null;
...
function xmlNestedText2(xml, parent, child) {
  const open2 = "<" + parent + ">";
  const start = xml.indexOf(open2);
  if (start < 0) return null;
  const close = xml.indexOf("</" + parent + ">", start + open2.length);
  if (close < 0) return null;
  return xmlTagText3(xml.slice(start, close + open2.length + parent.length + 3), child);
}
...
function parseFavContent(favType, xml) {
  ...
  const t = "title";
  const d = "desc";
  const title = xmlTagText3(xml, t) ?? "";
  const descRaw = xmlTagText3(xml, d) ?? "";
  if (favType === 1) {
    const text = decodeFavText(descRaw || (xml.includes("<") ? stripXmlTags2(xml) : xml));
    return { title: firstLineTitle(text), desc: text, url: "" };
  }
  if (favType === 8) {
    const name = descRaw || xmlTagText3(xml, "datatitle") || "";
    return { title: decodeFavText(name), desc: decodeFavText(descRaw), url: "" };
  }
  if (favType === 5) {
    const url = (xmlTagText3(xml, "url") ?? "").replace(/&amp;/g, "&");
    const linkTitle = title || xmlTagText3(xml, "datatitle") || "";
    const linkDesc = descRaw || xmlTagText3(xml, "datadesc") || "";
    return { title: decodeFavText(linkTitle), desc: decodeFavText(linkDesc), url };
  }
  if (favType === 14 || favType === 18) {
    const rt = xmlNestedText2(xml, "recordinfo", t) || title;
    const rd = xmlNestedText2(xml, "recordinfo", d) || descRaw;
    return { title: rt, desc: decodeFavText(rd), url: "" };
  }
  if (favType === 6) {
    const name = xmlTagText3(xml, "poiname") || title || xmlTagText3(xml, "label") || "";
    const label = xmlTagText3(xml, "label") || descRaw || "";
...
function parseFavParts(xml) {
  ...
  for (; ; ) {
    ...
    const datatype = Number.parseInt(xmlTagAttr2(body, "dataitem", "datatype") ?? "0", 10) || 0;
    const dataid = (xmlTagAttr2(body, "dataitem", "dataid") ?? "").trim().toLowerCase();
    const md5 = (xmlTagText3(body, "fullmd5") ?? "").trim().toLowerCase() || dataid;
    const thumbMd5 = (xmlTagText3(body, "thumbfullmd5") ?? "").trim().toLowerCase();
    const text = decodeFavText(xmlTagText3(body, "datadesc") ?? "") || decodeFavText(xmlTagText3(body, "datatitle") ?? "");
    const sourceName = xmlTagText3(body, "datasrcname") ?? "";
    const sourceTime = xmlTagText3(body, "datasrctime") ?? "";
    const sourceHead = xmlTagText3(body, "sourceheadurl") ?? "";
    const metaPart = () => {
      const p = { kind: "text" };
    ...
    if (datatype === 1) {
      ...
      p.kind = "video";
      if (md5) p.md5 = md5;
      const dur = Number.parseFloat(xmlTagText3(body, "duration") ?? "0");
      ...
      p.kind = "link";
      if (text) p.text = text;
      const u = (xmlTagText3(body, "stream_weburl") ?? xmlTagText3(body, "url") ?? "").replace(/&amp;/g, "&");
      if (u) p.url = u;
      pushPart(p);
    } else if (datatype === 8) {
      const p = metaPart();
      p.kind = "file";
      const n = xmlTagText3(body, "datatitle") ?? "";
      if (n) p.name = n;
      const e = xmlTagText3(body, "datafmt") ?? "";
      if (e) p.ext = e;
      const sz = Number.parseInt(xmlTagText3(body, "fullsize") ?? "0", 10);
      if (sz > 0) p.size = sz;
      pushPart(p);
    }
...
function deleteFavoriteItems(decryptedDir, ids) {
  ...
  const dbPath6 = join9(decryptedDir, "favorite", "favorite.db");
  if (!existsSync4(dbPath6)) return { ok: false, deleted: 0, error: "\u6536\u85CF\u5E93\u4E0D\u5B58\u5728" };
  try {
    const db = new DatabaseSync9(dbPath6);
    const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='fav_db_item'").get() !== void 0;
    if (!has) {
      db.close();
...
function loadFileSources(decryptedDir) {
  ...
  let resDb = null;
  try {
    resDb = new DatabaseSync10(join10(decryptedDir, "message", "message_resource.db"), { readOnly: true });
  } catch {
    return map;
  }
  try {
    ...
    for (const r of chatRows) chatMap.set(Number(r.rowid), cellStr2(r.user_name));
...
var FILE_SOURCES_TTL_MS = 3e4;
var fileSourcesCache = null;
function fileSourcesSig(decryptedDir) {
  try {
    const st = statSync3(join10(decryptedDir, "message", "message_resource.db"));
    return `${st.mtimeMs}:${st.size}`;
  } catch {
    return "";
  }
...
var DIR_SOURCE_TTL_MS = 3e4;
var dirSourceCache = null;
function dirSourceCached(decryptedDir) {
  let sig = "";
  try {
    const st = statSync3(join10(decryptedDir, "hardlink", "hardlink.db"));
  ...
  try {
    ...
    try {
      ...
      for (const [user, display] of names) {
        if (!user) continue;
        const h = createHash2("md5").update(user, "utf8").digest("hex");
    ...
    if (parts.length > 0) {
      try {
        ...
        for (const r of rows) {
          ...
          const item = {
            md5: cellStr2(r.md5 ?? ""),
            fileName: cellStr2(r.file_name ?? ""),
            fileSize: Number(r.file_size ?? 0),
            modifyTime: Number(r.modify_time ?? 0),
            category: cellStr2(r.category ?? ""),
...
function parsePackedName(blob) {
  ...
  const walk = (start, end, level) => {
    let i = start;
    while (i < end) {
      const tag = readVarint2(blob, i);
      if (!tag) return;
      i = tag.next;
      const field = tag.value >>> 3;
      const wire = tag.value & 7;
      if (wire === 2) {
        const len = readVarint2(blob, i);
        if (!len) return;
        i = len.next;
...
function countRevokedAcrossShards(dec) {
  const msgDir = join11(dec, "message");
  if (!existsSync5(msgDir)) return 0;
  let total = 0;
  for (const file of readdirSync2(msgDir)) {
    if (!file.endsWith(".db") || file.includes("_shm") || file.includes("_wal") || file.includes("tmp")) continue;
    try {
      const db = new DatabaseSync11(join11(msgDir, file), { readOnly: true });
      try {
...
function queryOverview(decryptedDir) {
  ...
  const sig = [
    fileSigOf(join11(dec, "session", "session.db")),
    fileSigOf(join11(dec, "contact", "contact.db")),
    fileSigOf(join11(dec, "sns", "sns.db")),
    fileSigOf(join11(dec, "sns", "db_sns", "sns.db")),
    fileSigOf(join11(dec, "favorite", "favorite.db")),
    fileSigOf(join11(dec, "emoticon", "emoticon.db")),
    fileSigOf(join11(dec, "message", "message_resource.db")),
...
function computeOverview(dec) {
  ...
  const sessions = countRows(join11(dec, "session", "session.db"), "SessionTable");
  const groups = cstats.group ?? 0;
  const contacts = cstats.friend ?? 0;
  const official = (cstats.official ?? 0) + (cstats.service ?? 0);
  const moments = countRows(join11(dec, "sns", "sns.db"), "SnsTimeLine");
  const favorites = countRows(join11(dec, "favorite", "favorite.db"), "fav_db_item");
  const emoticons = countRows(join11(dec, "emoticon", "emoticon.db"), "kNonStoreEmoticonTable");
  const revoked = countRevokedAcrossShards(dec);
  let total_size = 0;
  let total_count = 0;
  const categories = [];
  const resourcePath = join11(dec, "message", "message_resource.db");
  if (existsSync5(resourcePath)) {
    try {
      const db = new DatabaseSync11(resourcePath, { readOnly: true });
  ...
  const snsPath = existsSync5(join11(dec, "sns", "db_sns", "sns.db")) ? join11(dec, "sns", "db_sns", "sns.db") : join11(dec, "sns", "sns.db");
  if (existsSync5(snsPath)) {
    try {
      const db = new DatabaseSync11(snsPath, { readOnly: true });
      const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='SnsTimeLine'").get() !== void 0;
      if (has) {
        const cols = new Set(db.prepare("PRAGMA table_info(SnsTimeLine)").all().map((r) => r.name));
        const uname = cols.has("user_name") ? "user_name" : "userName";
        ...
        for (const r of db.prepare(sql).iterate()) {
          const u = cellStr3(r.u);
          if (!u) continue;
          const cur = authorMap.get(u);
          if (cur) cur.posts += 1;
          else authorMap.set(u, { name: names.get(u) || xmlNickname(cellStr3(r.c)) || u, posts: 1 });
        }
        const sorted = Array.from(authorMap.entries()).map(([username, v]) => ({ username, name: v.name, posts: v.posts })).sort((a, b) => b.posts - a.posts).slice(0, 20);
...
function findRevokeTable(decryptedDir) {
  const msgDir = join12(decryptedDir, "message");
  if (!existsSync6(msgDir)) return { db: null, table: "" };
  for (const file of readdirSync3(msgDir)) {
    if (!file.endsWith(".db") || file.includes("_shm") || file.includes("_wal")) continue;
    try {
      const db = new DatabaseSync12(join12(msgDir, file), { readOnly: true });
...
function queryRecords(decryptedDir, kind, limit, offset, q, opts) {
  const gdb = join12(decryptedDir, "general", "general.db");
  if (!existsSync6(gdb)) return { items: [], total: 0 };
  const db = new DatabaseSync12(gdb, { readOnly: true });
  try {
    const cap = Math.min(limit ?? 50, 500);
    const off = offset ?? 0;
    const kw = (q ?? "").trim();
    ...
    if (kind === "miniprograms") {
      ...
      const whereSql2 = combineWhere(baseWhere, s.timeCol ? timeSql("a." + s.timeCol) : "");
      const dir = direction === "asc" ? "ASC" : "DESC";
      const sql = "SELECT " + sel + " FROM wacontact w LEFT JOIN WeAppBizAttrSyncBufferTableV02 a ON a.user_name = w.user_name" + whereSql2 + " ORDER BY COALESCE(a.last_update_time, 0) " + dir + ", w.user_name ASC LIMIT ? OFFSET ?";
      let items2 = [];
      try {
        items2 = db.prepare(sql).all(cap, off);
      } catch {
        items2 = [];
      }
      for (const it of items2) {
        ...
        try {
          ...
          nickname = cellString3(v.RegisterSource?.NickName || v.NickName || "");
        } catch {
        }
        it["nickname"] = nickname;
      }
      enrichRecords(items2, kind, names);
      let total = 0;
      try {
        total = db.prepare("SELECT COUNT(*) AS n FROM wacontact w LEFT JOIN WeAppBizAttrSyncBufferTableV02 a ON a.user_name = w.user_name" + whereSql2).get().n;
      } catch {
        total = 0;
      }
...
function parseRevokeContent(raw, fallbackSender) {
  if (raw == null) return { sender: fallbackSender, content: "\uFF08\u65E0\u5185\u5BB9\u526F\u672C\uFF09" };
...
function tableColumns4(db, table) {
  try {
    const rows = db.prepare(`PRAGMA table_info(${table})`).all();
    return new Set(rows.map((r) => r.name));
  } catch {
...
function queryEmoticons(decryptedDir, limit, offset = 0) {
  return cachedBySig(
    ...
    fileSigOf(join13(decryptedDir, "emoticon", "emoticon.db")),
    () => computeEmoticons(decryptedDir, limit, offset)
  );
}
function computeEmoticons(decryptedDir, limit, offset = 0) {
  const path = join13(decryptedDir, "emoticon", "emoticon.db");
  if (!existsSync7(path)) return { custom: [], static: [], packages: [], total: 0, orderedBy: "builtin" };
  const db = new DatabaseSync13(path, { readOnly: true });
  try {
    const custom = [];
    let customOrderedBy = "builtin";
    const cCols = tableColumns4(db, "kNonStoreEmoticonTable");
    if (cCols.size > 0) {
      ...
      if (md5Col) {
        ...
        const hasOrder = tableColumns4(db, "kFavEmoticonOrderTable").has("md5");
        let rows;
        let orderedBy = "builtin";
        if (hasOrder) {
        ...
        for (const r of rows) {
          const md5 = cellStr4(r.md5 ?? "").trim();
          if (!md5) continue;
          const item = { md5, item_type: Number(r.item_type ?? 0) };
          const caption = cellStr4(r.caption ?? "").trim();
          if (caption) item.caption = caption;
          const cdnUrl = (cellStr4(r.cdn_url ?? "").trim() || cellStr4(r.thumb_url ?? "").trim()).replace(/&amp;/g, "&").replace(/&#x26;/gi, "&");
    ...
    const packages = [];
    const pCols = tableColumns4(db, "kStoreEmoticonPackageTable");
    if (pCols.size > 0) {
      ...
      if (idCol && nameCol) {
        ...
        for (const r of rows) {
          const pid = cellStr4(r.pid ?? "").trim();
          const name = cellStr4(r.name ?? "").trim() || pid || "\u672A\u547D\u540D\u8868\u60C5\u5305";
          let count = 0;
          if (pid) {
            try {
              const fcols = tableColumns4(db, "kStoreEmoticonFilesTable");
              if (fcols.size > 0) {
...
var ZSTD_MAGIC2 = Buffer.from([40, 181, 47, 253]);
function decodeMsgText(v) {
  if (v === null || v === void 0) return "";
  if (typeof v === "string") return v;
  const raw = Buffer.from(v instanceof Uint8Array ? v : []);
  const bytes = raw.length >= 4 && raw.subarray(0, 4).equals(ZSTD_MAGIC2) ? (() => {
    try {
      return Buffer.from(decompress2(raw));
    } catch {
      return raw;
    }
  })() : raw;
...
function findMsgShard(dec, username) {
  const table = "Msg_" + createHash3("md5").update(username, "utf8").digest("hex");
...
function resolveFileTitle(dec, username, svr) {
  ...
  if (!sh) return "";
  try {
    const db = new DatabaseSync14(sh.db, { readOnly: true });
    try {
...
function fileNamesSig(root) {
  const parts = [fileSigOf(root)];
  try {
    for (const e of readdirSync4(root, { withFileTypes: true })) {
      if (!e.isDirectory()) continue;
      parts.push(`${e.name}:${fileSigOf(join14(root, e.name))}`);
    }
  } catch {
  }
  return parts.join("|");
...
function loadFileNamesBySize(wechatBaseDir) {
  ...
  const root = join14(wechatBaseDir, "msg", "file");
  if (!existsSync8(root)) return /* @__PURE__ */ new Map();
  return cachedBySig("storage-file-names:" + root, fileNamesSig(root), () => {
    const map = /* @__PURE__ */ new Map();
    try {
      for (const e of readdirSync4(root, { withFileTypes: true })) {
        if (!e.isDirectory()) continue;
        const sub = join14(root, e.name);
        for (const f of readdirSync4(sub, { withFileTypes: true })) {
          if (f.isDirectory()) continue;
          const p = join14(sub, f.name);
          try {
...
function queryStorageStats(decryptedDir, wechatBaseDir) {
  ...
  if (!existsSync8(path)) return { total_size: 0, total_count: 0, categories: [], chats: [], senders: [], large_files: [] };
  const db = new DatabaseSync14(path, { readOnly: true });
  try {
    const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='MessageResourceDetail'").get() !== void 0;
...
function computeAnnual(decryptedDir) {
  const msgDir = join15(decryptedDir, "message");
  if (!existsSync9(msgDir)) return { years: [] };
  const years = /* @__PURE__ */ new Set();
  const files = readdirSync5(msgDir).filter((f) => f.endsWith(".db") && !f.includes("_shm") && !f.includes("_wal") && !f.includes("monitor_cache") && !f.includes("fts"));
  for (const file of files) {
    let db = null;
    try {
      db = new DatabaseSync15(join15(msgDir, file), { readOnly: true });
    } catch {
      continue;
    }
    try {
...
import { readFileSync as readFileSync2, existsSync as existsSync10 } from "node:fs";
import { join as join16 } from "node:path";
function configPath(decryptedDir) {
  const owned = join16(decryptedDir, "..", "config.json");
  return existsSync10(owned) ? owned : null;
}
...
// src/backend/wechat-data/src/config/atomic-json.ts
import { readFileSync as readFileSync3, readdirSync as readdirSync6, renameSync, rmSync, statSync as statSync5, writeFileSync } from "node:fs";
import { basename, dirname as dirname2, join as join17 } from "node:path";
...
// src/backend/wechat-data/src/config/wechat-config.ts
import { chmodSync, existsSync as existsSync11, mkdirSync, readFileSync as readFileSync4, readdirSync as readdirSync7, rmSync as rmSync2, statSync as statSync6 } from "node:fs";
import { dirname as dirname3, join as join18 } from "node:path";
function configPath2(decryptedDir) {
  return join18(decryptedDir, "..", "config.json");
}
...
function secretsPath(decryptedDir) {
  return join18(decryptedDir, "..", "secrets.json");
}
...
function getConfig(decryptedDir) {
  ...
  const wechatRoot = join18(decryptedDir, "..");
  const resolved = {
    decrypted_dir: decryptedDir,
    decoded_image_dir: join18(wechatRoot, "decoded_images"),
    keys_file: join18(wechatRoot, "all_keys.json")
  };
...
function saveConfig(decryptedDir, patch) {
  ...
  try {
    ...
    if (aesChanged || xorChanged) clearDecodedImages(join18(decryptedDir, "..", "decoded_images"));
    return { ok: true };
  } catch (e) {
...
function clearDecodedImages(dir) {
  try {
    ...
    for (const name of readdirSync7(dir)) {
      try {
        rmSync2(join18(dir, name), { recursive: true, force: true });
      } catch {
      }
    }
  } catch {
...
// src/backend/wechat-data/src/config/detect.ts
import { execFileSync } from "node:child_process";
import { existsSync as existsSync12, readdirSync as readdirSync8, readFileSync as readFileSync5, statSync as statSync7 } from "node:fs";
import { join as join19 } from "node:path";
var DEFAULT_DATA_BASES = [
  "E:\\Tencent",
  "D:\\Tencent",
  "C:\\Tencent",
  process.env.USERPROFILE ? join19(process.env.USERPROFILE, "Tencent") : "",
  process.env.USERPROFILE ? join19(process.env.USERPROFILE, "Documents") : ""
].filter(Boolean);
function iniDataBases() {
  const configDir = process.env.APPDATA ? join19(process.env.APPDATA, "Tencent", "xwechat", "config") : "";
  if (!configDir || !existsSync12(configDir)) return [];
  const bases = [];
  for (const name of readdirSync8(configDir)) {
    if (!name.endsWith(".ini")) continue;
    try {
      for (const line of readFileSync5(join19(configDir, name), "utf8").split(/\r?\n/)) {
...
function collectScanRoots() {
  ...
  const add = (base) => {
    if (!base) return;
    for (const root of [join19(base, "xwechat_files"), base]) {
...
function detectWechatAccounts(roots) {
  ...
  for (const root of roots ?? collectScanRoots()) {
    ...
    for (const entry of readdirSync8(root, { withFileTypes: true })) {
      ...
      const dir = join19(root, entry.name);
      const dbStorage = join19(dir, "db_storage");
      if (!existsSync12(dbStorage)) continue;
      const wxid = normalizeWxidDir(entry.name) || "\u672A\u77E5\u8D26\u53F7";
      let lastActive;
      try {
        const msgDir = join19(dbStorage, "message");
        if (existsSync12(msgDir)) {
          let newest = 0;
          for (const f of readdirSync8(msgDir)) {
            try {
              newest = Math.max(newest, statSync7(join19(msgDir, f)).mtimeMs);
            } catch {
            }
          }
...
function scanDbFiles(dir, depth = 0) {
  if (depth > 4 || !existsSync12(dir)) return [];
  const out = [];
  try {
    for (const e of readdirSync8(dir, { withFileTypes: true })) {
...
// src/backend/wechat-data/src/config/resolve.ts
...
function getKeysInfo(decryptedDir) {
  const p = join20(decryptedDir, "..", "all_keys.json");
  if (!existsSync13(p)) return { keyCount: 0, loaded: false };
  try {
    const raw = JSON.parse(readFileSync6(p, "utf8"));
...
function resolveSelfUsername(decryptedDir) {
  ...
  try {
    const keysPath = join20(decryptedDir, "..", "all_keys.json");
    if (existsSync13(keysPath)) {
      const raw = JSON.parse(readFileSync6(keysPath, "utf8"));
...
// src/backend/wechat-data/src/keys/db-key-verify.ts
import { createDecipheriv, createHmac, pbkdf2Sync } from "node:crypto";
import { closeSync, existsSync as existsSync14, mkdirSync as mkdirSync2, openSync, readSync, writeFileSync as writeFileSync2 } from "node:fs";
import { dirname as dirname4, relative } from "node:path";
var PAGE_SZ = 4096;
var SALT_SZ = 16;
var IV_SZ = 16;
var HMAC_SZ = 64;
var RESERVE_SZ = 80;
var PBKDF2_ITERS = 256e3;
var SQLITE_HDR = new TextEncoder().encode("SQLite format 3\0");
...
function verifyDbKey(page1, wxKeyBin) {
  ...
  const macSalt = Buffer.from(salt.map((b) => b ^ 58));
...
function verifyDatabaseKey(dbPath6, encKeyHex) {
  if (!existsSync14(dbPath6)) return { valid: false, error: "\u6570\u636E\u5E93\u6587\u4EF6\u4E0D\u5B58\u5728" };
  ...
  try {
    const page1 = readFirstPage(dbPath6);
...
function generateKeysFile(dbDir, keysFile, encKeyHex, keyFormat) {
  ...
  for (const db of dbs) {
    try {
      ...
      if (valid) verified += 1;
      entries2[rel] = { key: raw, valid };
    } catch {
    }
  }
  const payload = { ...entries2, _key_format: keyFormat ?? "wx_key_v4.1", _db_dir: dbDir };
  try {
    mkdirSync2(dirname4(keysFile), { recursive: true });
    writeFileSync2(keysFile, JSON.stringify(payload, null, 2), "utf8");
...
var IV_SZ2 = 16;
var RESERVE_SZ2 = 80;
var PBKDF2_ITERS2 = 256e3;
var SQLITE_HDR2 = Buffer.from("SQLite format 3\0");
var WAL_HEADER_SZ = 32;
var WAL_FRAME_HEADER_SZ = 24;
var SYNC_INTERVAL_MS = 1e4;
var FULL_DECRYPT_COOLDOWN_MS = 6e3;
var dirEntryCache = /* @__PURE__ */ new Map();
function cachedDirEntries(dir) {
  try {
    const st = statSync8(dir);
...
function readPrefix(file, n) {
  ...
  try {
    while (filled < n) {
      const r = readSync2(fd, buf, filled, n - filled, null);
      if (r === 0) break;
      filled += r;
    }
  } finally {
...
function deriveEncKey(rawKey, salt, keyFormat) {
  if (keyFormat === "wx_key_v4.1") {
    return pbkdf2Sync2(rawKey, salt, PBKDF2_ITERS2, 32, "sha512");
  }
  return rawKey;
}
function decryptPage(encKey, page, pgno) {
  const iv = page.subarray(PAGE_SZ2 - RESERVE_SZ2, PAGE_SZ2 - RESERVE_SZ2 + IV_SZ2);
  const encrypted = pgno === 1 ? page.subarray(SALT_SZ2, PAGE_SZ2 - RESERVE_SZ2) : page.subarray(0, PAGE_SZ2 - RESERVE_SZ2);
  const decipher = createDecipheriv2("aes-256-cbc", encKey, iv);
  decipher.setAutoPadding(false);
  const dec = Buffer.concat([decipher.update(encrypted), decipher.final()]);
  const out = Buffer.alloc(PAGE_SZ2);
  if (pgno === 1) SQLITE_HDR2.copy(out, 0);
  dec.copy(out, pgno === 1 ? SQLITE_HDR2.length : 0);
  return out;
}
var DECRYPT_CHUNK_PAGES = 64;
async function fullDecryptFile(dbPath6, outPath, encKey) {
  const input = await open(dbPath6, "r");
  ...
  try {
    for (; ; ) {
      ...
      if (filled === 0) break;
      for (let offset = 0; offset < filled; offset += PAGE_SZ2) {
        const page = rawChunk.subarray(offset, offset + PAGE_SZ2);
        const padded = page.length < PAGE_SZ2 ? Buffer.concat([page, Buffer.alloc(PAGE_SZ2 - page.length)]) : page;
...
async function decryptWalPatch(walPath, outPath, encKey) {
  if (!existsSync15(walPath)) return 0;
  ...
  try {
    ...
    while (offset + frameSize <= wal.length) {
      ...
      const ep = wal.subarray(offset + WAL_FRAME_HEADER_SZ, offset + WAL_FRAME_HEADER_SZ + PAGE_SZ2);
      const dec = decryptPage(encKey, ep, pgno);
      await fh.write(dec, 0, PAGE_SZ2, (pgno - 1) * PAGE_SZ2);
      patched += 1;
      offset += frameSize;
    }
  } finally {
...
async function walFramesPatchable(walPath) {
  if (!existsSync15(walPath)) return false;
  const wal = await readFile(walPath);
...
function looksDecrypted(dbPath6) {
  try {
    const head = readPrefix(dbPath6, 16);
...
async function atomicReplace(temp, target) {
  ...
  for (let i = 0; i < 8; i += 1) {
    try {
      ...
      lastErr = e;
      await new Promise((resolve3) => {
        setTimeout(resolve3, 120);
      });
    }
  }
  throw lastErr instanceof Error ? lastErr : new Error("atomicReplace failed");
}
var allKeysCache = /* @__PURE__ */ new Map();
var lastFullAt = /* @__PURE__ */ new Map();
...
function readAllKeysCached(decryptedDir) {
  const keysPath = join21(decryptedDir, "..", "all_keys.json");
  let sig = "";
  try {
    const st = statSync8(keysPath);
    sig = `${st.mtimeMs}:${st.size}`;
  } catch {
    return null;
  }
  ...
  let raw = null;
  try {
    raw = JSON.parse(readFileSync7(keysPath, "utf8"));
  } catch {
  }
...
function shardSig(msgDir, file) {
  ...
  for (const f of [file, file + "-wal"]) {
    try {
      const ms = statSync8(join21(msgDir, f)).mtimeMs;
...
async function stageWal(rawWal, stagingWal) {
  if (existsSync15(rawWal) && statSync8(rawWal).size > WAL_HEADER_SZ) await copyFile(rawWal, stagingWal);
  else if (existsSync15(stagingWal)) await unlink(stagingWal).catch(() => {
  });
}
async function syncMessageShard(rawMsgDir, decMsgDir, file, rawKeyHex, keyFormat, mode = "full") {
  const relKey = "message/" + file;
  const rawDb = join21(rawMsgDir, file);
  const rawWal = join21(rawMsgDir, file + "-wal");
  const target = join21(decMsgDir, file);
  ...
  if (mode === "wal") {
    ...
    try {
      const walPages = existsSync15(stagingWal) ? await decryptWalPatch(stagingWal, stagingDb, encKey) : 0;
  ...
  try {
    ...
    const walPages = existsSync15(stagingWal) ? await decryptWalPatch(stagingWal, temp, encKey) : 0;
...
async function syncChangedShards(rawDbDir, decryptedDir, lastState) {
  const msgDir = join21(rawDbDir, "message");
  const decMsgDir = join21(decryptedDir, "message");
  if (!existsSync15(msgDir)) return [];
  const cfg = getConfig(decryptedDir);
  ...
  for (const f of cachedDirEntries(msgDir)) {
    ...
    const target = join21(decMsgDir, f);
    if (prev !== void 0 && prev.main === sig.main && prev.wal === sig.wal && existsSync15(target)) continue;
    lastState.set(f, sig);
    const mode = prev === void 0 || prev.main !== sig.main || !existsSync15(target) ? "full" : "wal";
...
function sessionSig(sessionDir) {
  ...
  for (const f of ["session.db", "session.db-wal"]) {
    try {
      const ms = statSync8(join21(sessionDir, f)).mtimeMs;
...
async function syncSessionDb(rawDbDir, decryptedDir, lastState, rawKeyHex, keyFormat) {
  const rawSess = join21(rawDbDir, "session");
  const decSess = join21(decryptedDir, "session");
  const rawDb = join21(rawSess, "session.db");
  if (!existsSync15(rawDb)) return [];
  const sig = sessionSig(rawSess);
  const prev = lastState.get("session.db");
  const target = join21(decSess, "session.db");
  if (prev !== void 0 && prev.main === sig.main && prev.wal === sig.wal && existsSync15(target)) return [];
...
function contactSig(contactDir) {
  ...
  for (const f of ["contact.db", "contact.db-wal"]) {
    try {
      const ms = statSync8(join21(contactDir, f)).mtimeMs;
...
async function syncContactDb(rawDbDir, decryptedDir, lastState, rawKeyHex, keyFormat) {
  const rawContact = join21(rawDbDir, "contact");
  const decContact = join21(decryptedDir, "contact");
  const rawDb = join21(rawContact, "contact.db");
  if (!existsSync15(rawDb)) return [];
  const sig = contactSig(rawContact);
  const prev = lastState.get("contact.db");
  const target = join21(decContact, "contact.db");
  if (prev !== void 0 && prev.main === sig.main && prev.wal === sig.wal && existsSync15(target)) return [];
...
function plainDbSig(dir, dbName) {
  ...
  for (const f of [dbName, dbName + "-wal"]) {
    try {
      const ms = statSync8(join21(dir, f)).mtimeMs;
...
async function syncPlainDatabase(rawRoot, decryptedDir, sub, dbName, label, rawKeyHex, keyFormat, lastState) {
  const rawDir = join21(rawRoot, sub);
  const decDir = join21(decryptedDir, sub);
  const rawDb = join21(rawDir, dbName);
  if (!existsSync15(rawDb)) return [];
  const sig = plainDbSig(rawDir, dbName);
  const prev = lastState.get(label);
  const target = join21(decDir, dbName);
  if (prev !== void 0 && prev.main === sig.main && prev.wal === sig.wal && existsSync15(target)) return [];
    ...
    try {
      ...
      if (!raw || !dec) {
        if (!unconfiguredNoted) {
          ...
          console.log(
            "[wechat-sync] realtime sync idle: no data source configured",
      ...
      if (!existsSync15(raw)) {
        if (!unavailableWarned) {
          ...
          console.warn(
            "[wechat-sync] realtime sync paused: raw db dir unavailable",
...
var CATEGORIES = [
  ...
  { key: "bank_card", label: "\u94F6\u884C\u5361\u53F7", icon: "\u{1F4B3}", re: /(?:62\d{14,17}|[45]\d{15,18})/g, insensitive: false },
  { key: "email", label: "\u90AE\u7BB1", icon: "\u2709\uFE0F", re: /[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/g, insensitive: true },
  { key: "password", label: "\u5BC6\u7801\u53E3\u4EE4", icon: "\u{1F511}", re: /(?:密码|口令|pwd|password|passwd)\s*[=:：]\s*[A-Za-z0-9@#$%^&*!_.-]{4,32}/g, insensitive: true },
  { key: "address", label: "\u5730\u5740\u4FE1\u606F", icon: "\u{1F4CD}", re: /(?:住在|地址|住址|小区|门牌号|大厦|宿舍)[^\n，。！？]{2,60}/g, insensitive: false }
];
...
function loadSessionUsernames(decryptedDir) {
  const out = [];
  try {
    const db = new DatabaseSync16(join22(decryptedDir, "session", "session.db"), { readOnly: true });
...
function loadDisplayNames(decryptedDir) {
  const map = /* @__PURE__ */ new Map();
  try {
    const db = new DatabaseSync16(join22(decryptedDir, "contact", "contact.db"), { readOnly: true });
    ...
    for (const r of rows) {
      const u = cellStr5(r["username"] ?? "");
      const n = cellStr5(r["n"] ?? "").trim();
      if (u && n) map.set(u, n);
    }
    db.close();
  } catch {
...
function queryPrivacyScan(decryptedDir, rowBudget = 6e5) {
  const msgDir = join22(decryptedDir, "message");
  ...
  let scanned = 0;
  let involved = 0;
  if (existsSync16(msgDir)) {
    const shards = readdirSync10(msgDir).filter((f) => f.endsWith(".db") && !f.includes("_shm") && !f.includes("_wal") && !f.includes("tmp"));
    const targetUsernames = usernames.slice(0, 800);
    const tableToUser = /* @__PURE__ */ new Map();
    for (const username of targetUsernames) tableToUser.set("Msg_" + createHash4("md5").update(username, "utf8").digest("hex"), username);
    const fileInfos = [];
    for (const file of shards) {
      let probe = null;
      try {
        probe = new DatabaseSync16(join22(msgDir, file), { readOnly: true });
      } catch {
        continue;
      }
      try {
        const names2 = probe.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'Msg_%'").all().map((r) => r.name);
        const tables = [];
        for (const n of names2) {
          const u = tableToUser.get(n);
          if (u) tables.push([n, u]);
        }
        if (tables.length > 0) fileInfos.push({ path: join22(msgDir, file), tables });
      } catch {
      } finally {
        probe.close();
      }
    ...
    for (const fi of fileInfos) {
      ...
      let db = null;
      try {
        db = new DatabaseSync16(fi.path, { readOnly: true });
      } catch {
        continue;
      }
      try {
  ...
  const topContacts = Array.from(perContact.entries()).filter(([u]) => !u.endsWith("@chatroom") && !u.startsWith("gh_")).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])).slice(0, 10).map(([username, count]) => ({ username, name: names.get(username) ?? username, count }));
  const topGroups = Array.from(perContact.entries()).filter(([u]) => u.endsWith("@chatroom")).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])).slice(0, 10).map(([username, count]) => ({ username, name: names.get(username) ?? username, count }));
...
import { decompress as decompress3 } from "fzstd";
import { join as join23 } from "node:path";
var ZSTD_MAGIC3 = Buffer.from([40, 181, 47, 253]);
...
function decodeContent(v) {
  ...
  const bytes = raw.length >= 4 && raw.subarray(0, 4).equals(ZSTD_MAGIC3) ? (() => {
    try {
      return Buffer.from(decompress3(raw));
    } catch {
      return raw;
    }
  })() : raw;
...
function md5Index(decryptedDir) {
  const names = contactMeta(decryptedDir).names;
  const map = /* @__PURE__ */ new Map();
  for (const u of names.keys()) map.set(createHash5("md5").update(u, "utf8").digest("hex"), u);
  return map;
}
function scanCalls(decryptedDir, selfUsername) {
  ...
  for (const shard of shardCatalog(decryptedDir)) {
    ...
    let db;
    try {
      db = new DatabaseSync17(shard.file, { readOnly: true });
    } catch {
      continue;
    }
    try {
      for (const table of tables) {
        ...
        for (const r of rows) {
          ...
          const connected = durationSec !== void 0 || ANSWERED_ELSEWHERE.test(status);
...
function buildSnapshot(raw, names, topPeers, recentLimit) {
  ...
  const peers = [...byPeer.entries()].map(([username, p]) => ({ username, name: names.get(username) ?? username, ...p })).sort((a, b) => b.calls - a.calls || b.durationSec - a.durationSec);
  const months = [...byMonth.entries()].map(([month, m]) => ({ month, ...m })).sort((a, b) => a.month < b.month ? -1 : 1);
...
function queryCalls(decryptedDir, selfUsername, topPeers = 20, recentLimit = 50) {
  const self = selfUsername ?? "";
  const sig = shardCatalogSig(decryptedDir, ["message"]) + "|" + self + "|" + fileSigOf(join23(decryptedDir, "contact", "contact.db"));
  const names = contactMeta(decryptedDir).names;
...
function tableColumns5(db, table) {
  const rows = db.prepare(`PRAGMA table_info(${table})`).all();
  return new Set(rows.map((r) => r.name));
}
function cleanWxid2(username) {
  const m = username.match(/^(wxid_[A-Za-z0-9]+)(?:_[A-Za-z0-9]+)?$/);
  return m ? m[1] ?? username : username;
}
function loadMessageCounts(decryptedDir, usernames) {
  ...
  const tableToUser = /* @__PURE__ */ new Map();
  for (const u of usernames) tableToUser.set("Msg_" + createHash6("md5").update(u, "utf8").digest("hex"), u);
  for (const sh of shardCatalog(decryptedDir)) {
    for (const t of sh.tables.keys()) {
      ...
      let db = null;
      try {
        db = new DatabaseSync18(sh.file, { readOnly: true });
      } catch {
        continue;
      }
      try {
...
function loadContactMeta(decryptedDir) {
  ...
  const p = join24(decryptedDir, "contact", "contact.db");
  if (!existsSync17(p)) return meta;
  try {
    const db = new DatabaseSync18(p, { readOnly: true });
...
function loadRoomData(decryptedDir) {
  ...
  try {
    ...
    try {
      ...
      for (const r of users) idToUser.set(r.id, cellString4(r.username));
    } catch {
    }
    let rooms = [];
    try {
      const crCols = tableColumns5(db, "chat_room");
      if (crCols.has("ext_buffer")) {
        rooms = db.prepare("SELECT id, username, ext_buffer AS ext FROM chat_room").all();
      } else if (crCols.has("id") && crCols.has("username")) {
        rooms = db.prepare("SELECT id, username FROM chat_room").all();
      }
    } catch {
    }
    for (const room of rooms) {
      const u = cellString4(room.username);
      if (!u) continue;
    ...
    try {
      ...
      for (const room of rooms) roomByUsername.set(room.id, cellString4(room.username));
      for (const r of rows) {
...
function queryGraph(decryptedDir, selfUsername) {
  const nodes = [];
  const sessionPath = join24(decryptedDir, "session", "session.db");
  const talkers = [];
  if (existsSync17(sessionPath)) {
    try {
      const db = new DatabaseSync18(sessionPath, { readOnly: true });
      const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='SessionTable'").get() !== void 0;
      if (has) {
        const rows = db.prepare("SELECT username FROM SessionTable").all();
        for (const r of rows) {
          const u = cellString4(r.username);
  ...
  for (const [u, info] of meta) {
    ...
    const groups2 = Array.from(roomData.memberGroups.get(u) ?? []).sort();
    const node = {
      id: u,
      label: names.get(u) || u,
      kind,
      msg_count: counts.get(u) ?? 0,
      group_count: groups2.length,
      group_codes: groups2,
  ...
  groups.sort(
    (a, b) => (b.shared_count ?? 0) - (a.shared_count ?? 0) || (b.msg_count ?? 0) - (a.msg_count ?? 0) || b.id.localeCompare(a.id)
  );
...
function msgTableName2(username) {
  return "Msg_" + createHash7("md5").update(username, "utf8").digest("hex");
}
function getDailyCounts(decryptedDir, username, year, month) {
  ...
  const counts = {};
  const msgDir = join25(decryptedDir, "message");
  if (!existsSync18(msgDir)) return { counts, year, month };
  const files = readdirSync11(msgDir).filter(
    (f) => f.endsWith(".db") && f.startsWith("message_") && !f.includes("fts") && !f.includes("resource") && !f.includes("media")
  ).sort();
  const table = msgTableName2(username);
  for (const f of files) {
    let db = null;
    try {
      db = new DatabaseSync19(join25(msgDir, f), { readOnly: true });
    } catch {
      continue;
    }
    const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(table) !== void 0;
...
function indexReady(decryptedDir) {
  ...
  if (!existsSync19(p)) return false;
  try {
...
function searchIndexBatch(decryptedDir, terms, limit = 400, opts) {
  ...
  try {
    ...
    const hits = rows.map((r) => {
      ...
      const username = decodeCell2(r["username"]);
...
function tryDecompress2(data) {
  if (data.length >= 4 && data.subarray(0, 4).equals(ZSTD_MAGIC4)) {
    try {
      return Buffer.from(decompress4(data));
    } catch {
      return null;
    }
  }
...
function searchIndexPath(decryptedDir) {
  return join26(dirname6(decryptedDir), "wechat_search.db");
}
function msgTableName3(username) {
  return "Msg_" + createHash8("md5").update(username, "utf8").digest("hex");
}
function decodeCell2(v) {
  if (v === null || v === void 0) return "";
  if (typeof v === "string") return v;
  if (typeof v === "number" || typeof v === "boolean" || typeof v === "bigint" || typeof v === "symbol") return String(v);
  const raw = Buffer.from(v instanceof Uint8Array ? v : []);
  const decompressed = tryDecompress2(raw);
  const bytes = decompressed ?? raw;
  try {
    return new TextDecoder("utf-8", { fatal: false }).decode(bytes);
  } catch {
...
function loadSessionUsernames2(decryptedDir) {
  const dbPath6 = join26(decryptedDir, "session", "session.db");
  if (!existsSync19(dbPath6)) return [];
  const out = [];
  try {
    const db = new DatabaseSync20(dbPath6, { readOnly: true });
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all().map((r) => r.name);
    ...
    if (table) {
      ...
      for (const r of rows) {
        const u = decodeCell2(r["username"]).trim();
        if (u) out.push(u);
      }
    }
    db.close();
...
function loadDisplayNames2(decryptedDir) {
  ...
  const sessionDb = join26(decryptedDir, "session", "session.db");
  if (existsSync19(sessionDb)) {
    try {
      const db = new DatabaseSync20(sessionDb, { readOnly: true });
      const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='SessionNoContactInfoTable'").get() !== void 0;
      if (has) {
        const rows = db.prepare("SELECT username, session_title FROM SessionNoContactInfoTable").all();
        for (const r of rows) {
          const u = decodeCell2(r["username"]);
          const t = decodeCell2(r["session_title"]).trim();
...
function withIndexWrite(decryptedDir, fn) {
...
function formatFullTime(ts2) {
  if (!ts2) return "";
  const d = new Date(ts2 * 1e3);
  const p = (n) => String(n).padStart(2, "0");
  return `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())} ${p(d.getHours())}:${p(d.getMinutes())}`;
}
var windowTableCache = /* @__PURE__ */ new Map();
function loadMessageWindow(decryptedDir, username, centerMs, spanMs, limit = 14) {
  ...
  for (const shard of messageShardFiles(decryptedDir)) {
    ...
    if (has === void 0) {
      let probe = null;
      try {
        probe = new DatabaseSync20(shard, { readOnly: true });
    ...
    try {
      ...
      const rows = sdb.prepare(
        'SELECT local_id, sort_seq, create_time, message_content FROM "' + table + '" WHERE local_type=1 AND sort_seq BETWEEN ? AND ? ORDER BY sort_seq LIMIT ?'
      ).all(lo, hi, limit);
      for (const r of rows) {
        const raw = decodeCell2(r["message_content"]).trim();
...
function searchWechatFts(decryptedDir, q, cap, names, scopeUsername) {
  const p = join26(decryptedDir, "message", "message_fts.db");
  if (!existsSync19(p)) return { hits: [] };
  try {
    const db = new DatabaseSync20(p, { readOnly: true });
    const sessions = /* @__PURE__ */ new Map();
    try {
      const rows = db.prepare("SELECT rowid AS id, username AS u FROM name2id").all();
      for (const r of rows) sessions.set(r.id, decodeCell2(r.u));
    } catch {
    }
    const out = [];
    const like2 = "%" + q.replace(/[%_]/g, " ") + "%";
    for (const [content, aux] of [
      ...
      try {
        const exists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(content) !== void 0;
        if (!exists) continue;
...
function searchIndexMessages(decryptedDir, query, limit, username) {
  ...
  let budget = 8e5;
  const shards = messageShardFiles(decryptedDir);
  const scopeUsernames = username ? [username] : loadSessionUsernames2(decryptedDir).slice(0, 800);
    ...
    for (const shard of shards) {
      ...
      try {
        ...
        for (const r of sdb.prepare(sql).iterate()) {
          ...
          const text = decodeCell2(r["message_content"]).replace(/\n/g, " ").trim();
...
function hitsFromRows(rows, roomName) {
  const out = [];
  for (const r of rows) {
    const username = cellText2(r["username"]).trim();
    if (!username) continue;
    const name = cellText2(r["remark"]).trim() || cellText2(r["nick_name"]).trim() || username;
    const hit = { username, name };
    const head = cellText2(r["small_head_url"]).trim() || cellText2(r["big_head_url"]).trim();
    if (head) hit.head = head;
    const room = cellText2(r["room"]).trim();
    if (room) hit.roomUsername = room;
    if (roomName) hit.roomName = roomName;
    const sig = cellText2(r["signature"]).trim();
    if (sig) hit.signature = sig;
    const region = cellText2(r["region"]).trim();
    if (region) hit.region = region;
    out.push(hit);
  }
  return out;
}
function contactFtsReady(db) {
...
function buildContactFts(db, decryptedDir) {
  const contactPath = join27(decryptedDir, "contact", "contact.db");
  if (!existsSync20(contactPath)) return "unavailable";
  let cdb = null;
  let rows;
  try {
    cdb = new DatabaseSync21(contactPath, { readOnly: true });
    ...
    console.warn("[members] contact.db \u8BFB\u4E0D\u5230\uFF0C\u672C\u6B21\u6210\u5458\u641C\u7D22\u56DE\u9000 LIKE\uFF1A" + e.message);
    return "unavailable";
  } finally {
    try {
      cdb?.close();
  ...
  try {
    ...
    try {
      ...
      for (const r of rows) {
        const username = cellText2(r["username"]).trim();
        if (!username) continue;
        ins.run(cellText2(r["remark"]).trim() || cellText2(r["nick_name"]).trim() || username, username, cellText2(r["remark"]).trim(), cellText2(r["alias"]).trim(), cellText2(r["quan_pin"]).trim());
      }
      db.exec("COMMIT");
    } catch (e) {
      try {
    ...
    console.warn("[members] contact_fts \u6784\u5EFA\u5931\u8D25\uFF0C\u672C\u6B21\u6210\u5458\u641C\u7D22\u56DE\u9000 LIKE\uFF1A" + e.message);
    return "unavailable";
  }
}
  ...
  if (existsSync20(p)) {
    ...
    try {
      ...
      if (state === "ready") {
        ...
        if (rows.length > 0) {
          const items = [];
          const cdbPath = join27(decryptedDir, "contact", "contact.db");
          let cdb = null;
          try {
            cdb = new DatabaseSync21(cdbPath, { readOnly: true });
          } catch {
            cdb = null;
          }
          ...
          try {
            for (const r of rows) {
              const username = cellText2(r["username"]).trim();
              if (!username) continue;
              const hit = { username, name: cellText2(r["name"]).trim() || username };
              if (headStmt) {
                try {
                  const hr = headStmt.get(username);
                  const head = cellText2(hr?.small_head_url ?? "").trim() || cellText2(hr?.big_head_url ?? "").trim();
                  if (head) hit.head = head;
                } catch {
                }
              }
      ...
      console.warn("[members] contact_fts \u67E5\u8BE2\u5931\u8D25\uFF0C\u672C\u6B21\u6210\u5458\u641C\u7D22\u56DE\u9000 LIKE\uFF1A" + e.message);
    } finally {
      try {
        db?.close();
      } catch {
...
function searchGlobalLike(decryptedDir, term, cap) {
  const p = join27(decryptedDir, "contact", "contact.db");
  if (!existsSync20(p)) return { items: [], total: 0, source: "like" };
  try {
    const db = new DatabaseSync21(p, { readOnly: true });
    try {
      ...
      const rows = db.prepare(sql).all(like2, like2, like2, like2, like2, cap);
...
function searchRoomMembers(decryptedDir, roomUsername, term, cap) {
  ...
  try {
    ...
    try {
      ...
      if (room?.id === void 0) return { items: [], total: 0, source: "like" };
      const display = db.prepare("SELECT nick_name, remark FROM contact WHERE username=?").get(roomUsername);
      const roomName = (display ? cellText2(display.remark).trim() || cellText2(display.nick_name).trim() : "") || roomUsername;
      const like2 = "%" + term + "%";
      ...
      const rows = db.prepare(sql).all(room.id, like2, like2, like2, like2, like2, cap);
...
// src/backend/wechat-data/src/query/media-image.ts
var import_llm_retry = __toESM(require_llm_retry(), 1);
import { createDecipheriv as createDecipheriv3 } from "node:crypto";
import { DatabaseSync as DatabaseSync22 } from "node:sqlite";
import { existsSync as existsSync21, mkdirSync as mkdirSync3, readdirSync as readdirSync12, readFileSync as readFileSync8, writeFileSync as writeFileSync3 } from "node:fs";
import { createHash as createHash9 } from "node:crypto";
import { join as join28 } from "node:path";

// src/backend/wechat-data/src/query/cdn-policy.ts
var CDN_DISABLED_MESSAGE = "\u5DF2\u5173\u95ED\u300C\u81EA\u52A8\u83B7\u53D6\u539F\u56FE\uFF08CDN\uFF09\u300D\uFF1A\u53EA\u4F7F\u7528\u672C\u673A\u7F13\u5B58\uFF0C\u672A\u53D1\u8D77\u7F51\u7EDC\u8BF7\u6C42\u3002\u53EF\u5728\u300C\u8BBE\u7F6E \u2192 \u56FE\u7247\u89E3\u7801\u300D\u4E2D\u91CD\u65B0\u5F00\u542F\u3002";
var LOCAL_DECRYPT_DISABLED_MESSAGE = "\u5F53\u524D\u4E3A\u300C\u670D\u52A1\u7AEF\u89E3\u5BC6\u300D\uFF0C\u4F46\u8FDC\u7AEF\u8FD4\u56DE\u7684\u662F\u52A0\u5BC6\u6D41\u3001\u65E0\u6CD5\u76F4\u63A5\u7528\uFF1B\u8BF7\u6539\u56DE\u300C\u672C\u5730\u89E3\u5BC6\u300D\u3002";
function cdnFetchAllowed(opts) {
  return opts?.cdnEnabled !== false;
}
function localDecryptEnabled(opts) {
  return opts?.localDecrypt !== false;
}

// src/backend/wechat-data/src/query/media-image.ts
var V2_MAGIC = [7, 8, 86, 50];
var V1_MAGIC_FULL = [7, 8, 86, 49, 8, 7];
var V2_MAGIC_FULL = [7, 8, 86, 50, 8, 7];
var V1_AES_KEY = new TextEncoder().encode("cfcd208495d565ef");
var IMAGE_MAGIC = [
  ["png", [137, 80, 78, 71]],
  ["gif", [71, 73, 70, 56]],
  ["tif", [73, 73, 42, 0]],
...
function msgTableName4(username) {
  return "Msg_" + createHash9("md5").update(username, "utf8").digest("hex");
}
...
function extractMd5FromPacked(value) {
  ...
  const buf = bytes;
  const marker = [18, 34, 10, 32];
  for (let i = 0; i + marker.length + 32 <= buf.length; i += 1) {
    if (marker.every((b, j) => buf[i + j] === b)) {
      const s2 = String.fromCharCode(...buf.slice(i + marker.length, i + marker.length + 32));
      if (/^[0-9a-f]{32}$/.test(s2)) return s2;
    }
  }
  for (let i = 0; i + 34 <= buf.length; i += 1) {
    if (buf[i] === 34 && buf[i + 1] === 32) {
      const s2 = String.fromCharCode(...buf.slice(i + 2, i + 34));
      if (/^[0-9a-f]{32}$/.test(s2)) return s2;
    }
  }
  let s = "";
...
function resolveImageResourceHint(decryptedDir, username, localId) {
  ...
  let dataIndex = "";
  const resDb = join28(decryptedDir, "message", "message_resource.db");
  if (existsSync21(resDb)) {
    try {
      const db = new DatabaseSync22(resDb, { readOnly: true });
...
function detectImageFormat(header) {
...
function detectXorKey(data) {
  ...
  if ((data[0] ?? 0) === V2_MAGIC[0] && (data[1] ?? 0) === V2_MAGIC[1] && (data[2] ?? 0) === V2_MAGIC[2] && (data[3] ?? 0) === V2_MAGIC[3]) return null;
...
function aes128EcbDecrypt(key, data) {
  ...
  const decipher = createDecipheriv3("aes-128-ecb", Buffer.from(key), null);
  decipher.setAutoPadding(false);
...
function decodeDatBytes(data, aesKey, xorKey) {
  ...
  if (decrypted.length >= 4 && decrypted[0] === 119 && decrypted[1] === 120 && decrypted[2] === 103 && decrypted[3] === 102) {
...
var RENDERABLE_EXTS = ["jpg", "jpeg", "png", "gif", "webp", "bmp", "tif"];
function decodeImageDataUrl(decryptedDir, decodedDir, username, localId, wechatBaseDir, aesKey, xorKey) {
  ...
  if (md5) {
    for (const ext of RENDERABLE_EXTS) {
      const p = join28(decodedDir, md5 + "." + ext);
      if (existsSync21(p)) {
        try {
          const bytes = readFileSync8(p);
  ...
  if (md5) {
    const userDir = join28(decodedDir, username);
    if (existsSync21(userDir)) {
      for (const ext of RENDERABLE_EXTS) {
        const p = join28(userDir, md5 + "." + ext);
        if (existsSync21(p)) {
          try {
            const bytes = readFileSync8(p);
            return { url: toDataUrl(ext === "jpeg" ? "jpg" : ext, new Uint8Array(bytes)), format: ext === "jpeg" ? "jpg" : ext };
      ...
      const hevc = join28(userDir, md5 + ".hevc");
      if (existsSync21(hevc)) return { error: "hevc-unsupported" };
    }
  }
  if (wechatBaseDir && md5) {
    const attach = join28(wechatBaseDir, "msg", "attach", msgTableName4(username).replace(/^Msg_/, ""));
...
function decodeFileImageDataUrl(decryptedDir, decodedDir, wechatBaseDir, md5, aesKey, xorKey = 255) {
  ...
  for (const ext of ["jpg"]) {
    const p = join28(decodedDir, m + "." + ext);
    if (existsSync21(p)) {
      try {
        const bytes = readFileSync8(p);
...
function decodeEmoticonDataUrl(decryptedDir, decodedDir, wechatBaseDir, md5, aesKey, xorKey = 255) {
  ...
  const attachRoot = join28(wechatBaseDir, "msg", "attach");
  if (!existsSync21(attachRoot)) return { error: "\u8868\u60C5\u672A\u7F13\u5B58\u4E14\u627E\u4E0D\u5230 msg/attach" };
  const dats = [];
  const walk = (dir, depth) => {
    if (depth > 4 || dats.length > 8) return;
    let entries2 = [];
    try {
      entries2 = readdirSync12(dir, { withFileTypes: true }).map((e) => ({ name: e.name, isDir: e.isDirectory() }));
    } catch {
      return;
    }
    for (const e of entries2) {
      if (dats.length > 8) return;
      const p = join28(dir, e.name);
      if (e.isDir) {
        walk(p, depth + 1);
  ...
  for (const f of ordered) {
    try {
      ...
      if (dec.format === "hevc") continue;
      try {
        writeFileSync3(join28(decodedDir, m + "." + dec.format), Buffer.from(dec.bytes));
      } catch {
      }
      return { url: toDataUrl(dec.format, dec.bytes), format: dec.format };
    } catch {
...
async function fetchEmoticonRemote(url, decodedDir, md5, opts = {}) {
  if (!cdnFetchAllowed(opts)) return { error: CDN_DISABLED_MESSAGE };
  if (!/^https?:\/\//i.test(url)) return { error: "\u8868\u60C5\u94FE\u63A5\u4E0D\u662F http(s)" };
  try {
    const res = await (0, import_llm_retry.fetchWithRetry)(fetch, url, { headers: { "User-Agent": "Mozilla/5.0" } }, { timeoutMs: 1e4 });
    ...
    try {
      mkdirSync3(decodedDir, { recursive: true });
      writeFileSync3(join28(decodedDir, md5 + "." + fmt), Buffer.from(bytes));
    } catch {
    }
    return { url: toDataUrl(fmt, bytes), format: fmt };
  } catch (e) {
    return { error: "\u8868\u60C5\u4E0B\u8F7D\u5931\u8D25: " + e.message };
  }
}
...
function writeDecodedCache(decodedDir, username, md5, format, bytes) {
  const dir = join28(decodedDir, username);
  mkdirSync3(dir, { recursive: true });
  writeFileSync3(join28(dir, md5 + "." + format), Buffer.from(bytes));
}
...
function readDir2id(db) {
  ...
  for (const r of rows) map.set(r.rowid, cellText3(r.username).trim());
  return map;
}
function imageCandidatePaths(baseDir, file, n1, n2) {
  ...
  for (const [a, b] of pairs) {
    push(join28(baseDir, "msg", "attach", a, b, "Img", file));
    push(join28(baseDir, "msg", "attach", a, b, file));
  }
  push(join28(baseDir, "msg", "attach", n1, "Img", file));
  push(join28(baseDir, "msg", "attach", n1, file));
  push(join28(baseDir, "msg", "attach", n2, file));
  return out;
}
var HARDLINK_ROWS_PER_MD5 = 8;
var HARDLINK_MD5_CHUNK = 400;
function firstExistingDatPath(wechatBaseDir, rows, dirs) {
  for (const r of rows) {
    const file = cellText3(r.file_name).trim();
...
function resolveImageFilePathsByMd5(decryptedDir, wechatBaseDir, md5s) {
  const out = /* @__PURE__ */ new Map();
  const wanted = [...new Set(md5s.map((m) => (m ?? "").trim().toLowerCase()).filter((m) => m.length === 32))];
  if (wanted.length === 0) return out;
  const dbPath6 = join28(decryptedDir, "hardlink", "hardlink.db");
  if (!existsSync21(dbPath6)) return out;
  let db = null;
  try {
    db = new DatabaseSync22(dbPath6, { readOnly: true });
  } catch {
    return out;
  }
  try {
    ...
    for (let i = 0; i < wanted.length; i += HARDLINK_MD5_CHUNK) {
      ...
      for (const r of rows) {
        const key = cellText3(r.m).trim().toLowerCase();
...
function resolveImageFilePath(decryptedDir, wechatBaseDir, md5, dataIndex) {
  ...
  const dbPath6 = join28(decryptedDir, "hardlink", "hardlink.db");
  if (!existsSync21(dbPath6)) return null;
  let db = null;
  try {
    db = new DatabaseSync22(dbPath6, { readOnly: true });
  } catch {
...
function snsMonthRoots(wechatBaseDir) {
  const cacheRoot = join29(wechatBaseDir, "cache");
  const out = [];
  if (!existsSync22(cacheRoot)) return out;
  let months = [];
  try {
    months = readdirSync13(cacheRoot);
  } catch {
    return out;
  }
...
function scanMonth(root, aesKey, xorKey) {
  const byMd5 = /* @__PURE__ */ new Map();
  const files = [];
  walkFiles(root, files, 0);
  for (const f of files) {
    try {
      const dec = decodeDatBytes(new Uint8Array(readFileSync9(f)), aesKey ?? null, xorKey);
      if ("error" in dec) continue;
...
function md5Of(bytes) {
  return createHash10("md5").update(Buffer.from(bytes)).digest("hex");
}
function walkFiles(dir, out, depth) {
  if (depth > 5 || !existsSync22(dir)) return;
  let entries2 = [];
  try {
    entries2 = readdirSync13(dir, { withFileTypes: true }).map((e) => ({ name: e.name, isDir: e.isDirectory() }));
  } catch {
    return;
  }
  for (const e of entries2) {
    const p = join29(dir, e.name);
...
function snsImgRoots(wechatBaseDir) {
  ...
  if (!existsSync22(cacheRoot)) return out;
  try {
    for (const e of readdirSync13(cacheRoot, { withFileTypes: true })) {
      if (!e.isDirectory()) continue;
      const root = join29(cacheRoot, e.name, "Sns", "Img");
      if (existsSync22(root)) out.push(root);
    }
  } catch {
  }
  return out;
...
function resolveSnsImageDataUrl(wechatBaseDir, aesKey, xorKey, md5, timelineId, mediaId) {
  ...
  const cacheKey = timelineId && mediaId ? createHash10("md5").update(timelineId + "_" + mediaId + "_2", "utf8").digest("hex") : key;
  ...
  if (timelineId && mediaId) {
    for (const suffix of ["_2", "_1", "_0", ""]) {
      const k = createHash10("md5").update(timelineId + "_" + mediaId + suffix, "utf8").digest("hex");
      const sub = k.slice(0, 2);
      const rest = k.slice(2);
      for (const root of snsImgRoots(wechatBaseDir)) {
        const p = join29(root, sub, rest);
        if (existsSync22(p)) {
  ...
  const attachRoot = join29(wechatBaseDir, "msg", "attach");
  if (existsSync22(attachRoot)) {
    const attachFiles = [];
...
var import_llm_retry2 = __toESM(require_llm_retry(), 1);
import { createHash as createHash11 } from "node:crypto";
import { existsSync as existsSync23, mkdirSync as mkdirSync4, readFileSync as readFileSync10, writeFileSync as writeFileSync4 } from "node:fs";
...
async function fetchText(url, timeoutMs) {
  const res = await (0, import_llm_retry2.fetchWithRetry)(fetch, url, { headers: FETCH_HEADERS }, { timeoutMs });
...
function coverFile(cacheDir, key) {
  ...
  const hash = createHash11("md5").update(Buffer.from(key, "utf8")).digest("hex");
  return join30(cacheDir, "article-covers", hash + ".img");
}
async function resolveArticleCoverDataUrl(contentUrl, cacheDir, opts = {}) {
  ...
  if (!cdnFetchAllowed(opts)) return { error: CDN_DISABLED_MESSAGE };
  try {
    const html = await fetchText(key, 2e4);
    ...
    const bytes = await fetchBytes(imgUrl, 2e4);
    ...
    if (file) {
      try {
        mkdirSync4(join30(cacheDir ?? "", "article-covers"), { recursive: true });
        writeFileSync4(file, Buffer.from(bytes));
      } catch {
      }
    }
    boundedSet(coverCache, key, data);
...
function monthDirOf(createTime) {
  if (!createTime || !Number.isFinite(createTime)) return "";
  const d = new Date(createTime * 1e3);
  if (Number.isNaN(d.getTime())) return "";
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
}
function resolveMessageFileDataUrl(wechatBaseDir, fileName, opts = {}) {
  ...
  const fileRoot = join31(wechatBaseDir, "msg", "file");
  if (!existsSync24(fileRoot)) {
  ...
  try {
    ...
    const allDirs = readdirSync14(fileRoot, { withFileTypes: true }).filter((e) => e.isDirectory()).map((e) => join31(fileRoot, e.name));
    ...
    const bytes = readFileSync11(best.path);
...
function walkDb(dir, depth, out) {
  if (depth > 4 || !existsSync25(dir)) return;
  for (const e of readdirSync15(dir, { withFileTypes: true })) {
    const p = join32(dir, e.name);
    if (e.isDirectory()) {
...
function queryOverviewExtras(decryptedDir) {
  ...
  const sig = [
    ...
    fileSigOf(join32(dec, "message", "message_resource.db")),
    // loader 真正读了却没进签名的三处（原先靠「事件后整表清空」兜住，M8 去掉那层兜底后补齐）：
    // contact.db、session.db，以及整树 `walkDb` —— 后者没法用文件签名表达，用数据世代签名。
    fileSigOf(join32(dec, "contact", "contact.db")),
    fileSigOf(join32(dec, "session", "session.db")),
    dataGenerationSig()
  ].join("|");
...
function computeOverviewExtras(dec) {
  ...
  for (const u of names.keys()) md5ToUser.set(createHash12("md5").update(u, "utf8").digest("hex"), u);
  try {
    const sp = join32(dec, "session", "session.db");
    if (existsSync25(sp)) {
      const db = new DatabaseSync23(sp, { readOnly: true });
      if (db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='SessionTable'").get() !== void 0) {
        for (const r of db.prepare("SELECT username FROM SessionTable").all()) {
          const u = cellStr6(r["username"]);
          if (u) md5ToUser.set(createHash12("md5").update(u, "utf8").digest("hex"), u);
        }
      }
      db.close();
    }
  ...
  for (const sh of shardCatalogDirs(dec, ["message", "bizchat"])) {
    try {
      const db = new DatabaseSync23(sh.file, { readOnly: true });
      try {
  ...
  const rp = join32(dec, "message", "message_resource.db");
  if (existsSync25(rp)) {
    try {
      const db = new DatabaseSync23(rp, { readOnly: true });
      if (db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='MessageResourceDetail'").get() !== void 0) {
        ...
        if (cols.has("create_time")) {
          const r = db.prepare("SELECT COALESCE(SUM(size), 0) AS s FROM MessageResourceDetail WHERE create_time >= ?").get(day30);
  ...
  return {
    ...
    freshness: {
      ...
      lastSync: lastSync ? new Date(lastSync * 1e3).toLocaleString("zh-CN") : ""
    }
  };
}
...
function walkDbFiles(dir, out, depth) {
  if (depth > 4 || !existsSync26(dir)) return;
  for (const e of readdirSync16(dir, { withFileTypes: true })) {
    const p = join33(dir, e.name);
    if (e.isDirectory()) {
...
function queryOverviewInsights(decryptedDir, selfUsername) {
  const dec = decryptedDir;
  const self = (selfUsername ?? "").trim();
  const sns = existsSync26(join33(dec, "sns", "db_sns", "sns.db")) ? join33(dec, "sns", "db_sns", "sns.db") : join33(dec, "sns", "sns.db");
  const sig = [
    shardCatalogSig(dec, ["message", "bizchat"]),
    fileSigOf(join33(dec, "contact", "contact.db")),
    fileSigOf(join33(dec, "session", "session.db")),
    fileSigOf(sns),
    fileSigOf(join33(dec, "favorite", "favorite.db")),
    fileSigOf(join33(dec, "emoticon", "emoticon.db")),
    fileSigOf(join33(dec, "hardlink", "hardlink.db")),
    fileSigOf(join33(dec, "message", "message_resource.db"))
  ].join("|");
...
function computeOverviewInsights(decryptedDir, self = "") {
  ...
  const names = contactMeta(dec).names;
  const md5ToUser = /* @__PURE__ */ new Map();
  for (const u of names.keys()) md5ToUser.set(createHash13("md5").update(u, "utf8").digest("hex"), u);
  try {
    const sp = join33(dec, "session", "session.db");
    if (existsSync26(sp)) {
      const db = new DatabaseSync24(sp, { readOnly: true });
      const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='SessionTable'").get() !== void 0;
      if (has) {
        const rows = db.prepare("SELECT username FROM SessionTable").all();
        for (const r of rows) {
          const u = cellStr7(r["username"]);
          if (u) md5ToUser.set(createHash13("md5").update(u, "utf8").digest("hex"), u);
        }
      }
      db.close();
    }
  ...
  for (const sh of shardCatalogDirs(dec, ["message", "bizchat"])) {
    try {
      const db = new DatabaseSync24(sh.file, { readOnly: true });
      try {
        let selfRowId;
        if (self) {
  ...
  const contactRows = [];
  const cp = join33(dec, "contact", "contact.db");
  if (existsSync26(cp)) {
    try {
      const db = new DatabaseSync24(cp, { readOnly: true });
      const cols = new Set(db.prepare("PRAGMA table_info(contact)").all().map((r) => r.name));
      if (cols.has("username")) {
  ...
  const sp2 = existsSync26(join33(dec, "sns", "db_sns", "sns.db")) ? join33(dec, "sns", "db_sns", "sns.db") : join33(dec, "sns", "sns.db");
  if (existsSync26(sp2)) {
    try {
      const db = new DatabaseSync24(sp2, { readOnly: true });
      const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='SnsTimeLine'").get() !== void 0;
      if (has) {
        for (const r of db.prepare("SELECT content AS c FROM SnsTimeLine").iterate()) {
          const xml = cellStr7(r.c);
          moments.total += 1;
  ...
  assets.favorites = countRows2(join33(dec, "favorite", "favorite.db"), "fav_db_item");
  assets.emoticons = countRows2(join33(dec, "emoticon", "emoticon.db"), "kNonStoreEmoticonTable");
  for (const table of ["image_hardlink_info_v4", "file_hardlink_info_v4", "video_hardlink_info_v4"]) {
    const hp = join33(dec, "hardlink", "hardlink.db");
    if (!existsSync26(hp)) break;
    try {
      const db = new DatabaseSync24(hp, { readOnly: true });
  ...
  const rp = join33(dec, "message", "message_resource.db");
  if (existsSync26(rp)) {
    try {
      const db = new DatabaseSync24(rp, { readOnly: true });
      if (db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='MessageResourceDetail'").get() !== void 0) {
  ...
  return {
    ...
    time: {
      ...
      lastActive: lastActive ? new Date(lastActive * 1e3).toLocaleString("zh-CN") : ""
    },
    relations: {
...
function countRows2(path, table) {
  if (!existsSync26(path)) return 0;
  try {
    const db = new DatabaseSync24(path, { readOnly: true });
    const has = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='${table}'`).get() !== void 0;
    if (!has) {
      db.close();
      return 0;
...
function hasDbFile(dir, depth = 0) {
  if (depth > 5 || !existsSync27(dir)) return false;
  let entries2 = [];
  try {
    entries2 = readdirSync17(dir, { withFileTypes: true }).map((e) => ({ name: e.name, isDir: e.isDirectory() }));
  } catch {
    return false;
  }
  for (const e of entries2) {
    const p = join34(dir, e.name);
    if (e.isDir) {
...
function computeDbStatus(decryptedDir) {
  const lines = [];
  if (!existsSync27(decryptedDir)) {
    lines.push("\u26A0\uFE0F \u89E3\u5BC6\u76EE\u5F55\u4E0D\u5B58\u5728");
    return { lines, path: decryptedDir };
  }
  let dirs = [];
  try {
    dirs = readdirSync17(decryptedDir, { withFileTypes: true }).filter((e) => e.isDirectory()).map((e) => ({ name: e.name })).sort((a, b) => a.name.localeCompare(b.name));
  } catch (e) {
  ...
  for (const d of dirs) {
    ...
    const ok = hasDbFile(join34(decryptedDir, d.name));
...
function msgTableName5(username) {
  return "Msg_" + createHash14("md5").update(username, "utf8").digest("hex");
}
function messageShardFiles2(decryptedDir) {
  const dir = join35(decryptedDir, "message");
  if (!existsSync28(dir)) return [];
  return readdirSync18(dir).filter((f) => f.endsWith(".db") && !f.includes("_shm") && !f.includes("_wal") && !f.includes("media") && !f.includes("fts") && !f.includes("resource")).sort().map((f) => join35(dir, f));
}
function messageServerId(decryptedDir, username, localId) {
  const table = msgTableName5(username);
  for (const shard of messageShardFiles2(decryptedDir)) {
    let db = null;
    try {
      db = new DatabaseSync25(shard, { readOnly: true });
    } catch {
      continue;
    }
    const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(table) !== void 0;
...
function resolveVoiceInfo(decryptedDir, username, localId) {
  const mediaDb = join35(decryptedDir, "message", "media_0.db");
  if (!existsSync28(mediaDb)) {
    return { available: false, decodable: false, error: "\u8BED\u97F3\u5E93\u4E0D\u5B58\u5728 (media_0.db)" };
  }
  try {
    const db = new DatabaseSync25(mediaDb, { readOnly: true });
    const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='VoiceInfo'").get() !== void 0;
    if (!has) {
      db.close();
    ...
    db.close();
    if (!row) return { available: false, decodable: false, error: "\u672A\u627E\u5230\u8BED\u97F3\u6570\u636E" };
...
import { existsSync as existsSync29, readdirSync as readdirSync19, readFileSync as readFileSync12 } from "node:fs";
import { createHash as createHash15 } from "node:crypto";
import { join as join36 } from "node:path";
function msgTableName6(username) {
  return "Msg_" + createHash15("md5").update(username, "utf8").digest("hex");
}
function messageShardFiles3(decryptedDir) {
  const dir = join36(decryptedDir, "message");
  if (!existsSync29(dir)) return [];
  return readdirSync19(dir).filter((f) => f.endsWith(".db") && !f.includes("_shm") && !f.includes("_wal") && !f.includes("monitor_cache")).sort().map((f) => join36(dir, f));
}
function resolveVideoInfo(decryptedDir, decodedDir, username, localId, wechatBaseDir) {
  ...
  for (const ext of ["jpg", "jpeg", "png", "webp"]) {
    const p = join36(decodedDir, md5 + "." + ext);
    if (!existsSync29(p)) continue;
    try {
      return { available: true, md5, coverUrl: toJpegDataUrl(readFileSync12(p), ext), ...found.video ? { videoPath: found.video } : {} };
    } catch {
    }
  }
  const userDir = join36(decodedDir, username);
  if (existsSync29(userDir)) {
    for (const ext of ["jpg", "jpeg", "png", "webp"]) {
      const p = join36(userDir, md5 + "." + ext);
      if (!existsSync29(p)) continue;
      try {
        return { available: true, md5, coverUrl: toJpegDataUrl(readFileSync12(p), ext), ...found.video ? { videoPath: found.video } : {} };
      } catch {
    ...
    if (existsSync29(join36(userDir, md5 + ".hevc"))) {
  ...
  if (found.thumb) {
    try {
      return { available: true, md5, coverUrl: toJpegDataUrl(readFileSync12(found.thumb), "jpg"), ...found.video ? { videoPath: found.video } : {} };
    } catch {
    }
  }
...
function resolveVideoMd5(decryptedDir, username, localId) {
  const table = msgTableName6(username);
  for (const shard of messageShardFiles3(decryptedDir)) {
    let db = null;
    try {
      db = new DatabaseSync26(shard, { readOnly: true });
    } catch {
      continue;
    }
    try {
...
function findVideoArtifacts(baseDir, md5) {
  if (!baseDir) return {};
  const root = join36(baseDir, "msg", "video");
  if (!existsSync29(root)) return {};
  let thumb;
  let video;
  for (const month of readdirSync19(root)) {
    const dir = join36(root, month);
    if (!thumb) {
      const t = join36(dir, md5 + "_thumb.jpg");
      if (existsSync29(t)) thumb = t;
    }
    if (!video) {
      const v = join36(dir, md5 + ".mp4");
      if (existsSync29(v)) video = v;
    }
    if (thumb && video) break;
  }
  return { thumb, ...video ? { video } : {} };
}
function toJpegDataUrl(bytes, ext) {
...
import { existsSync as existsSync30, readFileSync as readFileSync13 } from "node:fs";
import { join as join37 } from "node:path";
function sniffImageFormat2(data) {
  if (data.length >= 3 && data[0] === 255 && data[1] === 216 && data[2] === 255) return "jpeg";
  if (data.length >= 4 && data[0] === 137 && data[1] === 80 && data[2] === 78 && data[3] === 71) return "png";
  if (data.length >= 4 && data[0] === 71 && data[1] === 73 && data[2] === 70 && data[3] === 56) return "gif";
  if (data.length >= 12 && data[0] === 82 && data[1] === 73 && data[2] === 70 && data[3] === 70 && data[8] === 87 && data[9] === 69 && data[10] === 66 && data[11] === 80) return "webp";
...
function avatarFromHeadImageDb(decryptedDir, username) {
  ...
  if (!existsSync30(dbPath6)) return null;
  try {
...
function avatarFromTempHeadFile(wechatBaseDir, url) {
  ...
  const hash = createHash16("md5").update(Buffer.from(url, "utf8")).digest("hex");
  const f = join37(wechatBaseDir, "temp", "head_image", hash);
  if (!existsSync30(f)) return null;
  try {
    const buf = readFileSync13(f);
    if (buf.length < 16) return null;
    const fmt = sniffImageFormat2(new Uint8Array(buf));
...
function contactByNickname(decryptedDir, nickname) {
  ...
  try {
    ...
    const sql = `SELECT username, COALESCE(NULLIF(${small}, ''), ${big}) AS u, ${md} AS m FROM contact WHERE nick_name = ? AND COALESCE(NULLIF(${small}, ''), ${big}) != '' LIMIT 1`;
    const row = db.prepare(sql).get(nickname);
    db.close();
    if (!row) return null;
    const username = cellStr8(row.username);
    const url = cellStr8(row.u);
    return username && url ? { username, url, md5: cellStr8(row.m) } : null;
  } catch {
    return null;
  }
}
...
var PAGE_GUARD = 256;
var MAX_USER_ADDRESS = 140737488355327;
var MBI_SIZE = 48;
var apiPromise;
...
function findKeyAddresses(chunk) {
  ...
  for (let i = 0; i + 32 <= chunk.length; i += 1) {
    ...
    if (addr > 0 && addr < 140737488355327) out.push(addr);
  }
  return out;
}
...
async function recoverDbKeyV4(pid, dbFilePath, internalDbKey) {
  let page1;
  try {
    page1 = readFileSync15(dbFilePath);
  } catch (e) {
  ...
  for (const candidate of filtered) {
    const testKey = internalDbKey && internalDbKey.length === KEY_SIZE ? Buffer.from(candidate.map((b, i) => b ^ (internalDbKey[i] ?? 0))) : candidate;
...
var MAX_CODE = 4294967295;
var MAX_PREFERRED_DIRS = 2e3;
var SKIPPED_FALLBACK_DIR_PARTS = ["thumb", "emoticon"];
var MONTH_DIR_RE = /^\d{4}-\d{2}$/;
function cleanWxid3(value) {
...
function deriveImageKeys(code, wxid) {
  ...
  const cleanedWxid = cleanWxid3(wxid);
  if (cleanedWxid === "") throw new Error("wxid must not be empty");
  const digest = createHash17("md5").update(String(code) + cleanedWxid, "utf8").digest("hex");
  return { xorKey: code & 255, aesKey: digest.slice(0, 16) };
}
function decryptAesBlock(aesKey, ciphertext) {
  ...
  try {
    const decipher = createDecipheriv4("aes-128-ecb", keyBytes.subarray(0, AES_BLOCK_SIZE), null);
    decipher.setAutoPadding(false);
...
function detectImageFormat2(plaintext) {
  if (plaintext === null || plaintext.length === 0) return null;
  if (plaintext.subarray(0, 3).equals(Buffer.from([255, 216, 255]))) return "jpeg";
  if (plaintext.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]))) return "png";
...
function verifyAesKey(aesKey, ciphertext) {
  return detectImageFormat2(decryptAesBlock(aesKey, ciphertext)) !== null;
}
...
function listChildDirs(dir) {
  ...
  try {
    for (const entry of readdirSync20(dir, { withFileTypes: true })) {
      if (entry.isDirectory()) out.push(join38(dir, entry.name));
    }
  } catch {
    return [];
  }
...
function offerRecentTemplates(dir, capacity) {
  ...
  try {
    for (const entry of readdirSync20(dir, { withFileTypes: true })) {
      ...
      const tpl = readV2Template(join38(dir, entry.name));
...
function trustedXorForVerifiedAesKey(aesKey, templateData) {
  ...
  if (detectImageFormat2(decryptAesBlock(aesKey, current.ciphertext)) === null) return null;
  const matching = [];
  for (const template of templates) {
    if (detectImageFormat2(decryptAesBlock(aesKey, template.ciphertext)) === "jpeg" && template.tailXorKey !== null) {
...
var KVCOMM_FILE_RE = /^(?:key_)?(\d+)_.+\.statistic$/i;
function kvcommCodesFromDir(kvcommDir) {
  const codes = [];
  try {
    for (const entry of readdirSync20(kvcommDir, { withFileTypes: true })) {
...
function resolveLocalImageKey(opts) {
  ...
  if (currentXorKey !== null) {
    orderedCodes.sort((a, b) => ((a & 255) !== currentXorKey ? 1 : 0) - ((b & 255) !== currentXorKey ? 1 : 0));
  }
...
// src/backend/wechat-data/src/keys/image-key-memory-scan.ts
var MEMORY_CHUNK_SIZE = 4 * 1024 * 1024;
var MEMORY_CHUNK_OVERLAP = 68;
  ...
  while (i + 1 < data.length) {
    ...
    if (isAsciiAlphaNum && data[i + 1] === 0) {
      ...
      while (end + 1 < data.length) {
        ...
        if (isAlphaNum && data[end + 1] === 0) end += 2;
        else break;
      }
      if (end - i === 64) {
  ...
  try {
    for (const region of regions) {
      ...
      while (offset < region.size) {
        ...
        if (fullRead) trailing = data.subarray(-MEMORY_CHUNK_OVERLAP);
...
import { existsSync as existsSync32, mkdirSync as mkdirSync6, readFileSync as readFileSync17, renameSync as renameSync2, writeFileSync as writeFileSync5 } from "node:fs";
...
var SQLITE_FILE_HEADER = "SQLite format 3\0";
...
function isSqliteDatabase(path) {
  ...
  try {
    ...
    fd = openSync3(path, "r");
    return readSync3(fd, header, 0, 16, 0) === 16 && header.toString("latin1") === SQLITE_FILE_HEADER;
  } catch {
    return false;
  } finally {
    if (fd !== void 0) {
...
function copyTree(src, dest, walCarried) {
  ...
  for (const entry of readdirSync21(src, { withFileTypes: true })) {
    if (isRuntimeArtifact(entry.name)) continue;
    copyTree(join39(src, entry.name), join39(dest, entry.name), walCarried);
  }
}
function resolveWechatDataRoot(env = process.env) {
  ...
  return resolve(join39(resolveDshHome(void 0, env), "wechat-data"));
}
function resolveDecryptedDir(env = process.env) {
  ...
  return join39(resolveWechatDataRoot(env), "decrypted");
}
function resolveDecodedDir(env = process.env) {
  const pinned = env[DECODED_DIR_ENV];
  if (pinned !== void 0 && pinned.trim().length > 0) return resolve(pinned.trim());
  return join39(resolveWechatDataRoot(env), "decoded_images");
}
...
function bootstrapWechatData(env = process.env) {
  ...
  if (env[DECRYPTED_DIR_ENV] !== void 0 && env[DECRYPTED_DIR_ENV].trim().length > 0) {
    return { root, copied: [], skipped: true, walCarried: [] };
  }
  const decrypted = join39(root, "decrypted");
  if (existsSync31(decrypted)) return { root, copied: [], skipped: true, walCarried: [] };
  const source = resolveSourceDir(env);
  if (source === null || !existsSync31(source)) {
    return { root, copied: [], skipped: true, walCarried: [] };
  }
  const copied = [];
  const walCarried = [];
  for (const item of BOOTSTRAP_ITEMS) {
    const src = join39(source, item);
    const dest = join39(root, item);
...
function atomicWriteJson(file, payload) {
  mkdirSync6(dirname7(file), { recursive: true });
  const tmp = file + ".tmp";
...
// src/backend/wechat-data/src/keys/service.ts
var WECHAT_EXECUTABLE_NAMES = ["weixin.exe", "wechat.exe"];
function defaultWeixinDllCandidates() {
  ...
  if (userProfile) out.push(join41(userProfile, "AppData", "Roaming", "Tencent", "Weixin", "Weixin.dll"));
  return out;
}
function findWechatPid() {
  try {
    const out = execFileSync2("tasklist", ["/FO", "CSV", "/NH"], { encoding: "utf8", windowsHide: true });
...
function scanDllInternalKey(wechatInstallDir) {
  const candidates = [];
  if (wechatInstallDir) {
    candidates.push(join41(wechatInstallDir, "Weixin.dll"));
    try {
      for (const entry of readdirSync22(wechatInstallDir, { withFileTypes: true })) {
        if (entry.isDirectory() && /^\d+\.\d+\.\d+/.test(entry.name)) candidates.push(join41(wechatInstallDir, entry.name, "Weixin.dll"));
      }
    } catch {
    }
  }
  const base = process.env.DSH_WECHAT_BASE_DIR;
  if (base) candidates.push(join41(base, "Weixin.dll"));
...
function pickProbeDb(decrypted, explicit) {
  if (explicit && existsSync33(explicit)) return explicit;
  const names = ["msg0.db", "msg.db", "micromsg.db", "favorite.db", "mediamsg0.db", "msg0.db"];
  for (const name of names) {
    const p = join41(decrypted, name);
    if (existsSync33(p)) return p;
  }
  const msgDir = join41(decrypted, "message");
  if (existsSync33(msgDir)) {
    for (const f of readdirSync22(msgDir)) {
      if (f.endsWith(".db") && !f.includes("-wal") && !f.includes("-shm")) return join41(msgDir, f);
    }
  }
  return null;
}
async function fetchDbKey(opts = {}) {
  ...
  if (result.ok && result.key) {
    upsertAccountKeysInStore("default", {
      db_key: result.key,
      db_key_source_db_storage_path: dirname8(probe)
    });
  }
  return result;
}
function normalizeAccountDir(accountDir) {
  ...
  if (!dir) return "";
  return (dir.split(/[\\/]/).pop() ?? "").toLowerCase() === "db_storage" ? dirname8(dir) : dir;
}
function kvcommCacheDir() {
  const appData = process.env.APPDATA;
  return appData ? join41(appData, "Tencent", "xwechat", "net", "kvcomm") : "";
}
...
// src/backend/wechat-data/src/query/decrypt-all.ts
...
import { dirname as dirname9, join as join42, relative as relative2 } from "node:path";
...
function readPrefix2(file, n) {
  ...
  try {
    while (filled < n) {
      const r = readSync4(fd, buf, filled, n - filled, null);
      if (r === 0) break;
      filled += r;
    }
  } finally {
...
function rawKeyHexFor(decryptedDir, rel, fallbackHex) {
  try {
    const keysPath = join42(decryptedDir, "..", "all_keys.json");
    if (existsSync34(keysPath)) {
...
function deriveEncKey2(rawKey, salt, keyFormat) {
  if (keyFormat === "wx_key_v4.1") return pbkdf2Sync3(rawKey, salt, PBKDF2_ITERS3, 32, "sha512");
  return rawKey;
}
async function decryptAllDbs(rawDbDir, decryptedDir, onProgress) {
  if (!rawDbDir || !existsSync34(rawDbDir)) {
    return {
      ok: false,
      total: 0,
      okCount: 0,
      failed: [],
      error: rawDbDir ? `\u6570\u636E\u5E93\u76EE\u5F55\u4E0D\u5B58\u5728\uFF08${rawDbDir}\uFF09\u3002\u8BF7\u786E\u8BA4\u300C\u6570\u636E\u914D\u7F6E\u300D\u4E2D\u7684\u6570\u636E\u5E93\u76EE\u5F55\u6307\u5411\u771F\u5B9E\u7684 db_storage\uFF0C\u6216\u4F7F\u7528\u300C\u68C0\u6D4B\u8D26\u53F7\u300D\u81EA\u52A8\u9009\u62E9\u3002` : "\u6570\u636E\u5E93\u76EE\u5F55\u672A\u914D\u7F6E\u3002\u8BF7\u5148\u5728\u300C\u6570\u636E\u914D\u7F6E\u300D\u4E2D\u68C0\u6D4B\u8D26\u53F7\u6216\u586B\u5199\u6570\u636E\u5E93\u76EE\u5F55\uFF08\u2026\\xwechat_files\\<wxid>\\db_storage\uFF09\u3002"
    };
  }
  const cfg = getConfig(decryptedDir);
  ...
  for (const db of dbs) {
    ...
    const target = join42(decryptedDir, rel);
    ...
    try {
      ...
      const encKey = deriveEncKey2(Buffer.from(keyHex, "hex"), salt, keyFormat);
...
import { existsSync as existsSync35, readdirSync as readdirSync23 } from "node:fs";
...
function walkDataDats(dir, out, depth) {
  if (depth > 10 || !existsSync35(dir)) return;
  let entries2;
  try {
...
async function decryptAllImageDats(rawRoot, decodedDir, aesKey, xorKey, concurrency = 8, onProgress) {
  const files = [];
  walkDataDats(join43(rawRoot, "msg", "attach"), files, 0);
    ...
    for (; ; ) {
      ...
      try {
        for (const ext2 of WRITE_EXTS) {
          if (existsSync35(join43(decodedDir, md5 + "." + ext2))) {
            skipped += 1;
        ...
        if ("error" in dec) {
          failed += 1;
          errors.push({ file: name, error: dec.error });
          continue;
        }
        if (dec.format === "hevc") {
          skipped += 1;
        ...
        await fs.writeFile(join43(decodedDir, md5 + "." + ext), Buffer.from(dec.bytes));
        okCount += 1;
      } catch (e) {
        failed += 1;
...
// src/backend/wechat-data/src/query/whisper.ts
...
function unpackedAware(p) {
  ...
  return join44(p.slice(0, i), `${ARCHIVE}.unpacked`, rest);
}
function onDiskPath(candidate, exists = existsSync36) {
...
var WHISPER_MODELS = [
  ...
  { id: "base", name: "Base", sizeLabel: "\u7EA6 145 MB \xB7 \u5F88\u5FEB" },
  { id: "small", name: "Small", sizeLabel: "\u7EA6 466 MB \xB7 \u8F83\u5FEB" },
  { id: "medium", name: "Medium", sizeLabel: "\u7EA6 1.5 GB \xB7 \u4E2D\u7B49" },
  { id: "large-v3", name: "Large v3", sizeLabel: "\u7EA6 3.1 GB \xB7 \u8F83\u6162" },
  { id: "turbo", name: "Turbo", sizeLabel: "\u7EA6 1.6 GB \xB7 \u5FEB" }
];
var WHISPER_DOWNLOAD_FILES = [
  ["tiny", "ggml-tiny.bin"],
  ["base", "ggml-base.bin"],
...
var reachableBase = null;
var DOWNLOAD_CONNECT_TIMEOUT_MS = 2e4;
var ENGINE_CONNECT_TIMEOUT_MS = 3e4;
var DOWNLOAD_MAX_ATTEMPTS = 2;
async function streamUrlToFile(url, dest, timeoutMs, onProgress) {
  ...
  const res = await (0, import_llm_retry3.fetchWithRetry)(
    fetch,
    url,
    { redirect: "follow", headers: have > 0 ? { range: `bytes=${have}-` } : void 0 },
...
function moveItem(src, dest) {
  try {
    ...
    mkdirSync8(dirname10(dest), { recursive: true });
    try {
  ...
  try {
    for (const entry of readdirSync24(fromDir, { withFileTypes: true })) {
      ...
      if (/^whisper(?:-cli)?(?:\.exe)?$/i.test(entry.name) || /^(?:ggml-.+|llama)\.dll$/i.test(entry.name)) {
        moved += moveItem(join45(fromDir, entry.name), join45(toDir, entry.name));
      }
    }
  } catch {
  }
...
function migrateWhisperModels(fromDir, toDir) {
  ...
  if (!existsSync37(fromDir)) return { ok: true, moved: 0 };
  try {
    mkdirSync8(toDir, { recursive: true });
    let moved = 0;
    for (const entry of readdirSync24(fromDir, { withFileTypes: true })) {
      const src = join45(fromDir, entry.name);
      if (entry.isDirectory()) {
        if (entry.name === "bin") {
          moved += moveItem(src, join45(toDir, "bin"));
        }
        continue;
      }
      if (/^ggml-.+\.bin$/i.test(entry.name)) {
        moved += moveItem(src, join45(toDir, entry.name));
      }
    ...
    try {
      rmSync3(join45(fromDir, ".engine-staging"), { recursive: true, force: true });
    } catch {
    }
...
function whisperEnginePath(configBin, modelsDir) {
  ...
  const dirSig2 = modelsDir ? fileSigOf(modelsDir) : "";
  return cachedBySig(
...
function resolveWhisperEngine(configBin, modelsDir) {
  ...
  if (modelsDir) {
    candidates.push(join45(modelsDir, "bin"), join45(modelsDir, "whisper-cli.exe"));
  }
  for (const candidate of candidates) {
  ...
  for (const name of ["whisper-cli", "whisper"]) {
    try {
      ...
      if (found && existsSync37(found)) return found;
    } catch {
    }
  }
  return "";
...
function resolveCommand(cmd) {
  ...
  const out = execFileSync3(locator, [cmd], { encoding: "utf8", windowsHide: true });
...
function whisperHasCuda() {
  return cachedBySig("whisper-cuda", "static", () => {
    try {
      execFileSync3("nvidia-smi", ["-L"], { encoding: "utf8", windowsHide: true });
      return true;
    } catch {
      return false;
    }
...
function bundledWhisperAssetsDir() {
  const here = fileURLToPath2(new URL(".", import.meta.url));
  return unpackedAware(resolve2(here, "..", "..", "..", "..", "wechat", "whisper"));
}
var seededWhisperDirs = /* @__PURE__ */ new Set();
...
function seedBundledWhisper(modelsDir) {
  ...
  for (const name of items) {
    ...
    if (!isDir) {
      mirrorWhisperItem(src, dst);
      continue;
    }
    try {
      mkdirSync8(dst, { recursive: true });
    } catch {
      continue;
    }
    let inner = [];
    try {
      inner = readdirSync24(src);
    } catch {
      continue;
    }
    for (const child of inner) mirrorWhisperItem(join45(src, child), join45(dst, child));
  }
}
function whisperDirUsable(dir) {
  if (existsSync37(join45(dir, "bin", "whisper-cli.exe"))) return true;
  try {
...
function defaultWhisperModelsDir(decryptedDir) {
  const bundled = bundledWhisperAssetsDir();
  const root = decryptedDir ? dirname10(decryptedDir) : resolveWechatDataRoot();
  if (!root) return bundled;
  const dir = join45(root, "whisper");
...
function resolveWhisperModelsDir(configured, decryptedDir) {
  const pinned = typeof configured === "string" ? configured.trim() : "";
  if (pinned) {
    const bundled = bundledWhisperAssetsDir();
    if (!pinned.includes("app.asar") && resolve2(pinned) !== resolve2(bundled)) return pinned;
  }
  return defaultWhisperModelsDir(decryptedDir);
}
async function installWhisperEngine(modelsDir, onProgress) {
  const binDir = join45(modelsDir, "bin");
  const target = join45(binDir, "whisper-cli.exe");
  if (existsSync37(target)) return { ok: true, path: target };
  ...
  for (const url of urls) {
    const zipPath = join45(modelsDir, "whisper-bin-x64.zip");
    try {
      const res = await (0, import_llm_retry3.fetchWithRetry)(fetch, url, { redirect: "follow" }, {
      ...
      const releaseDir = dirname10(found);
      mkdirSync8(binDir, { recursive: true });
      for (const name of readdirSync24(releaseDir)) {
        const src = join45(releaseDir, name);
        const dest = join45(binDir, name);
        if (existsSync37(dest)) continue;
        try {
          renameSync4(src, dest);
        } catch {
        }
      ...
      try {
        rmSync3(extractedBase, { recursive: true, force: true });
      } catch {
      }
      try {
        unlinkSync2(zipPath);
      ...
      return existsSync37(target) ? { ok: true, path: target } : { ok: false, error: "whisper-cli.exe \u5B89\u88C5\u5931\u8D25\uFF08\u62F7\u8D1D/\u79FB\u52A8\u672A\u5B8C\u6210\uFF09" };
    } catch (e) {
...
async function extractZip(zipPath, destDir) {
  const staging = join45(destDir, ".engine-staging");
  try {
    rmSync3(staging, { recursive: true, force: true });
  } catch {
  }
  mkdirSync8(staging, { recursive: true });
...
function findFile(dir, name, depth = 0) {
  if (depth > 4 || !existsSync37(dir)) return "";
  try {
    for (const entry of readdirSync24(dir, { withFileTypes: true })) {
      if (entry.name.startsWith(".")) continue;
      const p = join45(dir, entry.name);
      if (entry.isDirectory()) {
...
async function whisperDownloadModel(modelId, modelsDir, onProgress) {
  ...
  const file = entry[1];
  mkdirSync8(modelsDir, { recursive: true });
  const finalPath = join45(modelsDir, file);
  if (existsSync37(finalPath)) return { ok: true, file, bytes: 0 };
  let lastError = "\u4E0B\u8F7D\u5931\u8D25";
  for (const base of whisperDownloadBases()) {
    const url = `${base}/ggerganov/whisper.cpp/resolve/main/${file}`;
    const tmp = join45(modelsDir, file + ".part");
    try {
...
import { existsSync as existsSync39, mkdirSync as mkdirSync10, readFileSync as readFileSync19, readlinkSync, rmSync as rmSync5, symlinkSync, writeFileSync as writeFileSync7 } from "node:fs";
import { tmpdir } from "node:os";
import { basename as basename4, dirname as dirname12, join as join47 } from "node:path";

// src/backend/wechat-data/src/query/voice.ts
import { spawnSync as spawnSync2 } from "node:child_process";
import { existsSync as existsSync38, mkdirSync as mkdirSync9, readFileSync as readFileSync18, rmSync as rmSync4, writeFileSync as writeFileSync6 } from "node:fs";
...
function mediaDbPath(decryptedDir) {
  return join46(decryptedDir, "message", "media_0.db");
}
function usernameByChatId(decryptedDir, chatId) {
  try {
    const db = new DatabaseSync28(mediaDbPath(decryptedDir), { readOnly: true });
...
function recentVoiceMessages(decryptedDir, limit) {
  const dbPath6 = mediaDbPath(decryptedDir);
  if (!existsSync38(dbPath6)) return [];
  const db = new DatabaseSync28(dbPath6, { readOnly: true });
  try {
    const rows = db.prepare(
...
function voiceDataBySvr(decryptedDir, svrId) {
  const dbPath6 = mediaDbPath(decryptedDir);
  if (!existsSync38(dbPath6)) return null;
  const db = new DatabaseSync28(dbPath6, { readOnly: true });
  try {
...
function svrIdByChatLocal(decryptedDir, username, localId) {
  ...
  try {
    ...
    const db2 = new DatabaseSync28(dbPath6, { readOnly: true });
    try {
...
function silkDecoderBin(startDir) {
  ...
  try {
    ...
    for (let i = 0; i < 5; i += 1) {
      const candidate = join46(dir, "resources", "win32", "x64", "wx_silk.exe");
...
function silkToWav(silk, wavPath) {
  ...
  const tempDir = dirname11(wavPath);
  const silkPath = join46(tempDir, `.wx_silk_${process.pid}_${silk.length % 1e5}.silk`);
  try {
    mkdirSync9(tempDir, { recursive: true });
    writeFileSync6(silkPath, silk);
    ...
    if (res.status === 0 && existsSync38(wavPath)) return { ok: true };
...
function resolveVoiceDataUrl(decryptedDir, decodedDir, username, localId) {
  ...
  const wavPath = join46(decodedDir, "voices", svrId + ".wav");
  if (!existsSync38(wavPath)) {
  ...
  try {
    const bytes = readFileSync18(wavPath);
...
function voicesDir(decodedDir) {
  return join47(decodedDir, "voices");
}
function transcriptPath(decodedDir, svrId) {
  return join47(voicesDir(decodedDir), svrId + ".txt");
}
function cachedTranscript(decodedDir, svrId) {
  try {
    const t = readFileSync19(transcriptPath(decodedDir, svrId), "utf8").trim();
    return t;
  } catch {
    return "";
  }
}
var aliasLinks = /* @__PURE__ */ new Map();
function isAscii(s) {
  return !/[^\x00-\x7f]/.test(s);
}
function pickAliasBase(decodedDir) {
  const candidates = [join47(decodedDir, "..", "whisper-aliases"), join47(tmpdir(), "dsh-whisper-aliases")];
  for (const c of candidates) {
    if (!isAscii(c)) continue;
    try {
      mkdirSync10(c, { recursive: true });
      return c;
    } catch {
    }
  }
...
function ensureAsciiLink(realDir, aliasBase) {
  ...
  let link = join47(aliasBase, "wpa-" + createHash18("sha1").update(realDir.toLowerCase()).digest("hex").slice(0, 12));
  try {
    if (existsSync39(link)) {
      const target = readlinkSync(link).replace(/[\\/]+$/, "").toLowerCase();
      if (target !== realDir.replace(/[\\/]+$/, "").toLowerCase()) {
        rmSync5(link, { recursive: true, force: true });
...
function asciiPathForWhisper(filePath, aliasBase) {
  ...
  return join47(ensureAsciiLink(dirname12(filePath), aliasBase), basename4(filePath));
}
function whisperOne(bin, modelPath, wavPath, outBase) {
  ...
  if (done.status === 0 && existsSync39(outBase + ".txt")) {
    const text = readFileSync19(outBase + ".txt", "utf8").trim();
    if (text) return text;
  }
...
function transcribeVoiceText(decryptedDir, decodedDir, modelPath, engineBin, svrId, aliasBase) {
  ...
  try {
    ...
    const outBaseA = asciiPathForWhisper(join47(vdir, svrId), aliasBase);
    const modelPathA = asciiPathForWhisper(modelPath, aliasBase);
    const text = whisperOne(engineBin, modelPathA, wavPathA, outBaseA);
    if (text) {
      writeFileSync7(transcriptPath(decodedDir, svrId), text, "utf8");
      return { text };
    }
    return { error: "\u8F6C\u5199\u7ED3\u679C\u4E3A\u7A7A" };
  } catch (e) {
...
function transcribeOneVoice(decryptedDir, decodedDir, modelsDir, modelId, engineBin, username, localId) {
  ...
  const modelPath = join47(modelsDir, modelFile);
  if (!existsSync39(modelPath)) return { ok: false, error: `\u6A21\u578B\u672A\u5B89\u88C5: ${modelFile}` };
  if (!existsSync39(engineBin)) return { ok: false, error: "\u672A\u627E\u5230 whisper.cpp \u5F15\u64CE: " + engineBin };
  const svrId = svrIdByChatLocal(decryptedDir, username, localId);
...
import { mkdirSync as mkdirSync11, renameSync as renameSync5, rmSync as rmSync6, writeFileSync as writeFileSync8 } from "node:fs";
...
var HERE = dirname13(fileURLToPath4(import.meta.url));
...
function resolveAssetDir() {
  const candidates = [
    ...
    join48(processResourcesPath(), "app.asar.unpacked", "src", "backend", "wechat-data", "native", "weflow-isaac64"),
    join48(HERE, "..", "native", "weflow-isaac64"),
  ...
  for (const c of candidates) {
    try {
      if (readFileSync20(join48(c, "wasm_video_decode.wasm")).length > 0) return c;
    } catch {
    }
  }
...
function loadRuntime() {
  ...
  runtimePromise = (async () => {
    ...
    const wasmBinary = readFileSync20(join48(assetDir, "wasm_video_decode.wasm"));
    const glueJs = readFileSync20(join48(assetDir, "wasm_video_decode.js"), "utf8");
    let captured = null;
    let resolveInit = () => {
    };
    let rejectInit = () => {
    ...
    globals.location = { href: join48(assetDir, "wasm_video_decode.js") };
    globals.VTS_WASM_URL = `file://${join48(assetDir, "wasm_video_decode.wasm")}`;
...
var keystreamCache = /* @__PURE__ */ new Map();
async function wxIsaac64Keystream(seed, size) {
...
var MATCHABLE_EXT = [...VIDEO_EXT, ...IMAGE_EXT, ".tmp"];
var snsVideoIndex = /* @__PURE__ */ new Map();
var coverCache2 = /* @__PURE__ */ new Map();
var videoUrlCache = /* @__PURE__ */ new Map();
function md5OfFile(path) {
  try {
    return createHash19("md5").update(readFileSync21(path)).digest("hex");
  } catch {
    return null;
  }
}
function snsVideoMonthRoots(wechatBaseDir) {
  const cacheRoot = join49(wechatBaseDir, "cache");
  const out = [];
  if (!existsSync40(cacheRoot)) return out;
  let months = [];
  try {
...
function findByContentMd5(wechatBaseDir, wantMd5) {
  const roots = snsVideoMonthRoots(wechatBaseDir);
  let idx = snsVideoIndex.get(wechatBaseDir);
  if (!idx) {
    idx = { months: /* @__PURE__ */ new Map(), misses: /* @__PURE__ */ new Set() };
    snsVideoIndex.set(wechatBaseDir, idx);
  }
  const live = new Set(roots.map((r) => r.month));
  for (const m of [...idx.months.keys()]) if (!live.has(m)) idx.months.delete(m);
  if (idx.misses.has(wantMd5)) return void 0;
  for (const { month, root, mtimeMs, size } of roots) {
    const cached = idx.months.get(month);
    if (cached && cached.mtimeMs === mtimeMs && cached.size === size) {
      const hit2 = cached.byMd5.get(wantMd5);
...
async function fetchSnsVideoDataUrl(remoteUrl, expectMd5, opts = {}) {
...
async function fetchAndDecodeVideo(remoteUrl, expectMd5, opts = {}) {
  ...
  if (!isMp4Container(bytes)) {
    ...
    try {
      ...
      return { error: `\u89E3\u5BC6\u5931\u8D25\uFF1A${e?.message ?? String(e)}` };
    }
    if (!isMp4Container(bytes)) return { error: "\u89E3\u5BC6\u540E\u4ECD\u4E0D\u662F MP4 \u5BB9\u5668\uFF08<enc key> \u53EF\u80FD\u4E0D\u5339\u914D\uFF09" };
  }
  const got_md5 = createHash19("md5").update(bytes).digest("hex");
...
async function loadSnsVideoBytes(args) {
  const local = findLocalVideoPath(args.base, args.md5, args.timelineId, args.mediaId);
  if (local.path) {
    try {
      return { bytes: readFileSync21(local.path), source: "local" };
    } catch {
    }
  }
...
async function fetchSnsCoverDataUrl(remoteUrl, opts = {}) {
  const got = await fetchSnsMediaBytes(remoteUrl, opts);
  if (got.error || !got.bytes) return { error: got.error };
  let bytes = got.bytes;
...
async function fetchSnsMediaBytes(remoteUrl, opts) {
  if (!cdnFetchAllowed(opts)) return { error: CDN_DISABLED_MESSAGE };
  const timeoutMs = opts.timeoutMs ?? 6e4;
  const version = (opts.version || "").trim() || "4.1.13";
  try {
    const res = await (0, import_llm_retry4.fetchWithRetry)(fetch, remoteUrl, {
      headers: {
    ...
    if (e?.name === import_llm_retry4.TIMEOUT_ERROR_NAME) return { error: `\u4ECE\u5FAE\u4FE1 CDN \u53D6\u56DE\u8D85\u65F6\uFF08${Math.round(timeoutMs / 1e3)}s\uFF09` };
    const code = e?.cause?.code ?? "";
    return { error: `\u4ECE\u5FAE\u4FE1 CDN \u53D6\u56DE\u5931\u8D25\uFF1A${e?.message ?? String(e)}${code ? `\uFF08${code}\uFF09` : ""}` };
  }
}
function sibling(path, exts) {
...
function findInChatVideoCache(wechatBaseDir, names) {
  const root = join49(wechatBaseDir, "msg", "video");
  if (!existsSync40(root)) return null;
  const dirs = [root];
  try {
    for (const entry of readdirSync25(root, { withFileTypes: true })) {
      if (entry.isDirectory()) dirs.push(join49(root, entry.name));
    }
  } catch {
  }
  for (const dir of dirs) {
    for (const name of names) {
      const p = join49(dir, name);
      if (existsSync40(p)) return p;
    }
  }
  return null;
...
function resolveSnsVideoCoverDataUrl(wechatBaseDir, md5, _timelineId, _mediaId) {
  ...
  coverCache2.set(want, data);
  return { url: data };
}
function resolveSnsVideoDataUrl(wechatBaseDir, md5, _timelineId, _mediaId) {
  const want = (md5 || "").trim().toLowerCase();
  if (!want) return { error: "\u7F3A\u5C11\u89C6\u9891\u6807\u8BC6\uFF08XML \u672A\u7ED9\u51FA md5\uFF09" };
...
function findLocalVideoPath(wechatBaseDir, md5, _timelineId, _mediaId) {
  const want = (md5 || "").trim().toLowerCase();
  if (!want) return { error: "\u7F3A\u5C11\u89C6\u9891\u6807\u8BC6\uFF08XML \u672A\u7ED9\u51FA md5\uFF09" };
  if (!wechatBaseDir) return { error: "\u672A\u914D\u7F6E\u5FAE\u4FE1\u539F\u59CB\u76EE\u5F55\uFF0C\u65E0\u6CD5\u79BB\u7EBF\u89E3\u7801" };
...
import { existsSync as existsSync41, readdirSync as readdirSync26 } from "node:fs";
import { createHash as createHash20 } from "node:crypto";
import { join as join50 } from "node:path";
function msgTableName7(username) {
  return "Msg_" + createHash20("md5").update(username, "utf8").digest("hex");
}
function cellStr9(v) {
  if (typeof v === "string") return v;
  if (v === null || v === void 0) return "";
...
function queryAnnualReport(decryptedDir, year) {
  ...
  if (!existsSync41(msgDir)) return { year, total: 0 };
  ...
  for (const file of files) {
    let probe = null;
    try {
      probe = new DatabaseSync29(join50(msgDir, file), { readOnly: true });
    } catch {
      continue;
    }
    try {
      ...
      if (tables.length > 0) fileInfos.push({ path: join50(msgDir, file), tables });
    } catch {
    } finally {
      probe.close();
    }
  ...
  for (const fi of fileInfos) {
    let db = null;
    try {
      db = new DatabaseSync29(fi.path, { readOnly: true });
    } catch {
      continue;
    }
    try {
...
function loadUsernames(decryptedDir) {
  const p = join50(decryptedDir, "session", "session.db");
  if (!existsSync41(p)) return [];
  const out = [];
  try {
    const db = new DatabaseSync29(p, { readOnly: true });
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all().map((r) => r.name);
    const table = tables.includes("SessionTable") ? "SessionTable" : tables.includes("Session") ? "Session" : "";
    if (table) {
      const rows = db.prepare('SELECT username FROM "' + table + '"').all();
      for (const r of rows) {
        const u = cellStr9(r["username"] ?? "").trim();
        if (u) out.push(u);
      }
    }
    db.close();
...
function exportCsv(decryptedDir, kind, recordsKind) {
  const now = (/* @__PURE__ */ new Date()).toISOString().slice(0, 19).replace("T", " ").replace(/[-:]/g, "");
  const exportDir = join51(dirname14(decryptedDir), "exports");
  mkdirSync11(exportDir, { recursive: true });
  let rows = [];
  if (kind === "contacts") {
    rows = [["\u7528\u6237\u540D", "\u6635\u79F0", "\u5907\u6CE8", "\u7C7B\u578B"]];
  ...
  const filepath = join51(exportDir, filename);
...
function exportAnnualReport(decryptedDir, year, format, dir, filename) {
  ...
  const base = dir && dir.trim() ? dir.trim() : join51(dirname14(decryptedDir), "exports");
  ...
  const path = join51(base, safeName);
...
async function exportMoments(decryptedDir, opts) {
  ...
  if (opts?.zip) {
    ...
    const zipPath = join51(base, zipName);
    let mediaCount = 0;
...
async function exportAllSessions(decryptedDir, opts) {
  ...
  const path = join51(base, filename);
  const seen = /* @__PURE__ */ new Set();
  let total = 0;
...
var WINDOW_MAX_MSGS = 14;
var CHUNK_LIMIT = 10;
var CHUNK_LINES = 6;
function clusterChunks(ranked) {
...
function buildChunks(decryptedDir, ranked, termWeight, person, recency) {
...
function retrieveAskCitations(decryptedDir, question, hints, scope, limit) {
  ...
  let candidates = 0;
  let recallMode = "like";
  const termWeight = /* @__PURE__ */ new Map();
  ...
  if (person) {
    for (const c of ranked) {
      if (c.name.includes(person) || (c.sender ?? "").includes(person) || (c.body ?? "").includes(person)) c.score += 1;
    }
  ...
  if (start >= 0 && end > start) {
    try {
      ...
      if (Array.isArray(obj.subQueries)) {
        out.subQueries = obj.subQueries.filter((x) => typeof x === "string" && x.trim().length > 0).map((x) => x.trim().slice(0, 100)).slice(0, 4);
      }
  ...
  if (start >= 0 && end > start) {
    try {
      ...
      if (Array.isArray(obj.suggestions)) {
        out.suggestions = obj.suggestions.filter((x) => typeof x === "string" && x.trim().length > 0).map((x) => x.trim().slice(0, 200)).slice(0, 4);
      }
    } catch {
    }
  }
...
import { existsSync as existsSync42, readFileSync as readFileSync22, writeFileSync as writeFileSync9, mkdirSync as mkdirSync12 } from "node:fs";
...
function defaultRetrievalConfig() {
  return {
    ...
    compress: { maxChars: 6e3, maxChunks: 10, linesPerChunk: 6, dedupThreshold: 0.85 },
    intent: { llmAssist: false },
    feedback: { enabled: true, learningRate: 0.08, maxRecords: 500 }
  };
}
function retrievalRoot(decryptedDir) {
  return dirname15(decryptedDir);
}
function retrievalConfigPath(decryptedDir) {
  return join52(retrievalRoot(decryptedDir), "rag-config.json");
}
...
function saveRetrievalConfig(decryptedDir, patch) {
  ...
  const p = retrievalConfigPath(decryptedDir);
  mkdirSync12(dirname15(p), { recursive: true });
...
// src/backend/wechat-data/src/query/retrieval/intent.ts
var RECENCY_RE2 = /最近|最新|最后|上一次|上一回|上次|前几天|这两天|这几天|刚刚|近期/;
var AGG_RE = /一共|总共|合计|总计|汇总|加起来|总共多少|多少次|几次|几笔|多少笔|统计|总额|总金额|平均/;
var CMP_RE = /对比|比较|区别|差异|哪个更|谁更|哪个好|相比|vs\b|versus/i;
var ENTITY_ATTR_RE = /(电话|手机|号码|微信|微信号|地址|住址|生日|邮箱|邮箱地址|身份证|银行卡|老家|公司|单位|职位|职务|联系人)/;
var TIME_EXPR_RE = /(昨天|前天|今天|明天|上周[一二三四五六日天]?|这周[一二三四五六日天]?|周[一二三四五六日天]|星期[一二三四五六日天]|\d{1,2}月\d{1,2}[日号]|\d{4}年|\d{1,2}月份?|上个月|这个月|下个月|去年|前年|今年|\d{1,2}:\d{2})/;
var OPEN_ENDED_RE = /怎么样|怎样|如何|进展|什么情况|怎么回事/;
var WHO_RE = /是谁|谁的|哪个人|哪一位|哪位|何人/;
function classifyIntent(question, knownEntities = []) {
  const q = String(question || "");
  const recency = RECENCY_RE2.test(q);
...
function refineIntentWithLlm(base, llmIntent) {
  const allowed = ["recency_lookup", "entity_lookup", "time_range", "aggregation", "comparison", "open_qa"];
  const v = String(llmIntent || "").trim();
...
// src/backend/wechat-data/src/query/retrieval/rewrite.ts
var SYNONYM_GROUPS = [
  ["\u8F6C\u8D26", "\u6C47\u6B3E", "\u6253\u94B1", "\u8F6C\u94B1", "\u5FAE\u4FE1\u8F6C\u8D26"],
  ["\u7EA2\u5305", "\u538B\u5C81\u94B1", "\u53D1\u7EA2\u5305"],
  ["\u7535\u8BDD", "\u624B\u673A", "\u53F7\u7801", "\u624B\u673A\u53F7"],
...
function keyOf(username, localId) {
...
function fuseResults(channels, k, keep) {
  ...
  for (const ch of channels) {
    ...
    ch.hits.forEach((hit, idx) => {
      ...
      if (prevRank === void 0 || hit.rank < prevRank) {
...
function dedupeFused(docs, threshold, maxGapSec = 300) {
...
// src/backend/wechat-data/src/query/retrieval/rank.ts
function inSoft(ts2, fromMs, toMs) {
...
import { existsSync as existsSync43, statSync as statSync18 } from "node:fs";
import { join as join53 } from "node:path";
var VECTOR_SCHEMA_VERSION = "1";
function vectorDbPath(decryptedDir) {
  return join53(retrievalRoot(decryptedDir), "wechat_rag_vectors.db");
}
...
function openVectorDb(decryptedDir, readOnly = false) {
  const db = new DatabaseSync30(vectorDbPath(decryptedDir), readOnly ? { readOnly: true } : {});
  if (!readOnly) {
    db.exec("CREATE TABLE IF NOT EXISTS meta (key TEXT PRIMARY KEY, value TEXT NOT NULL)");
...
var STATUS_CACHE = /* @__PURE__ */ new Map();
function invalidateVectorStatusCache(p) {
  STATUS_CACHE.delete(p);
}
function readVectorIndexStatus(p) {
  if (!existsSync43(p)) return { exists: false, rows: 0, dim: 0, model: "", built_at: null, ready: false };
  try {
    const db = new DatabaseSync30(p, { readOnly: true });
    try {
      const meta = /* @__PURE__ */ new Map();
      try {
      ...
      const rows = Number.isFinite(rowsMeta) && hasVectorsTable(db) ? rowsMeta : db.prepare("SELECT COUNT(*) AS c FROM vectors").get().c;
...
function vectorIndexStatus(decryptedDir) {
...
var inflightVectorBuilds = /* @__PURE__ */ new Map();
function buildVectorIndex(decryptedDir, embed, opts) {
...
async function runBuildVectorIndex(decryptedDir, embed, opts) {
  ...
  try {
    ...
    if (!reset) {
      for (const row of db.prepare("SELECT fts_rowid FROM vectors").iterate()) {
    ...
    let pendingCount = 0;
    const sdb = new DatabaseSync30(src, { readOnly: true });
    try {
    ...
    if (pendingCount === 0) {
      db.exec("BEGIN");
      try {
        const known = db.prepare("SELECT COUNT(*) AS c FROM vectors").get().c;
    ...
    try {
      ...
      const total = db.prepare("SELECT COUNT(*) AS c FROM vectors").get().c;
...
async function searchDense(decryptedDir, queryText, embed, opts) {
  ...
  try {
    ...
    return { docs: [], scores: [], note: "embedding \u5931\u8D25: " + e.message };
  }
  const raw = qvecs[0];
...
function vectorIndexSummary(decryptedDir) {
...
function sparseChannel(decryptedDir, terms, topK, username) {
...
function feedbackDbPath(decryptedDir) {
  return join54(retrievalRoot(decryptedDir), "wechat_rag_feedback.db");
}
function openDb(decryptedDir) {
  const db = new DatabaseSync31(feedbackDbPath(decryptedDir));
...
function recordFeedback(decryptedDir, rec, maxRecords = 500) {
  const db = openDb(decryptedDir);
  try {
    db.prepare("INSERT OR REPLACE INTO feedback(id, question, answer, rating, cited_useful, cited_useless, intent, features, created_at) VALUES(?,?,?,?,?,?,?,?,?)").run(
      rec.id,
      rec.question,
      rec.answer,
      rec.rating,
      JSON.stringify(rec.citedUseful ?? []),
      JSON.stringify(rec.citedUseless ?? []),
      rec.intent,
      JSON.stringify(rec.features ?? []),
      rec.createdAt
    );
    const over = db.prepare("SELECT COUNT(*) AS c FROM feedback").get().c - maxRecords;
    if (over > 0) {
...
function listFeedback(decryptedDir, limit = 100) {
  if (!existsSync44(feedbackDbPath(decryptedDir))) return [];
  const db = openDb(decryptedDir);
  try {
...
function feedbackStats(decryptedDir) {
  if (!existsSync44(feedbackDbPath(decryptedDir))) return { total: 0, up: 0, down: 0 };
  const db = openDb(decryptedDir);
  try {
    const total = db.prepare("SELECT COUNT(*) AS c FROM feedback").get().c;
    const up = db.prepare("SELECT COUNT(*) AS c FROM feedback WHERE rating='up'").get().c;
...
function adaptWeights(base, recs, lr) {
  const out = { ...base };
  for (const r of recs) {
  ...
  for (const k of Object.keys(out)) out[k] = clamp(out[k]);
  return out;
}
function weightsPath(decryptedDir) {
  return join54(retrievalRoot(decryptedDir), "rag-weights.json");
}
...
// src/backend/wechat-data/src/query/retrieval/eval.ts
function isRelevant(key, relevantSet, textIndex) {
...
function ndcgAtK(retrieved, relevant, k, graded) {
  const gain = (key) => {
  ...
  const idealPool = Array.from(/* @__PURE__ */ new Set([...retrieved, ...relevant, ...Object.keys(graded ?? {})])).sort((a, b) => gain(b) - gain(a));
...
function evaluate(dataset, retrieve, opts = {}) {
  const k = opts.k ?? 10;
  const perCase = [];
  let sumP = 0;
  let sumR = 0;
  ...
  for (const c of dataset) {
    ...
    const n2 = ndcgAtK(retrieved, relList, k, c.graded);
    ...
    sumNdcg += n2;
    sumAp += a;
    perCase.push({ id: c.id, retrieved, precision: p, recall: r, mrr: m, ndcg: n2, ap: a });
  }
...
function formatEvalReport(name, report, k = 10) {
  const pct = (x) => (x * 100).toFixed(1) + "%";
  return [
    `\u8BC4\u6D4B\u96C6\u300C${name}\u300D\xB7 ${report.cases} \u4E2A\u7528\u4F8B \xB7 \u547D\u4E2D ${report.hits}/${report.cases}`,
...
// src/backend/wechat-data/src/query/retrieval/eval-dataset.ts
var SYNTHETIC_NOW = /* @__PURE__ */ new Date("2026-09-10T12:00:00");
function ts(y, mo, d, h = 10, mi = 0) {
  return Math.floor(new Date(y, mo - 1, d, h, mi, 0).getTime() / 1e3);
}
function doc(username, name, localId, createTime, text, sender) {
  return {
...
var CHEN = { u: "wxid_chen", n: "\u623F\u4E1C\u8001\u9648" };
var LISI = { u: "wxid_lisi", n: "\u674E\u56DB" };
var WANG = { u: "wxid_wangwu", n: "\u738B\u4E94" };
var GRP = { u: "12345678@chatroom", n: "\u9879\u76EE\u7EC4" };
var SYNTHETIC_CORPUS = [
...
function denseChannel2(query, corpus, topK, minSim) {
  const qv = ngramVec(query);
  const scored = corpus.map((d) => ({ d, s: cosine(qv, ngramVec(d.text)) })).filter((x) => x.s >= minSim).sort((a, b) => b.s - a.s).slice(0, topK);
  return {
    channel: "dense",
    active: true,
    hits: scored.map((x, i) => ({ doc: x.d, score: x.s, rank: i + 1 }))
...
function structuredChannel2(entity, from, to, corpus, topK) {
  const fromMs = from ? (/* @__PURE__ */ new Date(from + "T00:00:00")).getTime() : NaN;
  const toMs = to ? (/* @__PURE__ */ new Date(to + "T23:59:59")).getTime() : NaN;
  const hits = corpus.filter((d) => {
    const tsMs = d.create_time * 1e3;
...
function syntheticRetrieve(c, opts = {}) {
  ...
  const channels = [];
  if (opts.sparseEnabled !== false) channels.push(sparseChannel2(plan.terms, SYNTHETIC_CORPUS, policy.channelTopK.sparse));
  if (opts.denseEnabled !== false) channels.push(denseChannel2(plan.normalized, SYNTHETIC_CORPUS, policy.channelTopK.dense, 0.15));
  if (opts.structuredEnabled !== false) channels.push(structuredChannel2(plan.entity, plan.from, plan.to, SYNTHETIC_CORPUS, policy.channelTopK.structured));
  ...
  const ranked = rerankDocs({
    ...
    entity: plan.entity,
    softFromMs: plan.from ? (/* @__PURE__ */ new Date(plan.from + "T00:00:00")).getTime() : NaN,
    softToMs: plan.to ? (/* @__PURE__ */ new Date(plan.to + "T23:59:59")).getTime() : NaN,
    recency: plan.recency,
    recencyFirst: policy.recencyFirst,
    weights: { ...defaultWeights(), ...policy.weights },
    now: Math.floor(SYNTHETIC_NOW.getTime() / 1e3)
  });
...
function syntheticIntentAccuracy() {
  const knownEntities = [CHEN.n, LISI.n, WANG.n, GRP.n];
  let correct = 0;
  let total = 0;
...
function backupDir(decryptedDir) {
  return join55(dirname16(decryptedDir), "backups");
}
...
function collectFiles(root) {
  ...
  const walk = (dir) => {
    ...
    for (const e of readdirSync27(dir, { withFileTypes: true })) {
      ...
      else out.push({ abs: p, rel: relative3(root, p).split("\\").join("/"), size: statSync19(p).size });
    }
  };
  walk(root);
  return out;
...
function dirSize(dir) {
  ...
  const walk = (d) => {
    for (const e of readdirSync27(d, { withFileTypes: true })) {
      const p = join55(d, e.name);
      if (e.isDirectory()) walk(p);
      else try {
        size += statSync19(p).size;
      } catch {
      }
    }
  };
...
function dirSummary(dir) {
  ...
  return (db > 0 ? `${items} \u9879 \xB7 ${db} \u4E2A\u5E93` : `${items} \u9879`) + comp;
}
  ...
  try {
    if (kind === "enc") {
      ...
      try {
        ...
        readSync5(fd, buf, 0, MAGIC.length, 0);
    ...
    for (const e of readdirSync27(path, { withFileTypes: true })) {
      if (e.isDirectory()) {
        for (const sub of readdirSync27(join55(path, e.name))) {
          if (sub.endsWith(".db")) {
            hasDb = true;
            break;
          }
...
function previewBackup(decryptedDir, name) {
  ...
  const p = join55(dir, name);
  if (!existsSync45(p)) return { items: [], total: 0 };
  ...
  if (st.isDirectory()) {
    const walk = (d, prefix) => {
      ...
      let entries2 = [];
      try {
        entries2 = readdirSync27(d, { withFileTypes: true }).map((e) => ({ n: e.name, isDir: e.isDirectory() }));
      } catch {
        return;
      }
      for (const e of entries2) {
...
function listBackups(decryptedDir) {
  const dir = backupDir(decryptedDir);
  if (!existsSync45(dir)) return { items: [], total: 0 };
  const items = [];
  for (const name of readdirSync27(dir).sort().reverse()) {
    ...
    try {
      ...
      if (st.isDirectory()) {
        ...
        items.push({ name, path: p, size: st.size, modified: Math.floor(st.mtimeMs / 1e3), kind: "enc", summary: "AES-256 \u52A0\u5BC6\u5907\u4EFD", ok: backupOk(p, "enc") });
      }
    } catch {
    }
  }
...
function restoreEncryptedBackup(decryptedDir, name, password) {
  const dir = backupDir(decryptedDir);
  const src = join55(dir, name);
  if (!name.endsWith(".wcb") || !existsSync45(src)) return { ok: false, error: "\u52A0\u5BC6\u5907\u4EFD\u4E0D\u5B58\u5728" };
  const target = join55(dir, name.replace(/\.wcb$/, "") + ".restored");
  if (existsSync45(target)) rmSync7(target, { recursive: true, force: true });
  let fd = null;
  try {
    fd = openSync5(src, "r");
    ...
    const key = scryptSync(password, salt, 32);
    const expect = createHmac2("sha256", key).update(headerBuf).digest();
    ...
    for (const f of header.files) {
      ...
      const outPath = join55(target, f.name.split("/").join(process.platform === "win32" ? "\\" : "/"));
      mkdirSync14(dirname16(outPath), { recursive: true });
      try {
...
function readExactFd(fd, size) {
  ...
  let off = 0;
  while (off < size) {
    const n = readSync5(fd, buf, off, size - off, null);
...
function deleteBackup(decryptedDir, name) {
  const dir = backupDir(decryptedDir);
  const target = join55(dir, name);
  if (!target.startsWith(dir) || !existsSync45(target)) return { ok: false, error: "\u5907\u4EFD\u4E0D\u5B58\u5728" };
  try {
...
import { existsSync as existsSync46, readdirSync as readdirSync28 } from "node:fs";
...
function messageShardFiles4(decryptedDir) {
  const dir = join56(decryptedDir, "message");
  if (!existsSync46(dir)) return [];
  return readdirSync28(dir).filter((f) => f.endsWith(".db") && !f.includes("_shm") && !f.includes("_wal") && !f.includes("monitor_cache")).sort().map((f) => join56(dir, f));
}
function loadSessionUsernames3(decryptedDir) {
  const dbPath6 = join56(decryptedDir, "session", "session.db");
  if (!existsSync46(dbPath6)) return [];
  const out = [];
  try {
    const db = new DatabaseSync32(dbPath6, { readOnly: true });
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all().map((r) => r.name);
    const table = tables.includes("SessionTable") ? "SessionTable" : tables.includes("Session") ? "Session" : "";
    if (table) {
      const rows = db.prepare('SELECT username FROM "' + table + '"').all();
...
function decodeUtfOrGbk2(bytes) {
  const utf8 = new TextDecoder("utf-8", { fatal: false }).decode(bytes);
  if (!utf8.includes("\uFFFD")) return utf8;
  try {
    const gbk = new TextDecoder("gbk", { fatal: false }).decode(bytes);
...
function decodeCell4(v) {
  if (v === null || v === void 0) return "";
  if (typeof v === "string") return v;
  if (typeof v === "number" || typeof v === "boolean" || typeof v === "bigint" || typeof v === "symbol") return String(v);
  const raw = Buffer.from(v instanceof Uint8Array ? v : []);
  const decompressed = tryDecompress4(raw);
  return decodeUtfOrGbk2(decompressed ?? raw);
}
function stripXml2(xml) {
...
function isHumanText(text) {
  ...
  if (printable / t.length < 0.6) return false;
  if (/^[\d\s.,，。;；:：!！?？+*#\-—/]+$/.test(t)) return false;
  const nonSpace = t.length - space;
  if (nonSpace >= 40 && base64 / nonSpace > 0.75 && cjk === 0) return false;
...
var EMPTY_COLLECTED = { lines: [], count: 0, sessions: 0, total: 0, types: {}, hourly: [], topSessions: [] };
function tableColumns6(db, table) {
  const rows = db.prepare(`PRAGMA table_info("${table}")`).all();
  return new Set(rows.map((r) => r.name));
}
function collectRange(decryptedDir, start, end, cap, groupUsername, collectSources = false) {
  ...
  const sessionUsernames = loadSessionUsernames3(decryptedDir);
  const tableToUser = /* @__PURE__ */ new Map();
  for (const username of sessionUsernames) tableToUser.set(msgTableName8(username), username);
  const fileInfos = [];
  for (const shard of messageShardFiles4(decryptedDir)) {
    let probe = null;
    try {
      probe = new DatabaseSync32(shard, { readOnly: true });
    } catch {
      continue;
    }
    try {
      ...
      for (const [table, username] of fi.tables) {
        ...
        try {
          ...
          for (const r of rows) {
            ...
            const raw = decodeCell4(r["message_content"]);
            const text = stripXml2(raw);
            if (isHumanText(text)) {
              picked.push({ t, text });
              if (collectSources) {
                sources.push({
  ...
  const topSessions = Array.from(perSession.entries()).map(([username, count]) => ({ username, count })).sort((a, b) => b.count - a.count).slice(0, 8);
...
function decodeCell5(v) {
  ...
  if (typeof v === "number" || typeof v === "boolean" || typeof v === "bigint" || typeof v === "symbol") return String(v);
  const raw = Buffer.from(v instanceof Uint8Array ? v : []);
  const decompressed = raw.length >= 4 && raw.subarray(0, 4).equals(ZSTD_MAGIC7) ? (() => {
    try {
      return Buffer.from(decompress7(raw));
    } catch {
      return raw;
    }
  })() : raw;
...
function resolveMessagesByServerIds(decryptedDir, serverIds) {
  ...
  const scan = (serverExpr, params) => {
    ...
    for (const shard of shards) {
      let db;
      try {
        db = new DatabaseSync33(shard.file, { readOnly: true, readBigInts: true });
      } catch {
        continue;
      }
      try {
        for (const [table, meta] of shard.tables) {
          ...
          try {
            ...
            for (const r of rows) {
              const sid = decodeCell5(r["server_id"]).trim();
              if (sid && !out.has(sid)) {
                out.set(sid, {
                  createTime: Number(r["create_time"] ?? 0),
                  content: decodeCell5(r["message_content"])
                });
              }
            }
          } catch {
...
function parseAmount(s) {
  const str2 = typeof s === "string" || typeof s === "number" ? String(s) : "";
  const m = str2.match(/[\d.]+/);
  return m ? Number(m[0]) : 0;
}
function paymentFromContent(content) {
  if (!content) return { amount: 0, kind: null };
  const parsed = parseMessageContent(49, content, false);
  const rich = parsed.rich;
  if (!rich) return { amount: 0, kind: null };
  if (rich.type === "transfer") return { amount: parseAmount(rich.title), kind: "transfer" };
  if (rich.type === "redpacket") return { amount: parseAmount(rich.amount), kind: "redpacket" };
  return { amount: 0, kind: null };
}
function monthRange(month) {
  if (!month || !/^\d{4}-\d{2}$/.test(month)) return { from: 0, to: 0 };
  const parts = month.split("-");
  const y = Number(parts[0]);
  const m = Number(parts[1]);
  const from = Math.floor(new Date(y, m - 1, 1).getTime() / 1e3);
  const to = Math.floor(new Date(y, m, 1).getTime() / 1e3) - 1;
  return { from, to };
}
function queryLedger(decryptedDir, month, selfUsername) {
  ...
  const now = Math.floor(Date.now() / 1e3);
  const gdb = join57(decryptedDir, "general", "general.db");
  const transfers = [];
  const redpackets = [];
  const serverIds = /* @__PURE__ */ new Set();
  if (existsSync47(gdb)) {
    const db = new DatabaseSync33(gdb, { readOnly: true });
    try {
      ...
      if (tCols.has("session_name") && tCols.has("message_server_id")) {
        ...
        try {
          ...
          const rows = db.prepare(sql).all();
          for (const r of rows) {
            const sid = decodeCell5(r["message_server_id"]).trim();
            const sid2 = decodeCell5(r["second_message_server_id"]).trim();
            if (sid) serverIds.add(sid);
            if (sid2 && sid2 !== "0") serverIds.add(sid2);
            const payer = decodeCell5(r["pay_payer"]).trim();
            const receiver = decodeCell5(r["pay_receiver"]).trim();
            const session = decodeCell5(r["session_name"]).trim();
      ...
      if (rpCols.has("session_name") && rpCols.has("message_server_id")) {
        ...
        try {
          ...
          for (const r of rows) {
            const sid = decodeCell5(r["message_server_id"]).trim();
            if (sid) serverIds.add(sid);
            const sender = decodeCell5(r["sender_user_name"]).trim();
            const session = decodeCell5(r["session_name"]).trim();
  ...
  for (const [sid, info] of resolved) {
    ...
    if (pay.kind) byMsg.set(sid, { amount: pay.amount, time: info.createTime });
  }
...
function tableColumns7(db, table) {
  const rows = db.prepare(`PRAGMA table_info(${table})`).all();
  return new Set(rows.map((r) => r.name));
}
...
function momentsCount(decryptedDir, username) {
  for (const p of [join58(decryptedDir, "sns", "db_sns", "sns.db"), join58(decryptedDir, "sns", "sns.db")]) {
    ...
    try {
      ...
      const cols = tableColumns7(db, "SnsTimeLine");
...
function fundsCount(decryptedDir, username) {
  const p = join58(decryptedDir, "general", "general.db");
  if (!existsSync48(p)) return { transfers: 0, redpackets: 0 };
  try {
    const db = new DatabaseSync34(p, { readOnly: true });
    let transfers = 0;
    let redpackets = 0;
    try {
      const cols = tableColumns7(db, "transferTable");
    ...
    try {
      const cols = tableColumns7(db, "redEnvelopeTable");
      if (cols.has("session_name")) {
...
function commonGroups(decryptedDir, username) {
  const p = join58(decryptedDir, "contact", "contact.db");
  ...
  try {
    ...
    const cols = tableColumns7(db, "contact");
    const userCol = cols.has("username") ? "username" : cols.has("UserName") ? "UserName" : "";
    if (!userCol) {
      db.close();
      return [];
...
function queryContact360(decryptedDir, username) {
  const msg = messageStats2(decryptedDir, username);
...
import { existsSync as existsSync49, readdirSync as readdirSync29, statSync as statSync20 } from "node:fs";
import { dirname as dirname17, join as join59 } from "node:path";
function walkSqlite(dir, depth) {
  let dbCount = 0;
  let dbBytes = 0;
  let walCount = 0;
  let shmCount = 0;
  if (depth > 4 || !existsSync49(dir)) return { dbCount, dbBytes, walCount, shmCount };
  for (const e of readdirSync29(dir, { withFileTypes: true })) {
    const p = join59(dir, e.name);
    if (e.isDirectory()) {
...
function dirStats(dir) {
  let count = 0;
  let bytes = 0;
  if (!existsSync49(dir)) return { count, bytes };
  try {
    for (const e of readdirSync29(dir, { withFileTypes: true })) {
      const p = join59(dir, e.name);
      try {
...
function computeDbHealth(decryptedDir) {
  ...
  const root = dirname17(decryptedDir);
  const stores = [];
  for (const name of ["wechat_tasks.db", "daily_summary.db", "message_edits.db", "wechat_search.db", "config.json", "all_keys.json"]) {
    const p = join59(root, name);
    try {
      if (existsSync49(p)) stores.push({ name, size: statSync20(p).size });
    } catch {
    }
  }
  const images = dirStats(join59(root, "decoded_images"));
  return {
...
import { existsSync as existsSync50, readdirSync as readdirSync30 } from "node:fs";
...
function decodeCell6(v) {
  ...
  if (typeof v === "number" || typeof v === "boolean" || typeof v === "bigint" || typeof v === "symbol") return String(v);
  const raw = Buffer.from(v instanceof Uint8Array ? v : []);
  const decompressed = raw.length >= 4 && raw.subarray(0, 4).equals(ZSTD_MAGIC8) ? (() => {
    try {
      return Buffer.from(decompress8(raw));
    } catch {
      return raw;
    }
  })() : raw;
...
function tableColumns8(db, table) {
  const rows = db.prepare(`PRAGMA table_info(${table})`).all();
  return new Set(rows.map((r) => r.name));
}
...
function memberCount(decryptedDir, username) {
  const p = join60(decryptedDir, "contact", "contact.db");
  if (!existsSync50(p)) return 0;
  try {
    ...
    const cm = tableColumns8(db, "chatroom_member");
...
function announcement(decryptedDir, username) {
  const p = join60(decryptedDir, "contact", "contact.db");
  if (!existsSync50(p)) return { text: "", time: null };
  try {
    const db = new DatabaseSync35(p, { readOnly: true });
    const cols = tableColumns8(db, "contact");
    const userCol = cols.has("username") ? "username" : cols.has("UserName") ? "UserName" : "";
    ...
    const dc = tableColumns8(db, detail);
    ...
    return { text: decodeCell6(row.a).trim() || "", time: Number(row.t ?? 0) || null };
  } catch {
    return { text: "", time: null };
  }
}
...
function queryGroupInsights(decryptedDir, username) {
  ...
  const dir = join60(decryptedDir, "message");
  let total = 0;
  let minTime = null;
  let maxTime = null;
  ...
  if (existsSync50(dir)) {
    for (const f of readdirSync30(dir)) {
      ...
      let db;
      try {
        db = new DatabaseSync35(join60(dir, f), { readOnly: true });
      } catch {
        continue;
      }
      try {
        const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(table) !== void 0;
        if (!has) continue;
        const cols = tableColumns8(db, table);
        ...
        for (const r of db.prepare(sql).iterate()) {
          ...
          const text = decodeCell6(r["message_content"]);
  ...
  const topMembers = Array.from(perMember.entries()).map(([u, count]) => ({ username: u, name: names.get(u) ?? u, count })).sort((a, b) => b.count - a.count).slice(0, 10);
...
import { existsSync as existsSync51, readdirSync as readdirSync31 } from "node:fs";
...
function decodeCell7(v) {
  ...
  if (typeof v === "number" || typeof v === "boolean" || typeof v === "bigint" || typeof v === "symbol") return String(v);
  const raw = Buffer.from(v instanceof Uint8Array ? v : []);
  const decompressed = raw.length >= 4 && raw.subarray(0, 4).equals(ZSTD_MAGIC9) ? (() => {
    try {
      return Buffer.from(decompress9(raw));
    } catch {
      return raw;
    }
  })() : raw;
...
function tableColumns9(db, table) {
  const rows = db.prepare(`PRAGMA table_info(${table})`).all();
  return new Set(rows.map((r) => r.name));
}
function favoriteStats(decryptedDir) {
  const p = join61(decryptedDir, "favorite", "favorite.db");
  if (!existsSync51(p)) return { total: 0, byType: [], byYear: [] };
  try {
    ...
    const cols = tableColumns9(db, "fav_db_item");
    ...
    return {
      total: totalRow?.n ?? 0,
      byType: typeRows.map((r) => ({ type: r.t, label: favTypeLabel(r.t), count: r.n })).sort((a, b) => b.count - a.count),
...
function emoticonStats(decryptedDir, topUsages) {
  const p = join61(decryptedDir, "emoticon", "emoticon.db");
  let customCount = 0;
  let storePackages = 0;
  let captions = 0;
  if (existsSync51(p)) {
...
function topEmoticonUsage(decryptedDir, cap) {
  ...
  for (const f of readdirSync31(dir)) {
    ...
    try {
      ...
      for (const table of tables) {
        const cols = tableColumns9(db, table);
        ...
        try {
          ...
          for (const r of db.prepare(sql).iterate()) {
            const m = re.exec(decodeCell7(r["message_content"]));
            const md5 = m ? m[1] ?? "" : "";
...
function queryAssetInsights(decryptedDir) {
  const sig = [
    fileSigOf(join61(decryptedDir, "favorite", "favorite.db")),
    fileSigOf(join61(decryptedDir, "emoticon", "emoticon.db")),
    shardCatalogSig(decryptedDir, ["message"])
  ].join("|");
...
function tableColumns10(db, table) {
  const rows = db.prepare(`PRAGMA table_info(${table})`).all();
  return new Set(rows.map((r) => r.name));
}
function queryMediaAssets(decryptedDir) {
  return cachedBySig("media-assets:" + decryptedDir, fileSigOf(join62(decryptedDir, "hardlink", "hardlink.db")), () => computeMediaAssets(decryptedDir), 3e4);
}
function computeMediaAssets(decryptedDir) {
  const p = join62(decryptedDir, "hardlink", "hardlink.db");
  const categories = [];
  ...
  if (existsSync52(p)) {
    try {
      ...
      for (const [table, label] of MEDIA_TABLES) {
        const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(table) !== void 0;
        if (!has) continue;
        const cols = tableColumns10(db, table);
...
function computeTopItems(db, tidCol, names) {
  const empty = { rows: 0, users: 0, unread: 0, vanished: 0, top: [] };
  try {
    ...
    try {
      for (const r of db.prepare(`SELECT CAST(${tidCol} AS TEXT) AS t FROM SnsTimeLine`).iterate()) {
        alive.add(cellString6(r.t));
      }
    } catch {
    }
    ...
    let vanished = 0;
    for (const r of rows) {
      const username = cellString6(r.u);
      if (!username) continue;
      const isUnread = Number(r.d ?? 1) === 0;
      const isGone = alive.size > 0 && !alive.has(cellString6(r.t));
    ...
    const top = Array.from(byUser.entries()).map(([username, v]) => ({ username, name: names.get(username) ?? username, count: v.count, unread: v.unread, vanished: v.vanished, lastRead: v.lastRead })).sort((a, b) => b.count - a.count || a.username.localeCompare(b.username)).slice(0, 10);
    return { rows: rows.length, users: byUser.size, unread, vanished, top };
  } catch {
    return empty;
  }
}
function computeGeo(db) {
  const empty = { points: 0, cities: [], countries: [], places: [], pointList: [] };
  try {
    ...
    let points = 0;
    const pointList = [];
    for (const r of db.prepare("SELECT user_name AS u, content AS c FROM SnsTimeLine").iterate()) {
      const xml = cellString6(r.c);
...
function queryMomentsInsights(decryptedDir, author) {
  const dbPath6 = snsDb2(decryptedDir);
  const sig = [
    dbPath6 === null ? "" : fileSigOf(dbPath6),
    fileSigOf(join63(decryptedDir, "contact", "contact.db"))
  ].join("|");
...
function computeMomentsInsights(decryptedDir, author) {
  ...
  try {
    const db = new DatabaseSync38(dbPath6, { readOnly: true });
    const cols = new Set(db.prepare("PRAGMA table_info(SnsTimeLine)").all().map((r) => r.name));
    ...
    for (const r of rows) {
      ...
      for (const like2 of social.likes) addInteractor(likers, like2.username, like2.nickname);
      for (const cm of social.comments) addInteractor(commenters, cm.username, cm.nickname);
      if (parsed.createTime > 0) {
        const d = new Date(parsed.createTime * 1e3);
        const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
        monthly.set(key, (monthly.get(key) ?? 0) + 1);
        if (d.getMonth() === now.getMonth() && d.getDate() === now.getDate()) {
          today.push({ tid: cellString6(r.t), text: parsed.text, ts: parsed.createTime, author: cellString6(r.u) });
        }
      }
    }
  } catch (e) {
  ...
  const monthlyArr = Array.from(monthly.entries()).map(([month, count]) => ({ month, count })).sort((a, b) => a.month.localeCompare(b.month));
  return {
    author: author ?? "",
    posts,
    likes,
...
function queryMomentsMonthly(decryptedDir, author, authorName) {
  const dbPath6 = snsDb3(decryptedDir);
  const sig = [
    dbPath6 === null ? "" : fileSigOf(dbPath6),
...
function computeMomentsMonthly(decryptedDir, author, authorName) {
  ...
  if (dbPath6 === null) return [];
  try {
    const db = new DatabaseSync39(dbPath6, { readOnly: true });
    const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='SnsTimeLine'").get() !== void 0;
    if (!has) {
      db.close();
      return [];
    ...
    for (const r of rows) {
      if (authorName) {
        const u = cellString7(r.u);
        if ((names?.get(u) || u || "\u672A\u77E5") !== authorName) continue;
      }
      const xml = cellString7(r.c);
      if (!xml) continue;
    ...
    return Array.from(monthly.entries()).map(([month, count]) => ({ month, count })).sort((a, b) => a.month.localeCompare(b.month));
  } catch {
    return [];
  }
}
...
import { existsSync as existsSync55, readdirSync as readdirSync32 } from "node:fs";
...
function decodeCell8(v) {
  ...
  if (typeof v === "number" || typeof v === "boolean" || typeof v === "bigint" || typeof v === "symbol") return String(v);
  const raw = Buffer.from(v instanceof Uint8Array ? v : []);
  const decompressed = raw.length >= 4 && raw.subarray(0, 4).equals(ZSTD_MAGIC10) ? (() => {
    try {
      return Buffer.from(decompress10(raw));
    } catch {
      return raw;
    }
  })() : raw;
...
function tableColumns11(db, table) {
  const rows = db.prepare(`PRAGMA table_info(${table})`).all();
  return new Set(rows.map((r) => r.name));
}
function loadOfficialUsernames(decryptedDir) {
  const p = join65(decryptedDir, "session", "session.db");
  const out = [];
  if (!existsSync55(p)) return out;
  try {
    ...
    if (table) {
      const rows = db.prepare(`SELECT username FROM "${table}"`).all();
      for (const r of rows) {
        const u = decodeCell8(r["username"]).trim();
...
function queryOfficialAssets(decryptedDir) {
  const sig = [
    fileSigOf(join65(decryptedDir, "session", "session.db")),
...
function computeOfficialAssets(decryptedDir) {
  ...
  if (existsSync55(dir)) {
    for (const f of readdirSync32(dir)) {
      ...
      let db;
      try {
        db = new DatabaseSync40(join65(dir, f), { readOnly: true });
      } catch {
        continue;
      }
      try {
        ...
        for (const table of tables) {
          ...
          try {
            const rows2 = db.prepare(`SELECT create_time, local_type FROM "${table}"`).all();
            let acc = per.get(username);
            if (!acc) {
              acc = { messages: 0, articles: 0, lastArticleTime: null };
  ...
  const rows = Array.from(per.entries()).map(([username, v]) => ({ username, name: names.get(username) ?? username, ...v })).filter((r) => r.messages > 0).sort((a, b) => b.messages - a.messages).slice(0, 100);
  return { rows, total: rows.length, updatedAt: Math.floor(Date.now() / 1e3) };
}
...
var STATUSES = ["ok", "fail", "skip"];
var DEFAULT_LIMIT = 500;
var MAX_LIMIT = 5e3;
function dbPath(decryptedDir) {
  return join66(dirname18(decryptedDir), "wechat_privacy.db");
}
function openStore(decryptedDir) {
  const db = new DatabaseSync41(dbPath(decryptedDir));
...
function getPrivacyStateSnapshot(decryptedDir) {
  ...
  try {
    ...
    return {
      redactSensitive: settings.redactSensitive,
      blockOutbound: settings.blockOutbound,
      audit: { total: 0, byFeature: [], last: null, updatedAt: Math.floor(Date.now() / 1e3) }
    };
  }
}
...
function insertTask(decryptedDir, task) {
  try {
    ...
    const r = db.prepare("INSERT INTO tasks(title, status, due_at, source_username, source_local_id, message_time, created_at, updated_at) VALUES (?, 'open', ?, ?, ?, ?, ?, ?)").run(
      ...
      task.dueAt ?? null,
      task.sourceUsername ?? "",
      task.sourceLocalId ?? null,
      task.messageTime ?? null,
      ts2,
      ts2
    );
...
// src/backend/wechat-data/src/query/handoff.ts
function tableColumns12(db, table) {
  const rows = db.prepare(`PRAGMA table_info(${table})`).all();
  return new Set(rows.map((r) => r.name));
}
...
function listHandoffReminds(decryptedDir) {
  const p = join69(decryptedDir, "general", "general.db");
  if (!existsSync56(p)) return { items: [], total: 0 };
  try {
    ...
    const cols = tableColumns12(db, "handoff_remind_v0");
...
function tableColumns13(db, table) {
  const rows = db.prepare(`PRAGMA table_info(${table})`).all();
  return new Set(rows.map((r) => r.name));
}
function like(q) {
...
function searchContacts(decryptedDir, q, cap) {
  ...
  try {
    ...
    const cols = tableColumns13(db, "contact");
    const userCol = cols.has("username") ? "username" : cols.has("UserName") ? "UserName" : "";
    const remarkCol = cols.has("remark") ? "remark" : cols.has("Remark") ? "Remark" : "";
    ...
    db.close();
    return rows.map((r) => ({
      username: cellString8(r.u),
      name: cellString8(r.r) || cellString8(r.n) || cellString8(r.a) || cellString8(r.u),
      category: cellString8(r.u).endsWith("@chatroom") ? "group" : cellString8(r.u).startsWith("gh_") ? "official" : "contact"
    }));
  } catch {
    return [];
  }
...
function searchMoments(decryptedDir, q, cap) {
  const dbPathCandidates = [join70(decryptedDir, "sns", "db_sns", "sns.db"), join70(decryptedDir, "sns", "sns.db")];
  for (const p of dbPathCandidates) {
    if (!existsSync57(p)) continue;
    try {
      const db = new DatabaseSync45(p, { readOnly: true });
      const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='SnsTimeLine'").get() !== void 0;
      if (!has) {
      ...
      const cols = tableColumns13(db, "SnsTimeLine");
      const uname = cols.has("user_name") ? "user_name" : cols.has("userName") ? "userName" : "";
...
function searchFavorites(decryptedDir, q, cap) {
  ...
  try {
    ...
    const cols = tableColumns13(db, "fav_db_item");
...
function searchFiles(decryptedDir, q, cap) {
  const p = join70(decryptedDir, "hardlink", "hardlink.db");
  if (!existsSync57(p)) return [];
  try {
    const db = new DatabaseSync45(p, { readOnly: true });
    const out = [];
...
var PHRASE_MIN = 2;
var PHRASE_MAX = 8;
var CATCH_MIN_COUNT = 3;
function tryDecompress5(data) {
  if (data.length >= 4 && data.subarray(0, 4).equals(ZSTD_MAGIC11)) {
    try {
      return Buffer.from(decompress11(data));
    } catch {
      return null;
    }
  }
...
function decodeUtfOrGbk3(bytes) {
  const utf8 = new TextDecoder("utf-8", { fatal: false }).decode(bytes);
  if (!utf8.includes("\uFFFD")) return utf8;
  try {
    const gbk = new TextDecoder("gbk", { fatal: false }).decode(bytes);
...
function decodeCell9(v) {
  if (v === null || v === void 0) return "";
  if (typeof v === "string") return v;
  if (typeof v === "number" || typeof v === "boolean" || typeof v === "bigint" || typeof v === "symbol") return String(v);
  const raw = Buffer.from(v instanceof Uint8Array ? v : []);
  return decodeUtfOrGbk3(tryDecompress5(raw) ?? raw);
}
function splitSender2(content) {
...
var RE_ANSWERED_ELSEWHERE = /已在其它设备接听|已在其他设备接听/;
function parseCall(text) {
  const status = tagText2(text, "msg");
...
function argmax(arr) {
  let idx = 0;
  let best = -1;
  for (let i = 0; i < arr.length; i += 1) if ((arr[i] ?? 0) > best) {
    best = arr[i] ?? 0;
    idx = i;
  }
  return idx;
}
function chatOf(map, username) {
...
function isPersonChat(u) {
  return !!u && !u.endsWith("@chatroom") && !u.startsWith("gh_") && !u.endsWith("@openim");
}
function queryAnnualReview(decryptedDir, year, selfUsername = "") {
  const self = (selfUsername ?? "").trim();
  const names = contactMeta(decryptedDir).names;
  const start = Math.floor(new Date(year, 0, 1, 0, 0, 0, 0).getTime() / 1e3);
  const end = Math.floor(new Date(year + 1, 0, 1, 0, 0, 0, 0).getTime() / 1e3);
  ...
  let sent = 0;
  let totalAll = 0;
  const sentDays = /* @__PURE__ */ new Set();
  const allDays = /* @__PURE__ */ new Set();
  const perDayAll = /* @__PURE__ */ new Map();
  const heat = new Array(7 * 24).fill(0);
  ...
  const phraseCount = /* @__PURE__ */ new Map();
  const emojiCount = /* @__PURE__ */ new Map();
  const emojiDays = /* @__PURE__ */ new Set();
  const emojiHeat = new Array(7 * 24).fill(0);
  let threwStickers = 0;
  const stickerHashes = /* @__PURE__ */ new Set();
  let mediaSent = 0;
  let videoSent = 0;
  let voiceMsgSent = 0;
  ...
  const dayChat = /* @__PURE__ */ new Map();
  const dayBounds = /* @__PURE__ */ new Map();
  const chatEarliest = /* @__PURE__ */ new Map();
  for (const sh of shardCatalogDirs(decryptedDir, ["message", "bizchat"])) {
    let selfRowId;
    if (self) {
      const first = sh.tables.values().next().value;
      if (first) {
        for (const [id, name] of first.name2id) if (name === self) {
          selfRowId = id;
          break;
        }
        if (selfRowId === void 0) {
          for (const [id, name] of first.name2id) if (name === "") {
            selfRowId = id;
            break;
          }
        }
    ...
    let db = null;
    try {
      db = new DatabaseSync46(sh.file, { readOnly: true });
    } catch {
      continue;
    }
    try {
      for (const [table, meta] of sh.tables) {
        ...
        if (!username) continue;
        try {
          const minRow = db.prepare('SELECT MIN(create_time) AS m FROM "' + table + '"').get();
          ...
          for (const r of rows) {
            ...
            const raw = decodeCell9(r["message_content"]);
            const mine = hasSender && selfRowId !== void 0 && Number(r["real_sender_id"] ?? -1) === selfRowId;
            const d = new Date(ts2 * 1e3);
            ...
            const body0 = isRoom ? splitSender2(raw).body : raw;
            const text = readableText(body0);
            const b = dayBounds.get(dk);
            if (!b) dayBounds.set(dk, { first: ts2, firstText: text, last: ts2, lastText: text });
            else {
              if (ts2 < b.first) {
                b.first = ts2;
                b.firstText = text;
            ...
            if (hour <= 5) {
              if (mine) {
                chat.nightMine += 1;
                if (ts2 >= chat.nightSampleTs && text) {
                  chat.nightSampleTs = ts2;
                  chat.nightSampleText = text;
                }
            ...
            if (lt === 1 && text) {
              ...
              const sec = Math.round((Number(xmlAttr2(raw, "voicemsg", "voicelength")) || 0) / 1e3);
              if (mine) {
                voiceSentCount += 1;
                voiceMsgSent += 1;
                voiceSentSec += sec;
                if (sec > longestVoiceSec) {
                  longestVoiceSec = sec;
                  longestVoiceFrom = names.get(username) ?? username;
                }
              } else {
                voiceRecvCount += 1;
                voiceRecvSec += sec;
              ...
              if (mine) {
                threwStickers += 1;
                const h = xmlAttr2(raw, "emoji", "md5") || xmlAttr2(raw, "emoji", "cdnurl");
...
function assemble(b) {
  ...
  const nameOf = (u) => names.get(u) ?? u;
  for (const c of chats.values()) {
    let prev = null;
    let pendingTheirs = null;
  ...
  const yearStart = Math.floor(new Date(year, 0, 1).getTime() / 1e3);
  const newFriends = personChats.filter((c) => {
    const e = b.chatEarliest.get(c.username);
    return e !== void 0 && e >= yearStart;
  }).length;
  const calendar = [];
  ...
  let busiest = null;
  {
    let bestDay = "";
    let bestN = 0;
    for (const [d, n] of b.perDayAll) if (n > bestN) {
      bestN = n;
      bestDay = d;
    }
    if (bestDay) {
      ...
      let topCount = 0;
      if (dc) {
        for (const [u, n] of dc) if (n > topCount) {
          topCount = n;
          topName = nameOf(u);
        }
      }
  ...
  const monthlyStar = [];
  const starMonths = /* @__PURE__ */ new Map();
  ...
  let night = { share: 0, mine: 0, theirs: 0, topName: "", topCount: 0, sampleAt: "", sampleText: "" };
  {
    ...
    if (top && top.nightMine + top.nightTheirs > 0) {
      ...
      night = {
        ...
        sampleAt: t ? `${new Date(t * 1e3).getMonth() + 1}\u6708${new Date(t * 1e3).getDate()}\u65E5 ${clock(t)}` : "",
  ...
  for (let i = 0; i < b.heat.length; i += 1) {
    if ((b.heat[i] ?? 0) > brightestCount) {
      brightestCount = b.heat[i] ?? 0;
      brightestDow = Math.floor(i / 24);
      brightestHour = i % 24;
    }
  }
  let quietestHour = 0;
  ...
  for (let h = 0; h < 24; h += 1) {
    if ((b.hourAll[h] ?? 0) < quietestCount) {
      quietestCount = b.hourAll[h] ?? 0;
      quietestHour = h;
    }
  }
  ...
  const phraseRows = [...b.phraseCount.entries()].map(([phrase, count]) => ({ phrase, count })).sort((x, y) => y.count - x.count);
  const catchphrase = {
  ...
  const replyGapRows = personChats.filter((c) => c.replyGaps.length > 0).map((c) => ({ name: nameOf(c.username), gaps: c.replyGaps }));
  ...
  const ranking = personChats.map((c) => ({ username: c.username, name: nameOf(c.username), total: c.total, mine: c.mine, theirs: c.theirs })).sort((x, y) => y.total - x.total).slice(0, 10);
  let peakDow = 0;
  let peakHour = 0;
  let peakCount = 0;
  for (let i = 0; i < b.emojiHeat.length; i += 1) {
    if ((b.emojiHeat[i] ?? 0) > peakCount) {
      peakCount = b.emojiHeat[i] ?? 0;
  ...
  if (topCall) highlights.push({ label: "\u6700\u5E38\u8FDE\u7EBF", name: nameOf(topCall.username), username: topCall.username, value: `${topCall.calls} \u901A \xB7 ${humanDur(topCall.callSec)}` });
  ...
  const balanced = [...personChats].filter((c) => c.mine >= 20 && c.theirs >= 20).sort((x, y) => {
...
import { existsSync as existsSync58, readdirSync as readdirSync33 } from "node:fs";
...
function messageShardFiles5(decryptedDir) {
  ...
  return readdirSync33(dir).filter((f) => f.endsWith(".db") && !f.includes("_shm") && !f.includes("_wal") && !f.includes("monitor_cache")).sort().map((f) => join71(dir, f));
}
function editDbPath(decryptedDir) {
  return join71(dirname21(decryptedDir), "message_edits.db");
}
function openEditStore(decryptedDir) {
  const db = new DatabaseSync47(editDbPath(decryptedDir));
...
function listEditedMessages(decryptedDir, sessionId) {
  ...
  if (!existsSync58(p)) return { items: [], total: 0 };
  try {
    const db = new DatabaseSync47(p, { readOnly: true });
    let rows;
    if (sessionId) {
...
function findShard(decryptedDir, table, localId) {
  let fallback = null;
  for (const f of messageShardFiles5(decryptedDir)) {
    try {
      const db = new DatabaseSync47(f);
      const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(table) !== void 0;
      if (!has) {
        db.close();
        continue;
      ...
      const hit = db.prepare('SELECT 1 FROM "' + table + '" WHERE local_id = ? LIMIT 1').get(localId) !== void 0;
      if (hit) {
        if (fallback) fallback.close();
        return db;
      }
...
function editChatMessage(decryptedDir, username, localId, newContent) {
  ...
  try {
    ...
    if (!existing) {
      ...
      store.prepare(`INSERT INTO message_edits(account, session_id, db, table_name, local_id, first_edited_at, last_edited_at, edit_count, original_msg_json, edited_cols_json) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, '["` + contentCol + `"]')`).run(username, username, "message", table, localId, ts2, ts2, originalJson);
    } else {
      store.prepare("UPDATE message_edits SET last_edited_at = ?, edit_count = edit_count + 1 WHERE session_id = ? AND local_id = ?").run(ts2, username, localId);
    }
    store.close();
...
function resetEditedMessage(decryptedDir, username, localId) {
  const p = editDbPath(decryptedDir);
  if (!existsSync58(p)) return { ok: false, error: "\u7F16\u8F91\u8BB0\u5F55\u5E93\u4E0D\u5B58\u5728" };
  try {
    const store = new DatabaseSync47(p);
    ...
    store.close();
    const table = rec.table_name ?? msgTableName10(username);
    const db = findShard(decryptedDir, table, localId);
    if (!db) return { ok: false, error: "\u672A\u627E\u5230\u6D88\u606F\u5206\u5E93" };
    try {
      const cols = db.prepare('PRAGMA table_info("' + table + '")').all().map((r) => r.name);
      for (const [col2, val] of Object.entries(original)) {
        if (!cols.includes(col2)) continue;
        db.prepare('UPDATE "' + table + '" SET "' + col2 + '" = ? WHERE local_id = ?').run(decodeOriginalCell(val), localId);
      }
      const store2 = new DatabaseSync47(p);
...
function openSessionDb(decryptedDir) {
  const p = join72(decryptedDir, "session", "session.db");
  if (!existsSync59(p)) return null;
  try {
    return new DatabaseSync48(p);
  } catch {
    return null;
  }
}
...
function clearSessionDraft(decryptedDir, username) {
  ...
  if (!db) return { ok: false, updated: 0, error: "\u89E3\u5BC6 session.db \u4E0D\u5B58\u5728" };
  try {
    const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='SessionTable'").get() !== void 0;
    if (!has) {
      db.close();
...
function clearAllSessionDrafts(decryptedDir) {
  const db = openSessionDb(decryptedDir);
  if (!db) return { ok: false, cleared: [], count: 0, error: "\u89E3\u5BC6 session.db \u4E0D\u5B58\u5728" };
  try {
    const has = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='SessionTable'").get() !== void 0;
    if (!has) {
      db.close();
...
var WIKI_LINK_RE = /\[\[([^[\]\n]+)\]\]/g;
var WIKI_LINK_STRIP_RE = /\[\[([^[\]\n]+)\]\]/g;
function parseWikiLinks(body) {
  const out = [];
  if (!body) return out;
  WIKI_LINK_RE.lastIndex = 0;
  let m = WIKI_LINK_RE.exec(body);
  while (m !== null) {
    const inner = m[1] ?? "";
...
function excerptOf(body, limit = 140) {
  const flat = body.replace(WIKI_LINK_STRIP_RE, (_all, inner) => {
  ...
  try {
    ...
    if (id !== void 0 && Number.isFinite(id)) {
      ...
      const r2 = db.prepare("UPDATE notes SET title = ?, body = ?, tags = ?, source_kind = ?, source_username = ?, source_question = ?, updated_at = ? WHERE id = ?").run(
        ...
        input.sourceKind === void 0 ? prev.sourceKind : sourceKind,
...
function buildKnowledgeGraph(decryptedDir, names) {
  ...
  const edges = [];
  const stubMeta = /* @__PURE__ */ new Map();
  const outKeys = /* @__PURE__ */ new Map();
  ...
  for (const n of all) {
    ...
    for (const [key, weight] of uses) {
      ...
      if (targetId !== void 0) {
        ...
        if (!s) {
          s = { label: labelByKey.get(key) ?? key, refCount: 0, referencedBy: /* @__PURE__ */ new Set() };
          stubMeta.set(key, s);
        }
        s.refCount += weight;
  ...
  const stubs = [...stubMeta.entries()].map(([key, s]) => ({ key, label: s.label, refCount: s.refCount, referencedBy: [...s.referencedBy].sort((a, b) => a - b) })).sort((a, b) => b.refCount - a.refCount || a.label.localeCompare(b.label));
  const sessionNames = {};
  for (const n of all) {
    if (!n.sourceUsername || sessionNames[n.sourceUsername] !== void 0) continue;
...
function saveSummaryTask(decryptedDir, task) {
  try {
    ...
    if (task.id && task.id > 0) {
      db.prepare("UPDATE summary_tasks SET group_username = ?, group_name = ?, target_users = ?, format = ?, custom_prompt = ?, schedule_time = ?, enabled = ?, updated_at = ? WHERE id = ?").run(task.groupUsername, task.groupName, target, task.format, task.customPrompt, task.scheduleTime, task.enabled ? 1 : 0, ts2, task.id);
      db.close();
      return { ok: true, id: task.id };
    }
    const r = db.prepare("INSERT INTO summary_tasks(group_username, group_name, target_users, format, custom_prompt, schedule_time, enabled, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").run(task.groupUsername, task.groupName, target, task.format, task.customPrompt, task.scheduleTime, task.enabled ? 1 : 0, ts2, ts2);
    const id = Number(r.lastInsertRowid);
    db.close();
    return { ok: true, id };
  } catch (e) {
...
var _WechatDataGateway = class _WechatDataGateway extends (_a = TypertRemoteService, _getSessions_dec = [Remote("getSessions")], _getContacts_dec = [Remote("getContacts")], _getOverviewInsights_dec = [Remote("getOverviewInsights")], _getOverview_dec = [Remote("getOverview")], _getRegionMap_dec = [Remote("getRegionMap")], _getRecords_dec = [Remote("getRecords")], _searchMembers_dec = [Remote("searchMembers")], _getRevoked_dec = [Remote("getRevoked")], _getEmoticons_dec = [Remote("getEmoticons")], _getStorageStats_dec = [Remote("getStorageStats")], _getAnnual_dec = [Remote("getAnnual")], _getWechatConfig_dec = [Remote("getWechatConfig")], _getPrivacyScan_dec = [Remote("getPrivacyScan")], _getGraph_dec = [Remote("getGraph")], _getNotes_dec = [Remote("getNotes")], _saveNote_dec = [Remote("saveNote")], _deleteNote_dec = [Remote("deleteNote")], _getKnowledgeGraph_dec = [Remote("getKnowledgeGraph")], _getMoments_dec = [Remote("getMoments")], _getSelfUsername_dec = [Remote("getSelfUsername")], _getMomentsAuthors_dec = [Remote("getMomentsAuthors")], _getFavorites_dec = [Remote("getFavorites")], _getFiles_dec = [Remote("getFiles")], _getMessages_dec = [Remote("getMessages")], _getNewMessages_dec = [Remote("getNewMessages")], _getSearchIndexStatus_dec = [Remote("getSearchIndexStatus")], _buildSearchIndex_dec = [Remote("buildSearchIndex")], _searchMessages_dec = [Remote("searchMessages")], _getGroupInfo_dec = [Remote("getGroupInfo")], _getPaymentStatus_dec = [Remote("getPaymentStatus")], _resolveChatHistory_dec = [Remote("resolveChatHistory")], _getDailyCounts_dec = [Remote("getDailyCounts")], _getVoiceInfo_dec = [Remote("getVoiceInfo")], _getVoiceDataUrl_dec = [Remote("getVoiceDataUrl")], _getVideoInfo_dec = [Remote("getVideoInfo")], _exportSessionMessages_dec = [Remote("exportSessionMessages")], _askWechat_dec = [Remote("askWechat")], _optimizeAskQuestion_dec = [Remote("optimizeAskQuestion")], _listBackups_dec = [Remote("listBackups")], _previewBackup_dec = [Remote("previewBackup")], _createBackup_dec = [Remote("createBackup")], _deleteBackup_dec = [Remote("deleteBackup")], _getRetrievalStatus_dec = [Remote("getRetrievalStatus")], _saveRetrievalConfig_dec = [Remote("saveRetrievalConfig")], _buildRagVectorIndex_dec = [Remote("buildRagVectorIndex")], _submitAskFeedback_dec = [Remote("submitAskFeedback")], _listRetrievalFeedback_dec = [Remote("listRetrievalFeedback")], _resetRetrievalWeights_dec = [Remote("resetRetrievalWeights")], _evaluateRetrieval_dec = [Remote("evaluateRetrieval")], _generateDailySummary_dec = [Remote("generateDailySummary")], _listEditedMessages_dec = [Remote("listEditedMessages")], _editChatMessage_dec = [Remote("editChatMessage")], _resetEditedMessage_dec = [Remote("resetEditedMessage")], _listLlmProviders_dec = [Remote("listLlmProviders")], _listLlmModels_dec = [Remote("listLlmModels")], _exportAnnualReport_dec = [Remote("exportAnnualReport")], _exportAllSessions_dec = [Remote("exportAllSessions")], _exportMoments_dec = [Remote("exportMoments")], _exportCsv_dec = [Remote("exportCsv")], _clearSessionDraft_dec = [Remote("clearSessionDraft")], _clearAllSessionDrafts_dec = [Remote("clearAllSessionDrafts")], _listSummaryTasks_dec = [Remote("listSummaryTasks")], _saveSummaryTask_dec = [Remote("saveSummaryTask")], _deleteSummaryTask_dec = [Remote("deleteSummaryTask")], _toggleSummaryTask_dec = [Remote("toggleSummaryTask")], _listSummaryRecords_dec = [Remote("listSummaryRecords")], _deleteSummaryRecord_dec = [Remote("deleteSummaryRecord")], _runSummaryTask_dec = [Remote("runSummaryTask")], _getAvatar_dec = [Remote("getAvatar")], _getAvatarsLocal_dec = [Remote("getAvatarsLocal")], _getWechatConfigFull_dec = [Remote("getWechatConfigFull")], _saveWechatConfig_dec = [Remote("saveWechatConfig")], _getWhisperStatus_dec = [Remote("getWhisperStatus")], _downloadWhisperModel_dec = [Remote("downloadWhisperModel")], _detectWechatAccounts_dec = [Remote("detectWechatAccounts")], _verifyDatabaseKey_dec = [Remote("verifyDatabaseKey")], _generateKeysFile_dec = [Remote("generateKeysFile")], _getWechatKeysInfo_dec = [Remote("getWechatKeysInfo")], _autoGetDbKey_dec = [Remote("autoGetDbKey")], _autoGetImageKey_dec = [Remote("autoGetImageKey")], _openPath_dec = [Remote("openPath")], _openConfig_dec = [Remote("openConfig")], _verifyImageKey_dec = [Remote("verifyImageKey")], _decryptAllDatabases_dec = [Remote("decryptAllDatabases")], _decryptAllImages_dec = [Remote("decryptAllImages")], _getDecryptStatus_dec = [Remote("getDecryptStatus")], _installWhisperEngine_dec = [Remote("installWhisperEngine")], _transcribeVoiceBatch_dec = [Remote("transcribeVoiceBatch")], _getVoiceTranscript_dec = [Remote("getVoiceTranscript")], _transcribeVoiceMessage_dec = [Remote("transcribeVoiceMessage")], _setCdnImageEnabled_dec = [Remote("setCdnImageEnabled")], _setCdnImageLocalDecrypt_dec = [Remote("setCdnImageLocalDecrypt")], _deleteFavoriteItems_dec = [Remote("deleteFavoriteItems")], _getAnnualReview_dec = [Remote("getAnnualReview")], _getAnnualReport_dec = [Remote("getAnnualReport")], _getDbStatus_dec = [Remote("getDbStatus")], _getImageDataUrl_dec = [Remote("getImageDataUrl")], _getSnsImageDataUrl_dec = [Remote("getSnsImageDataUrl")], _getFileImageDataUrl_dec = [Remote("getFileImageDataUrl")], _getEmoticonDataUrl_dec = [Remote("getEmoticonDataUrl")], _getArticleCover_dec = [Remote("getArticleCover")], _getMessageFile_dec = [Remote("getMessageFile")], _addTask_dec = [Remote("addTask")], _clearOperationLog_dec = [Remote("clearOperationLog")], _clearPrivacyAudit_dec = [Remote("clearPrivacyAudit")], _createEncryptedBackup_dec = [Remote("createEncryptedBackup")], _deleteTask_dec = [Remote("deleteTask")], _extractTasks_dec = [Remote("extractTasks")], _generatePeriodSummary_dec = [Remote("generatePeriodSummary")], _getAssetInsights_dec = [Remote("getAssetInsights")], _getContact360_dec = [Remote("getContact360")], _getDbHealth_dec = [Remote("getDbHealth")], _getCalls_dec = [Remote("getCalls")], _getGroupInsights_dec = [Remote("getGroupInsights")], _getHandoffReminds_dec = [Remote("getHandoffReminds")], _getLedger_dec = [Remote("getLedger")], _getMediaAssets_dec = [Remote("getMediaAssets")], _getMomentsInsights_dec = [Remote("getMomentsInsights")], _getMomentsMonthly_dec = [Remote("getMomentsMonthly")], _getOfficialAssets_dec = [Remote("getOfficialAssets")], _getOperationLog_dec = [Remote("getOperationLog")], _getPrivacyAuditRows_dec = [Remote("getPrivacyAuditRows")], _getPrivacyState_dec = [Remote("getPrivacyState")], _getSnsVideoCoverDataUrl_dec = [Remote("getSnsVideoCoverDataUrl")], _getSnsVideoDataUrl_dec = [Remote("getSnsVideoDataUrl")], _exportSnsVideo_dec = [Remote("exportSnsVideo")], _listTasks_dec = [Remote("listTasks")], _restoreBackup_dec = [Remote("restoreBackup")], _searchUnified_dec = [Remote("searchUnified")], _setPrivacyState_dec = [Remote("setPrivacyState")], _setTaskStatus_dec = [Remote("setTaskStatus")], _syncHandoffTasks_dec = [Remote("syncHandoffTasks")], _a) {
  constructor(ctx) {
     ...
     */
    this._askTrace = /* @__PURE__ */ new Map();
  ...
  selfUsername() {
    ...
    const key = this._dirs.decrypted + "\0" + dbDir;
...
\u8BF7\u8F93\u51FA JSON\uFF08\u4E0D\u8981\u8F93\u51FA\u5176\u4ED6\u5185\u5BB9\uFF09\u3002`;
  ...
  saveRetrievalConfig(options) {
    const patch = options && typeof options === "object" && "patch" in options ? options.patch : options;
    const saved = saveRetrievalConfig(this._dirs.decrypted, patch);
  ...
  submitAskFeedback(options) {
    ...
    const pick = (idx) => {
      ...
      for (const i of idx ?? []) {
        const k = keyOf2(i);
        const f = k ? trace.features.get(k) : void 0;
        if (f) out.push(f);
      }
      return out;
    };
  ...
  listLlmProviders() {
    ...
    if (!provider) {
      try {
        ...
        const sel = adm && adm.currentSelection ? adm.currentSelection() : void 0;
        provider = sel?.provider ?? "";
      } catch {
      }
    }
  ...
  async listLlmModels(options) {
    ...
    if (ns) {
      try {
        const doc2 = ctx.settings?.get(ns) ?? {};
        let profile = doc2;
  ...
  async maybeRunDueTasks() {
    ...
    try {
      ...
      for (const t of tasks) {
        ...
        if (sched === hhmm && now.getTime() - lastRun > 6e4) due.push(t.id);
      }
      let next = 0;
      let failure = null;
      const worker = async () => {
      ...
      await Promise.all(
        Array.from({ length: Math.min(_WechatDataGateway.DUE_SUMMARY_CONCURRENCY, due.length) }, () => worker())
      );
      if (failure) throw failure;
    } catch (e) {
  ...
  saveWechatConfig(options) {
    ...
    const oldDirRaw = resolveWhisperModelsDir(before["whisper_models_dir"], this._dirs.decrypted);
  ...
  getWhisperStatus() {
    const cfg = getConfig(this._dirs.decrypted);
    const configured = resolveWhisperModelsDir(cfg["whisper_models_dir"], this._dirs.decrypted);
  ...
  async downloadWhisperModel(options) {
    ...
    const cfg = getConfig(this._dirs.decrypted);
    const modelsDir = resolveWhisperModelsDir(cfg["whisper_models_dir"], this._dirs.decrypted);
  ...
  async autoGetImageKey(options) {
    ...
    if (accountDir && existsSync60(accountDir)) {
      const cfg = getConfig(this._dirs.decrypted);
  ...
  async openConfig(signal) {
    const p = join75(this._dirs.decrypted, "..", "config.json");
    try {
      await openNativePath(p, signal);
      return { ok: true, path: p };
    } catch {
  ...
  async decryptAllImages(options) {
    ...
    this.decryptState.message = "";
    try {
      const result = await decryptAllImageDats(
        rawRoot,
        this._dirs.decoded,
        aesKey,
        xorKey,
  ...
  getAnnualReview(options) {
    try {
      ...
      this.op(
        ...
        "ok",
        String(options.year),
        `\u53D1\u51FA ${r.sent} \xB7 \u5168\u90E8 ${r.ranking.reduce((a, x) => a + x.total, 0)} \xB7 \u6D3B\u8DC3 ${r.activeDaysMine} \u5929`
      );
      return r;
    } catch (e) {
      this.op("task", "annual_review", "fail", String(options.year), e.message);
      throw e;
    }
  }
  ...
  async getEmoticonDataUrl(options) {
    const base = rawWechatBase(this._dirs.decrypted) || void 0;
    const cfg = getConfig(this._dirs.decrypted);
    const aesKey = typeof cfg["image_aes_key"] === "string" && cfg["image_aes_key"].length > 0 ? cfg["image_aes_key"] : void 0;
    const xorKey = Number(cfg["image_xor_key"] ?? 255);
  ...
  getMessageFile(options) {
    return resolveMessageFileDataUrl(rawWechatBase(this._dirs.decrypted) || void 0, options.fileName, {
      ...
      ...options.createTime !== void 0 ? { createTime: options.createTime } : {}
    });
  }
  addTask(options) {
  ...
  async getSnsVideoCoverDataUrl(options) {
    const base = rawWechatBase(this._dirs.decrypted) || void 0;
  ...
  async getSnsVideoDataUrl(options) {
    const base = rawWechatBase(this._dirs.decrypted) || void 0;
  ...
  async exportSnsVideo(options) {
    ...
    const loaded = await loadSnsVideoBytes({
      base: rawWechatBase(this._dirs.decrypted) || void 0,
      md5: options.md5,
    ...
    this.op("task", "export_sns_video", "ok", options.md5?.slice(0, 8) ?? "", `${loaded.bytes.length} \u5B57\u8282 \xB7 ${loaded.source}`);
...
__decorateElement(_init, 1, "getSessions", _getSessions_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getContacts", _getContacts_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getOverviewInsights", _getOverviewInsights_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getOverview", _getOverview_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getRegionMap", _getRegionMap_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getRecords", _getRecords_dec, _WechatDataGateway);
__decorateElement(_init, 1, "searchMembers", _searchMembers_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getRevoked", _getRevoked_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getEmoticons", _getEmoticons_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getStorageStats", _getStorageStats_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getAnnual", _getAnnual_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getWechatConfig", _getWechatConfig_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getPrivacyScan", _getPrivacyScan_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getGraph", _getGraph_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getNotes", _getNotes_dec, _WechatDataGateway);
...
__decorateElement(_init, 1, "getFavorites", _getFavorites_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getFiles", _getFiles_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getMessages", _getMessages_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getNewMessages", _getNewMessages_dec, _WechatDataGateway);
...
__decorateElement(_init, 1, "getDailyCounts", _getDailyCounts_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getVoiceInfo", _getVoiceInfo_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getVoiceDataUrl", _getVoiceDataUrl_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getVideoInfo", _getVideoInfo_dec, _WechatDataGateway);
__decorateElement(_init, 1, "exportSessionMessages", _exportSessionMessages_dec, _WechatDataGateway);
__decorateElement(_init, 1, "askWechat", _askWechat_dec, _WechatDataGateway);
__decorateElement(_init, 1, "optimizeAskQuestion", _optimizeAskQuestion_dec, _WechatDataGateway);
__decorateElement(_init, 1, "listBackups", _listBackups_dec, _WechatDataGateway);
__decorateElement(_init, 1, "previewBackup", _previewBackup_dec, _WechatDataGateway);
__decorateElement(_init, 1, "createBackup", _createBackup_dec, _WechatDataGateway);
__decorateElement(_init, 1, "deleteBackup", _deleteBackup_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getRetrievalStatus", _getRetrievalStatus_dec, _WechatDataGateway);
__decorateElement(_init, 1, "saveRetrievalConfig", _saveRetrievalConfig_dec, _WechatDataGateway);
__decorateElement(_init, 1, "buildRagVectorIndex", _buildRagVectorIndex_dec, _WechatDataGateway);
__decorateElement(_init, 1, "submitAskFeedback", _submitAskFeedback_dec, _WechatDataGateway);
__decorateElement(_init, 1, "listRetrievalFeedback", _listRetrievalFeedback_dec, _WechatDataGateway);
__decorateElement(_init, 1, "resetRetrievalWeights", _resetRetrievalWeights_dec, _WechatDataGateway);
__decorateElement(_init, 1, "evaluateRetrieval", _evaluateRetrieval_dec, _WechatDataGateway);
__decorateElement(_init, 1, "generateDailySummary", _generateDailySummary_dec, _WechatDataGateway);
__decorateElement(_init, 1, "listEditedMessages", _listEditedMessages_dec, _WechatDataGateway);
__decorateElement(_init, 1, "editChatMessage", _editChatMessage_dec, _WechatDataGateway);
__decorateElement(_init, 1, "resetEditedMessage", _resetEditedMessage_dec, _WechatDataGateway);
__decorateElement(_init, 1, "listLlmProviders", _listLlmProviders_dec, _WechatDataGateway);
...
__decorateElement(_init, 1, "saveSummaryTask", _saveSummaryTask_dec, _WechatDataGateway);
__decorateElement(_init, 1, "deleteSummaryTask", _deleteSummaryTask_dec, _WechatDataGateway);
__decorateElement(_init, 1, "toggleSummaryTask", _toggleSummaryTask_dec, _WechatDataGateway);
__decorateElement(_init, 1, "listSummaryRecords", _listSummaryRecords_dec, _WechatDataGateway);
__decorateElement(_init, 1, "deleteSummaryRecord", _deleteSummaryRecord_dec, _WechatDataGateway);
__decorateElement(_init, 1, "runSummaryTask", _runSummaryTask_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getAvatar", _getAvatar_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getAvatarsLocal", _getAvatarsLocal_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getWechatConfigFull", _getWechatConfigFull_dec, _WechatDataGateway);
__decorateElement(_init, 1, "saveWechatConfig", _saveWechatConfig_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getWhisperStatus", _getWhisperStatus_dec, _WechatDataGateway);
__decorateElement(_init, 1, "downloadWhisperModel", _downloadWhisperModel_dec, _WechatDataGateway);
__decorateElement(_init, 1, "detectWechatAccounts", _detectWechatAccounts_dec, _WechatDataGateway);
__decorateElement(_init, 1, "verifyDatabaseKey", _verifyDatabaseKey_dec, _WechatDataGateway);
__decorateElement(_init, 1, "generateKeysFile", _generateKeysFile_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getWechatKeysInfo", _getWechatKeysInfo_dec, _WechatDataGateway);
__decorateElement(_init, 1, "autoGetDbKey", _autoGetDbKey_dec, _WechatDataGateway);
__decorateElement(_init, 1, "autoGetImageKey", _autoGetImageKey_dec, _WechatDataGateway);
__decorateElement(_init, 1, "openPath", _openPath_dec, _WechatDataGateway);
__decorateElement(_init, 1, "openConfig", _openConfig_dec, _WechatDataGateway);
__decorateElement(_init, 1, "verifyImageKey", _verifyImageKey_dec, _WechatDataGateway);
__decorateElement(_init, 1, "decryptAllDatabases", _decryptAllDatabases_dec, _WechatDataGateway);
__decorateElement(_init, 1, "decryptAllImages", _decryptAllImages_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getDecryptStatus", _getDecryptStatus_dec, _WechatDataGateway);
__decorateElement(_init, 1, "installWhisperEngine", _installWhisperEngine_dec, _WechatDataGateway);
__decorateElement(_init, 1, "transcribeVoiceBatch", _transcribeVoiceBatch_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getVoiceTranscript", _getVoiceTranscript_dec, _WechatDataGateway);
__decorateElement(_init, 1, "transcribeVoiceMessage", _transcribeVoiceMessage_dec, _WechatDataGateway);
__decorateElement(_init, 1, "setCdnImageEnabled", _setCdnImageEnabled_dec, _WechatDataGateway);
__decorateElement(_init, 1, "setCdnImageLocalDecrypt", _setCdnImageLocalDecrypt_dec, _WechatDataGateway);
__decorateElement(_init, 1, "deleteFavoriteItems", _deleteFavoriteItems_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getAnnualReview", _getAnnualReview_dec, _WechatDataGateway);
...
__decorateElement(_init, 1, "getArticleCover", _getArticleCover_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getMessageFile", _getMessageFile_dec, _WechatDataGateway);
__decorateElement(_init, 1, "addTask", _addTask_dec, _WechatDataGateway);
__decorateElement(_init, 1, "clearOperationLog", _clearOperationLog_dec, _WechatDataGateway);
__decorateElement(_init, 1, "clearPrivacyAudit", _clearPrivacyAudit_dec, _WechatDataGateway);
__decorateElement(_init, 1, "createEncryptedBackup", _createEncryptedBackup_dec, _WechatDataGateway);
__decorateElement(_init, 1, "deleteTask", _deleteTask_dec, _WechatDataGateway);
__decorateElement(_init, 1, "extractTasks", _extractTasks_dec, _WechatDataGateway);
__decorateElement(_init, 1, "generatePeriodSummary", _generatePeriodSummary_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getAssetInsights", _getAssetInsights_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getContact360", _getContact360_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getDbHealth", _getDbHealth_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getCalls", _getCalls_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getGroupInsights", _getGroupInsights_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getHandoffReminds", _getHandoffReminds_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getLedger", _getLedger_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getMediaAssets", _getMediaAssets_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getMomentsInsights", _getMomentsInsights_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getMomentsMonthly", _getMomentsMonthly_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getOfficialAssets", _getOfficialAssets_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getOperationLog", _getOperationLog_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getPrivacyAuditRows", _getPrivacyAuditRows_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getPrivacyState", _getPrivacyState_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getSnsVideoCoverDataUrl", _getSnsVideoCoverDataUrl_dec, _WechatDataGateway);
__decorateElement(_init, 1, "getSnsVideoDataUrl", _getSnsVideoDataUrl_dec, _WechatDataGateway);
__decorateElement(_init, 1, "exportSnsVideo", _exportSnsVideo_dec, _WechatDataGateway);
__decorateElement(_init, 1, "listTasks", _listTasks_dec, _WechatDataGateway);
__decorateElement(_init, 1, "restoreBackup", _restoreBackup_dec, _WechatDataGateway);
__decorateElement(_init, 1, "searchUnified", _searchUnified_dec, _WechatDataGateway);
__decorateElement(_init, 1, "setPrivacyState", _setPrivacyState_dec, _WechatDataGateway);
__decorateElement(_init, 1, "setTaskStatus", _setTaskStatus_dec, _WechatDataGateway);
__decorateElement(_init, 1, "syncHandoffTasks", _syncHandoffTasks_dec, _WechatDataGateway);
__decoratorMetadata(_init, _WechatDataGateway);
/** Services this gateway depends on at runtime (LLM + default model). */
_WechatDataGateway.inject = ["llm", "agentDefaultModel"];
/**
 ...
 */
_WechatDataGateway.DUE_SUMMARY_CONCURRENCY = 2;
var WechatDataGateway = _WechatDataGateway;
