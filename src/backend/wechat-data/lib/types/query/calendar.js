 ...
 */
export function getDailyCounts(decryptedDir, username, year, month) {
    if (month < 1 || month > 12)
    ...
    const files = readdirSync(msgDir).filter(f => f.endsWith('.db') && f.startsWith('message_')
        && !f.includes('fts') && !f.includes('resource') && !f.includes('media')).sort();
        ...
        try {
            ...
            for (const r of rows) {
                ...
                if (Number.isFinite(day))
                    counts[String(day)] = (counts[String(day)] ?? 0) + r.c;
            }
        }
        catch {
