...
import { parseChatRoomExtBuffer } from "./group-info.js";
...
function loadMessageCounts(decryptedDir, usernames) {
    ...
    for (const u of usernames)
        tableToUser.set('Msg_' + createHash('md5').update(u, 'utf8').digest('hex'), u);
...
function loadRoomData(decryptedDir) {
    const out = { memberGroups: new Map(), roomMembers: new Map(), roomCounts: new Map() };
    const p = join(decryptedDir, 'contact', 'contact.db');
    if (!existsSync(p))
        return out;
    try {
        ...
        try {
            ...
            for (const r of users)
                idToUser.set(r.id, cellString(r.username));
        }
        ...
        for (const room of rooms) {
            ...
            if (!u)
                continue;
            if (!out.roomMembers.has(u))
                out.roomMembers.set(u, new Set());
        }
        try {
            const rows = db.prepare('SELECT room_id, member_id FROM chatroom_member').all();
            const roomByUsername = new Map();
            for (const room of rooms)
                roomByUsername.set(room.id, cellString(room.username));
            for (const r of rows) {
        ...
        for (const [u, members] of out.roomMembers)
            out.roomCounts.set(u, members.size);
        db.close();
    }
 ...
 */
export function queryGraph(decryptedDir, selfUsername) {
    const nodes = [];
    ...
    for (const [u, info] of meta) {
        if (selfWxid && cleanWxid(u) === selfWxid)
            continue;
    ...
    for (const u of talkers) {
        if (!u.endsWith('@chatroom'))
            continue;
        const members = roomData.roomMembers.get(u) ?? new Set();
        const shared = [];
    ...
    groups.sort((a, b) => (b.shared_count ?? 0) - (a.shared_count ?? 0)
