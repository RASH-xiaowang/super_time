 ...
 */
export function listEditedMessages(decryptedDir, sessionId) {
 ...
 */
export function editChatMessage(decryptedDir, username, localId, newContent) {
    const table = msgTableName(username);
    const db = findShard(decryptedDir, table);
    if (!db)
        return { ok: false, error: '未找到消息分库' };
    try {
        ...
        if (!contentCol) {
            db.close();
            return { ok: false, error: '消息表缺少内容列' };
        }
        ...
        if (strCol)
            db.prepare('UPDATE "' + table + '" SET "' + strCol + '" = ? WHERE local_id = ?').run(newContent, localId);
 ...
 */
export function resetEditedMessage(decryptedDir, username, localId) {
    const p = editDbPath(decryptedDir);
    if (!existsSync(p))
        return { ok: false, error: '编辑记录库不存在' };
    try {
