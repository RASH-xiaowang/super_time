...
function isHumanText(text) {
    ...
    if (printable / t.length < 0.6)
        return false; // binary remnant
    if (/^[\d\s.,，。;；:：!！?？+*#\-—/]+$/.test(t))
        return false; // numeric-only (0 0 0 0 0)
    const nonSpace = t.length - space;
    if (nonSpace >= 40 && base64 / nonSpace > 0.75 && cjk === 0)
        return false; // base64/hex blob
    if ((t.startsWith('{') || t.startsWith('[')) && t.includes(':'))
        return false; // JSON-ish
    if (cjk === 0 && nonSpace - base64 < 8)
        return false; // no real human chars
    return true;
}
...
const EMPTY_COLLECTED = { lines: [], count: 0, sessions: 0, total: 0, types: {}, hourly: [], topSessions: [] };
...
function collectRange(decryptedDir, start, end, cap, groupUsername, collectSources = false) {
    ...
    for (const shard of messageShardFiles(decryptedDir)) {
        ...
        try {
            ...
            if (tables.length > 0)
                fileInfos.push({ path: shard, tables });
        }
        catch { /* skip */ }
        finally {
            probe.close();
    ...
    for (const fi of fileInfos) {
        ...
        try {
            for (const [table, username] of fi.tables) {
                if (groupUsername && username !== groupUsername)
                    continue;
                const picked = [];
                try {
 ...
 */
export function collectDayMessages(decryptedDir, date, maxPerSession, groupUsername) {
 ...
 * @returns per-session message lines.
 */
export function collectPeriodMessages(decryptedDir, from, to, maxPerSession, groupUsername) {
    const cap = Math.min(maxPerSession ?? 30, 200);
 ...
 */
export function collectPeriodLinesWithSources(decryptedDir, from, to, maxPerSession, groupUsername) {
    const cap = Math.min(maxPerSession ?? 30, 200);
    const start = toStartSeconds(from);
    const end = toStartSeconds(to, 1);
    if (!Number.isFinite(start) || !Number.isFinite(end) || start > end)
