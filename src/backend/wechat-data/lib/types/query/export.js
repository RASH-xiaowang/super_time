...
import { queryMessages } from "./messages.js";
import { queryContacts } from "./contacts.js";
import { queryFavorites } from "./favorites.js";
import { queryRecords } from "./records.js";
import { queryMoments } from "./moments.js";
import { getConfig } from "./config.js";
import { resolveSnsImageDataUrl } from "./sns-image.js";
import { resolveSnsVideoDataUrl } from "./sns-video.js";
...
import { queryPrivacyScan } from "./privacy.js";
import { queryAnnualReport } from "./annual-report.js";
import { querySessions } from "./sessions.js";
...
function collectMessages(decryptedDir, username, count) {
    ...
    while (pages.length < target && guard < 600) {
        const env = queryMessages(decryptedDir, username, 100, cursor);
...
function rowOf(m, username) {
    ...
    return { time: fmtFull(m.createTime), sender, typeLabel, text: m.displayText || m.strContent || m.msgContent || '' };
}
/** txt export body. */
...
function collectChatlogMedia(msgs) {
    ...
    for (const m of msgs) {
        const rich = m.rich;
        if (!rich || rich.type !== 'chatlog' || !Array.isArray(rich.records))
            continue;
        for (const rec of rich.records) {
            ...
            if (rec.datatype)
                item.datatype = rec.datatype;
            if (rec.fullmd5)
                item.fullmd5 = rec.fullmd5;
            if (rec.thumbfullmd5)
                item.thumbfullmd5 = rec.thumbfullmd5;
            if (rec.md5)
                item.md5 = rec.md5;
            if (rec.cdnurlstring)
                item.cdnurlstring = rec.cdnurlstring;
            if (rec.encrypturlstring)
                item.encrypturlstring = rec.encrypturlstring;
            if (rec.link)
                item.link = rec.link;
            if (rec.fromnewmsgid)
                item.fromnewmsgid = rec.fromnewmsgid;
            out.push(item);
        }
    }
    return out;
...
export function exportSessionMessages(decryptedDir, username, format, count, dir, types, richTypes, from, to, filename, zip) {
    const all = collectMessages(decryptedDir, username, count ?? 0);
    const msgs = filterMessages(all, types, richTypes).filter((m) => {
        if (from && from > 0 && m.createTime < from)
            return false;
        if (to && to > 0 && m.createTime > to)
            return false;
        return true;
    });
    ...
    let content = '';
    if (format === 'csv')
        content = formatCsv(msgs, username);
    else if (isXlsx)
        content = formatXlsx(msgs, username);
    else if (format === 'html')
        content = formatHtml(msgs, username, now);
    else if (format === 'md')
        content = formatMarkdown(msgs, username);
    else if (format === 'sql')
        content = formatSql(msgs, username);
    else if (format === 'json')
        content = formatJson(msgs, username);
    else
        content = formatTxt(msgs, username);
 ...
 */
export function exportCsv(decryptedDir, kind, recordsKind) {
    const now = new Date().toISOString().slice(0, 19).replace('T', ' ').replace(/[-:]/g, '');
 ...
 */
export function exportAnnualReport(decryptedDir, year, format, dir, filename) {
    ...
    if (ext === 'html') {
        ...
        if (kindLine)
            items.push('<h2>消息类型</h2><p>' + kindLine + '</p>');
        if (phrases)
            items.push('<h2>高频短语</h2><p>' + phrases + '</p>');
        if (emoji)
            items.push('<h2>表情宇宙</h2><p>' + emoji + '</p>');
        ...
        if (kindLine)
            md.push('消息类型: ' + kindLine);
        if (phrases)
            md.push('高频短语: ' + phrases);
        if (emoji)
            md.push('表情宇宙: ' + emoji);
        if (contacts.length > 0) {
            md.push('聊得最多的人:');
 ...
 */
export function exportMoments(decryptedDir, opts) {
    ...
    const filtered = items.filter((m) => {
        ...
        if (authorName && m.author !== authorName)
            return false;
        // 媒体类型（与列表类型筛选一致）。
        if (media && media !== 'all') {
            if (media === 'image' && m.images.length === 0)
                return false;
            if (media === 'video' && m.videos.length === 0)
                return false;
            if (media === 'link' && !(m.link_title || m.contentType === 3 || m.contentType === 28))
                return false;
            if (media === 'location' && !m.location)
                return false;
            if (media === 'text' && !(m.images.length === 0 && m.videos.length === 0 && !m.link_title))
                return false;
        }
        // 范围（我/他人）。
        if (mine === 'mine' && !m.is_self)
            return false;
        if (mine === 'others' && m.is_self)
            return false;
        // 月份（YYYY-MM）。
        if (month && m.ts) {
        ...
        for (const m of filtered) {
            for (const im of m.images) {
                ...
                if (r.url) {
                    ...
                    if (buf)
                        entries.push({ name: 'media/images/img_' + String(idx++) + '.jpg', data: buf });
                }
            ...
            for (const v of m.videos) {
                ...
                if (r.url) {
                    const buf = dataUrlToBuffer(r.url);
                    if (buf)
                        entries.push({ name: 'media/videos/vid_' + String(idx++) + '.mp4', data: buf });
                }
                if (entries.length > 5000)
                    break;
            }
    ...
    if (ext === 'json') {
        ...
        for (const m of filtered) {
            ...
            if (m.text)
                parts.push('<div class="content">' + htmlEscape(m.text) + '</div>');
            ...
            if (m.location)
                parts.push('<div class="tag">📍' + htmlEscape(m.location) + '</div>');
            if (m.link_title) {
            ...
            if (m.likes.length > 0)
                parts.push('<div class="tag">❤ ' + htmlEscape(m.likes.map(l => l.nickname || l.username || '').join('、')) + '</div>');
        ...
        for (const m of filtered) {
            ...
            const tags = [];
            if (m.images.length > 0)
                tags.push('图片×' + String(m.images.length));
            if (m.videos.length > 0)
                tags.push('视频×' + String(m.videos.length));
            if (m.location)
                tags.push('📍' + m.location);
            if (m.link_title)
                tags.push('🔗' + m.link_title + (m.link_url ? ' ' + m.link_url : ''));
            if (tags.length > 0)
 ...
 */
export function exportAllSessions(decryptedDir, opts) {
