...
import { cachedBySig, fileSigOf, shardCatalogSig } from "./meta.js";
import { favTypeLabel } from "./favorites.js";
...
function favoriteStats(decryptedDir) {
    ...
    try {
        ...
        const typeRows = typeCol
            ? db.prepare(`SELECT ${typeCol} AS t, COUNT(*) AS n FROM fav_db_item GROUP BY ${typeCol}`).all()
            : [];
        const yearRows = timeCol
            ? db.prepare(`SELECT CAST(strftime('%Y', datetime(${timeCol}, 'unixepoch')) AS INTEGER) AS y, COUNT(*) AS n FROM fav_db_item GROUP BY y ORDER BY y DESC`).all()
            : [];
...
function emoticonStats(decryptedDir, topUsages) {
    ...
    if (existsSync(p)) {
        try {
            const db = new DatabaseSync(p, { readOnly: true });
            if (tableExists(db, 'kNonStoreEmoticonTable'))
                customCount = db.prepare('SELECT COUNT(*) AS n FROM kNonStoreEmoticonTable').get().n;
            if (tableExists(db, 'kStoreEmoticonPackageTable'))
                storePackages = db.prepare('SELECT COUNT(*) AS n FROM kStoreEmoticonPackageTable').get().n;
            if (tableExists(db, 'kStoreEmoticonCaptionsTable'))
                captions = db.prepare('SELECT COUNT(*) AS n FROM kStoreEmoticonCaptionsTable').get().n;
            db.close();
        }
        catch { /* keep zeros */ }
    }
    ...
    for (const f of readdirSync(dir)) {
        ...
        try {
            ...
            for (const table of tables) {
                ...
                if (!cols.has('local_type') || !cols.has('message_content'))
                    continue;
                try {
 ...
 */
export function queryAssetInsights(decryptedDir) {
    const sig = [
...
//# sourceMappingURL=asset-insights.js.map