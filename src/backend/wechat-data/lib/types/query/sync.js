...
export async function fullDecryptFile(dbPath, outPath, encKey) {
 ...
 */
export async function decryptWalPatch(walPath, outPath, encKey) {
    if (!existsSync(walPath))
        return 0;
    const wal = await readFile(walPath);
    if (wal.length <= WAL_HEADER_SZ)
        return 0;
...
export async function walFramesPatchable(walPath) {
    if (!existsSync(walPath))
        return false;
    const wal = await readFile(walPath);
    if (wal.length <= WAL_HEADER_SZ)
...
function fullDecryptDue(key) {
    ...
    if (Date.now() - last < FULL_DECRYPT_COOLDOWN_MS)
        return false;
 ...
 */
export function resolveShardKey(decryptedDir, relKey, fallbackHex) {
 ...
 */
export function deriveEncKeyCached(decryptedDir, relKey, rawDbFile, rawKeyHex, keyFormat) {
    ...
    if (derivedKeyCache.size >= 128) {
        ...
        if (!first.done)
            derivedKeyCache.delete(first.value);
    }
...
async function stageWal(rawWal, stagingWal) {
    if (existsSync(rawWal) && statSync(rawWal).size > WAL_HEADER_SZ)
        await copyFile(rawWal, stagingWal);
    else if (existsSync(stagingWal))
        await unlink(stagingWal).catch(() => { });
}
/**
 ...
 */
export async function syncMessageShard(rawMsgDir, decMsgDir, file, rawKeyHex, keyFormat, mode = 'full') {
    ...
    if (mode === 'wal') {
        ...
        try {
            ...
            if (!looksDecrypted(stagingDb))
                throw new Error('wal patch invalid for ' + file);
    ...
    try {
        ...
        if (!looksDecrypted(temp))
            throw new Error('decrypt result invalid for ' + file);
 ...
 */
export async function syncChangedShards(rawDbDir, decryptedDir, lastState) {
    ...
    for (const f of cachedDirEntries(msgDir)) {
        if (!/^(biz_)?message_\d+\.db$/.test(f))
            continue;
        const sig = shardSig(msgDir, f);
        const prev = lastState.get(f);
        const target = join(decMsgDir, f);
        if (prev !== undefined && prev.main === sig.main && prev.wal === sig.wal && existsSync(target))
            continue;
        lastState.set(f, sig);
        const mode = (prev === undefined || prev.main !== sig.main || !existsSync(target)) ? 'full' : 'wal';
        ...
        try {
            ...
            if (mode === 'full' || r.walPages > 0)
                synced.push(`${f}:${mode}(${r.fullPages}p/w${r.walPages})`);
        }
        catch (e) {
 ...
 */
export async function syncSessionDb(rawDbDir, decryptedDir, lastState, rawKeyHex, keyFormat) {
    ...
    try {
        ...
        if (mode === 'wal') {
            ...
            try {
                const walPages = existsSync(stagingWal) ? await decryptWalPatch(stagingWal, stagingDb, encKey) : 0;
                if (!looksDecrypted(stagingDb))
                    throw new Error('session wal patch invalid');
                await atomicReplace(stagingDb, target);
                if (walPages > 0)
                    return [`session.db:wal(w${walPages})`];
                return [];
            }
            finally {
                for (const f of [stagingWal, temp]) {
        ...
        try {
            ...
            const walPages = existsSync(stagingWal) ? await decryptWalPatch(stagingWal, temp, encKey) : 0;
            if (!looksDecrypted(temp))
                throw new Error('session decrypt invalid');
            await atomicReplace(temp, target);
 ...
 * @returns descriptions of what was synced (empty when nothing changed).
 */
export async function syncContactDb(rawDbDir, decryptedDir, lastState, rawKeyHex, keyFormat) {
    ...
    try {
        ...
        if (mode === 'wal') {
            ...
            try {
                const walPages = existsSync(stagingWal) ? await decryptWalPatch(stagingWal, stagingDb, encKey) : 0;
                if (!looksDecrypted(stagingDb))
                    throw new Error('contact wal patch invalid');
                await atomicReplace(stagingDb, target);
                if (walPages > 0)
                    return [`contact.db:wal(w${walPages})`];
                return [];
            }
            finally {
                for (const f of [stagingWal, temp]) {
        ...
        try {
            ...
            const walPages = existsSync(stagingWal) ? await decryptWalPatch(stagingWal, temp, encKey) : 0;
            if (!looksDecrypted(temp))
                throw new Error('contact decrypt invalid');
            await atomicReplace(temp, target);
 ...
 * @returns descriptions of what was synced (empty when nothing changed).
 */
export async function syncPlainDatabase(rawRoot, decryptedDir, sub, dbName, label, rawKeyHex, keyFormat, lastState) {
    ...
    try {
        ...
        if (mode === 'wal') {
            ...
            try {
                const walPages = existsSync(stagingWal) ? await decryptWalPatch(stagingWal, stagingDb, encKey) : 0;
                if (!looksDecrypted(stagingDb))
                    throw new Error(label + ' wal patch invalid');
                await atomicReplace(stagingDb, target);
                if (walPages > 0)
                    return [label + ':wal(w' + String(walPages) + ')'];
                return [];
            }
            finally {
                for (const f of [stagingWal, temp]) {
        ...
        try {
            ...
            const walPages = existsSync(stagingWal) ? await decryptWalPatch(stagingWal, temp, encKey) : 0;
            if (!looksDecrypted(temp))
                throw new Error(label + ' decrypt invalid');
            await atomicReplace(temp, target);
 ...
 */
export async function cleanStaleStagingFiles(root) {
 ...
 */
export function startRealtimeSync(rawDbDir, decryptedDir, onSync) {
