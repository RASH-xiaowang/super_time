 ...
 */
export function queryGroupInsights(decryptedDir, username) {
    ...
    if (existsSync(dir)) {
        for (const f of readdirSync(dir)) {
            ...
            try {
                ...
                if (!cols.has('message_content') || !cols.has('create_time'))
                    continue;
                ...
                for (const r of rows) {
                    ...
                    if (sender)
                        perMember.set(sender, (perMember.get(sender) ?? 0) + 1);
                }
            }
            catch { /* skip */ }
            finally {
    ...
    const topMembers = Array.from(perMember.entries())
...
//# sourceMappingURL=group-insights.js.map