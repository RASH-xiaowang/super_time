...
export function queryOverviewExtras(decryptedDir) {
...
function computeOverviewExtras(dec) {
    ...
    let prev7 = 0;
    let prev30 = 0;
    const contactActive7 = new Set();
    const contactActive30 = new Set();
    const groupActive7 = new Set();
    const groupActive30 = new Set();
    let lastSync = 0;
                ...
                for (const [table, meta] of sh.tables) {
                    ...
                    for (const r of rows) {
                        ...
                        if (day >= day7) {
                            m7 += r.n;
                            if (!isGroup)
                                contactActive7.add(talker);
                            else
                                groupActive7.add(talker);
                        }
                        if (day >= day30) {
                            m30 += r.n;
                            if (!isGroup)
                                contactActive30.add(talker);
                            else
                                groupActive30.add(talker);
                        }
                        if (day >= day60)
                            m60 += r.n;
                        if (day >= day14 && day < day7)
                            prev7 += r.n;
                        if (day >= day60 && day < day30)
                            prev30 += r.n;
                        if (day > lastSync)
                            lastSync = day + 86399;
                    }
...
//# sourceMappingURL=overview-extras.js.map