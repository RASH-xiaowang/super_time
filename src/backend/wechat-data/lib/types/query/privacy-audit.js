...
export function readPrivacySettings(decryptedDir) {
    try {
...
export function writePrivacySettings(decryptedDir, patch) {
    const db = openStore(decryptedDir);
    try {
        const entries = [
...
export function recordPrivacyAudit(decryptedDir, feature, chars, sessions, messages) {
    try {
        const db = openStore(decryptedDir);
...
export function getPrivacyStateSnapshot(decryptedDir) {
...
export function listPrivacyAudit(decryptedDir, limit = 200) {
    try {
        const db = openStore(decryptedDir);
...
export function clearPrivacyAudit(decryptedDir) {
    try {
        const db = openStore(decryptedDir);
