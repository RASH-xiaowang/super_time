...
function walkDataDats(dir, out, depth) {
    ...
    for (const e of entries) {
        ...
        if (e.isDir)
            walkDataDats(p, out, depth + 1);
        else if (e.name.toLowerCase().endsWith('.dat') && MD5_PREFIX_RE.test(e.name))
            out.push(p);
    }
}
/**
 ...
 */
export async function decryptAllImageDats(rawRoot, decodedDir, aesKey, xorKey, concurrency = 8, onProgress) {
    const files = [];
    ...
    return { total: files.length, okCount, failed, skipped, errors };
}
//# sourceMappingURL=decrypt-images.js.map