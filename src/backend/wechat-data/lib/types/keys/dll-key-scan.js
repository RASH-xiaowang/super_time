...
function matchKeyAt(buf, start) {
    ...
    const parts = [];
    for (let i = 0; i < 4; i += 1) {
        if (!MOV_RDX.equals(buf.subarray(offset, offset + 2)))
            return null;
 ...
 */
export function extractXorKeysFromDll(dllPath) {
    ...
    if (peOffset + 24 > file.length)
        return [];
    if (file.toString('latin1', peOffset, peOffset + 4) !== 'PE\0\0')
        return [];
    ...
    for (let i = 0; i < numberOfSections; i += 1) {
        ...
        if ((characteristics & CODE_SECTION_CHARACTERISTIC) === 0)
            continue;
...
//# sourceMappingURL=dll-key-scan.js.map