...
import { readProcessMemory, enumerateScannableRegions } from "./win32-memory.js";
import { verifyDbKey } from "../query/config.js";
/** 32-byte key length. */
const KEY_SIZE = 32;
...
function findKeyAddresses(chunk) {
    ...
    for (let i = 0; i + 32 <= chunk.length; i += 1) {
        if (chunk[i + 16] !== 0x20 || chunk[i + 24] !== 0x2F)
            continue;
        let ok = true;
 ...
 */
export async function scanProcessKeyCandidates(pid) {
        ...
        for (const region of regions) {
            ...
            while (offset < region.size) {
                ...
                if (chunk.length > 0) {
                    ...
                    for (const addr of findKeyAddresses(data))
                        addresses.add(addr);
 ...
 */
export async function recoverDbKeyV4(pid, dbFilePath, internalDbKey) {
    let page1;
    try {
    ...
    if (page1.length < PAGE_SIZE)
        return { ok: false, error: '数据库文件太小，不是有效的 V4 加密库' };
    const rawCandidates = await scanProcessKeyCandidates(pid);
    if (rawCandidates.length === 0)
        return { ok: false, error: '进程内存中未找到密钥候选' };
...
//# sourceMappingURL=db-key-v4.js.map