 ...
 */
export function cleanWxid(value) {
 ...
 */
export function deriveImageKeys(code, wxid) {
    ...
    if (cleanedWxid === '')
        throw new Error('wxid must not be empty');
...
function decryptAesBlock(aesKey, ciphertext) {
    ...
    if (keyBytes.length < AES_BLOCK_SIZE || ciphertext.length !== AES_BLOCK_SIZE)
        return null;
    try {
 ...
 */
export function detectImageFormat(plaintext) {
    if (plaintext === null || plaintext.length === 0)
        return null;
    if (plaintext.subarray(0, 3).equals(Buffer.from([0xFF, 0xD8, 0xFF])))
        return 'jpeg';
    if (plaintext.subarray(0, 8).equals(Buffer.from([0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A])))
        return 'png';
    if (plaintext.subarray(0, 4).toString('latin1') === 'RIFF' && plaintext.subarray(8, 12).toString('latin1') === 'WEBP')
        return 'webp';
    if (plaintext.subarray(0, 4).toString('latin1').toLowerCase() === 'wxgf')
        return 'wxgf';
    if (plaintext.subarray(0, 5).toString('latin1').startsWith('GIF8'))
        return 'gif';
    return null;
}
/**
 ...
 */
export function verifyAesKey(aesKey, ciphertext) {
 ...
 */
export function inferXorKeyFromV2Tails(tails) {
...
function readV2Template(filePath) {
    try {
        ...
        if (fd.length < headerLen || !fd.subarray(0, V2_MAGIC.length).equals(V2_MAGIC))
            return null;
...
function offerRecentTemplates(dir, capacity) {
    ...
    try {
        for (const entry of readdirSync(dir, { withFileTypes: true })) {
            if (!entry.name.toLowerCase().endsWith('_t.dat') || !entry.isFile())
                continue;
 ...
 */
export function scanV2Templates(accountDir, limit = 32, maxFallbackDirs = 500) {
    if (limit <= 0)
        return { templates: [], inferredXorKey: null, usedFallback: false, filesScanned: 0, xorSupport: 0 };
    ...
    for (const attachDir of attachDirs) {
        ...
        for (const monthDir of monthDirs) {
            ...
            for (const imgDir of listChildDirs(monthDir)) {
                if ((imgDir.split(/[\\/]/).pop() ?? '').toLowerCase() !== 'img')
                    continue;
                if (visited >= MAX_PREFERRED_DIRS)
                    break;
                visited += 1;
    ...
    if (templates.length === 0 && maxFallbackDirs > 0) {
        ...
        while (queueIndex < queue.length && queueIndex < maxFallbackDirs) {
            ...
            for (const child of children) {
                ...
                if (SKIPPED_FALLBACK_DIR_PARTS.some(part => lower.includes(part)))
                    continue;
                if (queue.length >= maxFallbackDirs)
                    break;
                queue.push(child);
            }
        }
 ...
 */
export function trustedXorForVerifiedAesKey(aesKey, templateData) {
    ...
    if (current === undefined)
        return null;
    if (detectImageFormat(decryptAesBlock(aesKey, current.ciphertext)) === null)
        return null;
    const matching = [];
 ...
 */
export function kvcommCodesFromDir(kvcommDir) {
    const codes = [];
    try {
        for (const entry of readdirSync(kvcommDir, { withFileTypes: true })) {
            ...
            if (code > 0 && code <= MAX_CODE && !codes.includes(code))
                codes.push(code);
        }
    }
    catch {
...
export function resolveLocalImageKey(opts) {
    ...
    if (codes.length === 0)
        return null;
    const nativeValues = [];
    if (typeof localNativeWxids === 'string')
        nativeValues.push(localNativeWxids);
    else if (localNativeWxids)
        nativeValues.push(...localNativeWxids);
    ...
    for (const wxid of wxids) {
        for (const code of orderedCodes) {
            ...
            if (!verifyAesKey(keys.aesKey, current.ciphertext))
                continue;
            return {
                code,
                wxid: cleanWxid(wxid),
...
//# sourceMappingURL=image-key-resolver.js.map