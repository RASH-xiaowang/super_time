...
import { extractXorKeysFromDll } from "./dll-key-scan.js";
import { recoverDbKeyV4 } from "./db-key-v4.js";
import { scanImageKeyOnce } from "./image-key-memory-scan.js";
import { resolveLocalImageKey, scanV2Templates, trustedXorForVerifiedAesKey } from "./image-key-resolver.js";
import { getAccountKeysFromStore, upsertAccountKeysInStore } from "./key-store.js";
import { resolveDecryptedDir } from "../dirs.js";
import { detectWechatAccounts } from "../query/config.js";
...
function defaultWeixinDllCandidates() {
    ...
    if (userProfile)
        out.push(join(userProfile, 'AppData', 'Roaming', 'Tencent', 'Weixin', 'Weixin.dll'));
    return out;
}
/**
...
export function findWechatPid() {
    try {
        ...
        for (const line of out.split(/\r?\n/)) {
            ...
            if (WECHAT_EXECUTABLE_NAMES.includes(name))
 ...
 */
export function scanDllInternalKey(wechatInstallDir) {
    const candidates = [];
    if (wechatInstallDir) {
        ...
        try {
            for (const entry of readdirSync(wechatInstallDir, { withFileTypes: true })) {
                if (entry.isDirectory() && /^\d+\.\d+\.\d+/.test(entry.name))
                    candidates.push(join(wechatInstallDir, entry.name, 'Weixin.dll'));
            }
        }
    ...
    if (base)
        candidates.push(join(base, 'Weixin.dll'));
    ...
    for (const p of candidates) {
        ...
        try {
            ...
            const first = hits[0];
            if (first?.keyHex)
                return Buffer.from(first.keyHex, 'hex');
        }
        catch {
 ...
 */
export async function fetchDbKey(opts = {}) {
    const pid = findWechatPid();
    if (pid === null)
        return { ok: false, error: '未检测到运行中的微信进程（Weixin.exe/WeChat.exe）' };
    const probe = pickProbeDb(resolveDecryptedDir(), opts.dbPath);
    if (probe === null)
        return { ok: false, error: '找不到可用于校验的 V4 加密数据库' };
 ...
 */
export function normalizeAccountDir(accountDir) {
 ...
 */
export function kvcommCacheDir() {
 ...
 */
export async function fetchImageKey(opts = {}) {
    const pid = opts.pid ?? findWechatPid();
    if (pid === null || pid <= 0)
        return { ok: false, error: '未检测到运行中的微信进程' };
    ...
    if (match === null)
        return { ok: false, error: '进程内存中未找到通过 V2 验证的图片 AES 密钥' };
 ...
 */
export function getKeysInfoSummary() {
    ...
    if (stored.updated_at)
        result.updatedAt = stored.updated_at;
    return result;
}
