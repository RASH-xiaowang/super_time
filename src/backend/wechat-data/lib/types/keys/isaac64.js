...
export class Isaac64 {
    ...
    #init(flag) {
        const s = {
            a: _a.GOLDEN, b: _a.GOLDEN, c: _a.GOLDEN, d: _a.GOLDEN,
            e: _a.GOLDEN, f: _a.GOLDEN, g: _a.GOLDEN, h: _a.GOLDEN,
        };
    ...
    #isaac64() {
        ...
        for (let i = 0; i < 256; i += 1) {
            ...
            if ((i & 3) === 0)
                this.#aa = u64(this.#aa ^ (u64(this.#aa << 21n) ^ MASK_64));
            else if ((i & 3) === 1)
                this.#aa = u64(this.#aa ^ (this.#aa >> 5n));
            else if ((i & 3) === 2)
                this.#aa = u64(this.#aa ^ (this.#aa << 12n));
            else
     ...
     */
    static rawToBytes(raw, wordFormat) {
        const v = u64(raw);
        if (wordFormat === 'raw_le')
            return Buffer.from(v.toString(16).padStart(16, '0'), 'hex').reverse();
        if (wordFormat === 'raw_be')
            return Buffer.from(v.toString(16).padStart(16, '0'), 'hex');
     ...
     */
    generateKeystream(size, wordFormat = 'be_swap32') {
        ...
        for (let i = 0; i < blocks; i += 1)
            out.push(_a.rawToBytes(this.randU64(), wordFormat));
