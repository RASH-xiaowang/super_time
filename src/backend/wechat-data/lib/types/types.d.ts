 ...
 */
export type KbFileParseState = 'queued' | 'parsing' | 'chunking' | 'embedding' | 'ready' | 'unsupported' | 'failed' | 'sparse_only';
...
export interface KbFileRegisterResult {
    ok: boolean;
    code?: 'bad-kb' | 'bad-path' | 'not-accepted' | 'too-large' | 'too-many' | 'duplicate' | 'read-failed' | 'store-failed';
    error?: string;
    /** 成功时给出落地的那一行。 */
    file?: KbFileMeta;
