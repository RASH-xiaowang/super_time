 ...
 */
import { enumerateScannableRegions, readProcessMemory } from './win32-memory.ts'
import { trustedXorForVerifiedAesKey, type TemplateScanResult } from './image-key-resolver.ts'
 ...
 */
export function iterMemoryAesCandidates(data: Buffer): Array<{ key: string; encoding: 'ascii' | 'utf-16le' }> {
  const seen = new Set<string>()
  const out: Array<{ key: string; encoding: 'ascii' | 'utf-16le' }> = []
 ...
 */
export function findVerifiedAesKeyInChunk(data: Buffer, templateScan: TemplateScanResult): ProcessMemoryKeyMatch | null {
 ...
 */
export async function scanProcessForImageKey(pid: number, templateScan: TemplateScanResult): Promise<ProcessMemoryKeyMatch | null> {
 ...
 * @returns the verified match or null.
 */
export async function scanImageKeyOnce(pid: number, templateScan: TemplateScanResult): Promise<ProcessMemoryKeyMatch | null> {
