 ...
 */
export function resolveImageKeyPair(decrypted: string): { aesKey: string; xorKey: number } {
  ...
  if (aesKey === '') {
    ...
    if (typeof stored.image_aes_key === 'string' && stored.image_aes_key.trim() !== '') {
      ...
      if (Number.isFinite(storedXor) && storedXor >= 0 && storedXor <= 255) xorKey = storedXor
    }
  }
