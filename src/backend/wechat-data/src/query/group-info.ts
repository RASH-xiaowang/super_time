...
import type { GroupInfo, GroupInfoSnapshot, GroupMember } from '../types.ts'
import { buildRegion, cellText, parseContactExtra, readVarint } from './region.ts'
 ...
 */
export function parseChatRoomExtBuffer(raw: Buffer): ChatRoomSnapshotMember[] {
  const out: ChatRoomSnapshotMember[] = []
  let idx = 0
  const n = raw.length
  while (idx < n) {
    ...
    if (field === 1) {
      ...
      const item: ChatRoomSnapshotMember = { username: '' }
      let i = 0
...
let _openimNames: { dir: string; map: Map<string, string> } | null = null
function openimNames(decryptedDir: string): Map<string, string> {
  ...
  try {
    ...
    try {
      const rows = db.prepare('SELECT wording_id AS id, wording AS w FROM openim_wording').all() as Array<{ id: unknown; w: unknown }>
      for (const r of rows) {
 ...
 */
export function queryGroupInfo(
  decryptedDir: string,
  username: string,
  selfUsername?: string,
): GroupInfoSnapshot {
  ...
  try {
    ...
    const row = db.prepare('SELECT username, nick_name, remark, chat_room_notify, flag FROM contact WHERE username=?').get(username) as Record<string, unknown> | undefined
    ...
    const cr = db.prepare('SELECT owner FROM chat_room WHERE username=?').get(username) as { owner?: unknown } | undefined
    const info = db.prepare('SELECT announcement_, announcement_editor_, announcement_publish_time_ FROM chat_room_info_detail WHERE username_=?').get(username) as {
      announcement_?: unknown
      announcement_editor_?: unknown
      announcement_publish_time_?: unknown
    } | undefined

    const members: GroupMember[] = []
    let totalMembers = 0
    const roomRow = db.prepare('SELECT id, ext_buffer FROM chat_room WHERE username=?').get(username) as {
      id?: number
      ext_buffer?: unknown
    } | undefined
    ...
    const pushMember = (mUsername: string, r?: Record<string, unknown>): void => {
      ...
      if (r) {
        ...
        if (ebBuf.length > 0) {
          ...
          if (extra.gender !== undefined) member.gender = extra.gender
    ...
    if (roomId !== undefined) {
      try {
        const rows = db.prepare(
          'SELECT c.username, c.nick_name, c.remark, c.small_head_url, c.big_head_url, c.extra_buffer ' +
          'FROM chatroom_member m LEFT JOIN contact c ON c.id = m.member_id ' +
          'WHERE m.room_id = ? ORDER BY m.rowid',
    ...
    if (extBuf.length > 0) {
      ...
      for (const s of snap) {
        ...
        try { c = contactStmt.get(s.username) } catch { c = undefined }
        pushMember(s.username, c)
      }
    }
    totalMembers = Math.max(snapCount, seenMembers.size)

    let myAlias: string | undefined
