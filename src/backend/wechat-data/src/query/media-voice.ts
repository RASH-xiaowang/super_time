 ...
 */
function messageServerId(decryptedDir: string, username: string, localId: number): string | null {
  ...
  for (const shard of messageShardFiles(decryptedDir)) {
    ...
    if (has) {
      try {
        const row = db.prepare('SELECT CAST(server_id AS TEXT) AS s FROM "' + table + '" WHERE local_id = ? LIMIT 1').get(localId) as { s?: string } | undefined
        db.close()
 ...
 */
export function resolveVoiceInfo(
  decryptedDir: string,
  username: string,
  localId: number,
): { available: boolean; svrId?: string; length?: number; decodable: boolean; error?: string } {
  ...
  try {
    ...
    if (!has) { db.close(); return { available: false, decodable: false, error: 'VoiceInfo 表不存在' } }
    // 1. Name2Id direct (chat_name_id, local_id)
    let row: { svr_id?: string; len?: number } | undefined
    const chat = db.prepare('SELECT rowid FROM Name2Id WHERE user_name = ?').get(username) as { rowid: number } | undefined
    if (chat) {
    ...
    return { available: false, decodable: false, error: (e as Error).message }
  }
}
