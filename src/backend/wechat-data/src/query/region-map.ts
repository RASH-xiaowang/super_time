...
import type { RegionMapSnapshot, RegionNode } from '../types.ts'
import { parseContactExtra, regionLabel } from './region.ts'

/** One parsed contact row with its region. */
interface RegionRow {
  username: string
  displayName: string
  remark: string
...
function makeNode(name: string, count: number): RegionNode {
 ...
 */
export function queryRegionMap(decryptedDir: string): RegionMapSnapshot {
  ...
  try {
    ...
    try {
      ...
      const has = (c: string): string => (cols.includes(c) ? c : 'NULL')
      const sql = 'SELECT ' +
        ...
        ' FROM contact'
      const raw = db.prepare(sql).all() as Array<Record<string, unknown>>
      for (const r of raw) {
  ...
  const sortNode = (n: RegionNode): void => {
  ...
  const world: RegionNode = { key: '世界', name: '世界', count: rows.length, children: worldChildren, friends: [] }
  sortNode(world)
