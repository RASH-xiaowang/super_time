...
import { cachedBySig, shardCatalogSig } from './meta.ts'
 ...
 */
export function queryAnnual(decryptedDir: string): { years: number[] } {
  return cachedBySig('annual:' + decryptedDir, shardCatalogSig(decryptedDir, ['message']), () => computeAnnual(decryptedDir), 30_000)
}

function computeAnnual(decryptedDir: string): { years: number[] } {
  ...
  for (const file of files) {
    ...
    try { db = new DatabaseSync(join(msgDir, file), { readOnly: true }) } catch { continue }
    try {
      ...
      for (const t of tables) {
        ...
        try {
          const rows = db.prepare(`SELECT DISTINCT CAST(strftime('%Y', datetime(create_time, 'unixepoch')) AS INTEGER) y FROM "${t}" WHERE create_time > 0`).all() as Array<{ y: number }>
          for (const r of rows) {
            const y = r.y
