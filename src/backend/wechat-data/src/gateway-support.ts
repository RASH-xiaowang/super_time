/**
 * 网关的模块级支撑（M21 结构刀自 `gateway-core.ts` 拆出）：数据目录解析、长任务控制槽、
 * 隐私库不可读的统一文案与 jobId 规整 —— 都是不依赖实例状态的纯声明。
 */
 ...
 */
export function privacyStoreUnreadable(feature: string, detail: string): string {
 ...
 */
export function rawWechatBase(decrypted: string): string {
...
export const STREAM_JOB_CAP = 20

/** 导出/备份进度事件名（渲染层按 jobId 过滤）。 */
export const EXPORT_PROGRESS_EVENT = 'wechat-export/progress'
 ...
 */
export const CACHED_IMAGE_EXTS = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'tif']
