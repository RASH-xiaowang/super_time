...
describe('M1 复审后的边界：不固化默认值、不静默清空、损坏留痕', () => {
  ...
  it('用户显式保存过的密钥字段，secrets.json 优先', () => {
    const root = tempRoot()
    const decrypted = join(root, 'decrypted')
    saveConfig(decrypted, { image_aes_key: 'e57c869f15dd8764' })
    expect(getConfig(decrypted)['image_aes_key']).toBe('e57c869f15dd8764')
  })
  ...
  it('patch 里全是空串/默认值（界面未加载完就点保存）时，secrets.json 一字不动', () => {
    ...
    saveConfig(decrypted, { db_enc_key: dbKey, image_aes_key: 'e57c869f15dd8764', api_token: 'tok-real' })
    const secretsFile = join(root, 'secrets.json')
    ...
    expect(cfgNow['image_aes_key']).toBe('e57c869f15dd8764')
