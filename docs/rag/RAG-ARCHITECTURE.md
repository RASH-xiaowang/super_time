...
**调参工作流**：改 `rag-config.json` → `npm run rag:check` 看指标是否退化 → 必要时跑
`evaluateRetrieval` 做混合/稀疏消融对比。
...
| 检索设置栏 `RetrievalPanel` | 上下文条「检索」chip（与模型配置同车道、互斥） | 概览三格 + 通道开关 + 阈值容量 + 反馈闭环 + 动作（保存 / 评估 / 增量构建 / 全量重建 / 重置权重 / 查看反馈） |
...
| `ui:smoke` | 新 UI 组件 SSR 渲染 13 项断言（`splitMarks` 分组、漏斗行降级告警、反馈三态、面板开关、会话 AI 面板空/全屏态） | 无（react-dom/server） |
...
| 项 | 说明 | 建议 |
|---|---|---|
| 首次提问会被向量索引构建阻塞 | 开启稠密后第一次提问会同步建索引（上限 `maxDocsPerBuild`，默认 40000 条），期间界面无进度、提问看起来像卡住 | 建议在提问路径上用小上限（如 2000）快速起步，剩余交给「增量构建」按钮；或加进度反馈 |
...
| ≤900px 窄窗布局 | 本轮未在窄窗下验收（默认窗口 1664px） | 手动缩放窗口确认侧栏对换 |
