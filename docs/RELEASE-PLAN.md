...
| 2026-09-22 | 实施 | M21 | 进行中（第二十五刀：Moments 第二刀之三 —— 卡片抽成组件） | 新建 `moments-card.tsx`（353 行）：把 `g.items.map((m) => { …局部 const… return (…卡片…) })` 的整段搬成 `MomentsCard`（31 个道具），`moments-panel.tsx` 1312 → **1067**（棘轮条目同步收紧）。局部 const（`cover` / `textExpanded` / `visibleComments` 等 10 个）随卡片进组件 —— 它们都是从 `m` 与道具派生的；面板侧只剩 `<MomentsCard key={m.tid} … />`，`key` 从根 div 挪到调用处（列表语义不变）。**没有**把按 tid 索引的整列表状态（`expandedTextByCard` / `commentCounts` / `failedImgs` / `videoSrcs`…）搬进卡片：卡片只是读写其中一格，搬进去会变成「每卡一份」，跨卡语义就变了。搬移同样**自证**：逐行等价 + 闭包审计（无未解析标识符）。面板里因此空出来的四个 import（`Fragment` / `MomentsAvatar` / `fmtCommentTime` / `replyTarget`）已清掉。门禁：`npm test` 2204 通过、typecheck 0、`ui:smoke` 39 项、棘轮 5 条绿。距 1000 行还差 67 ⇒ **下一刀**：筛选面板 + 月份直方图 + 洞察统计（~160 行）抽成 `moments-toolbar.tsx`，届时可从白名单删条目。 |
