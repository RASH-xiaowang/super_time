...
| 2026-09-22 | 实施 | M21 | 进行中（第二十六刀：Moments 第二刀之四 —— 左侧栏抽成组件，面板达标出白名单） | 新建 `moments-sidebar.tsx`（136 行）：把 `<aside>`（筛选面板 + 月份直方图 + 洞察统计，~133 行）整段搬成 `MomentsSidebar`（24 个道具），`moments-panel.tsx` 1067 → **957** ⇒ **已达标，棘轮白名单里的 moments-panel 条目删除**（条目数 6 → 5；`moments-*.tsx` 四份都 ≤ 1000，联合 1999 行）。搬移自证：逐行等价（唯一 token 改动是 `moments.length` → `loadedCount`，语义相同、避免让子组件知道整个列表）+ 闭包审计。面板里空出来的 `DateRangeField` / `Segmented` / `MEDIA_LABELS` 已清。**覆盖面**：`ui:smoke` 的两条朋友圈用例渲染的就是面板本体（无数据时左侧栏也渲染），所以这一刀在 SSR 冒烟下真实走过；卡片那刀只有 typecheck + 逐行等价兜底（有数据的交互路径本仓没有自动化）。门禁：`npm test` 2204 通过、typecheck 0、`ui:smoke` 39 项、棘轮 5 条绿。**下一刀**：M21 剩余白名单 5 条（`gateway.ts` 4837 / `Chats.tsx` 3957 / `chats.module.css` 2904 / `Settings.tsx` 1978 / `onboarding.module.css` 1618）。 |
