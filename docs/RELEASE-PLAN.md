...
| 2026-09-22 | 实施 | M21 | 进行中（第二十四刀：Moments 第二刀之二 —— 图片查看器 + 单条动态详情抽成组件） | `moments-portals.tsx` 增加 `MomentsImageViewer`（21 道具）与 `MomentsDetail`（16 道具），`moments-panel.tsx` 1505 → **1312**（棘轮条目同步收紧到 1312）。拆法沿用同一约定：**只搬 JSX**，面板里的 state 与四个 effect（打开重置 / 滚轮缩放 / 键盘左右 / 按需解码）原地不动 —— 状态留在上层，组件只在 `viewer`/`detail` 为假时早返回，语义与原先的 `{cond && createPortal(…)}` 一致；若把这些 state 一起搬进组件，「何时重跑 effect」会变（例如键盘监听与按需解码的依赖），那属于另一个改动。搬移由脚本完成并**自证**：搬走的前后两段 JSX 逐行等价（去缩进后完全相同），另做**闭包审计**（新组件体里除 props/import 外无自由标识符 —— 审计当场抓出 `Fragment` / `fmtCommentTime` / `MomentsMiniAvatar` / `replyTarget` 四个漏 import）。顺带把 `moments-cover.wiring.spec.ts` 的手写联合清单换成 **readdir 前缀扫描**（`moments-*.tsx` + 桶）：手写清单在本次搬移后当场变红（`setFailedImgs(prev => …add(dfk))` 搬去了 portals），这正是 `transient-notice.wiring.spec.ts` 早就用的写法。门禁：`npm test` 2204 通过、typecheck 0、`build:ui` 通过、`ui:smoke` 39 项、棘轮 5 条绿。**下一刀**：卡片 map（315 行，39 道具）⇒ 预计落到 ~1045，再视情况做视频格子去重（卡片与详情里各有一份 ~40 行、只差一个 `videoTileDetail` 类）与工具栏切片，把面板压到 1000 以内。 |
