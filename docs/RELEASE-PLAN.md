...
| N19 | **打包版语音 silk 解码坏掉**：`wx_silk.exe` 不在 `asarUnpack`，而它是要被 spawn 的可执行文件（M19 取证时发现，复审独立证实） | `src/backend/wechat-data/resources/win32/x64/wx_silk.exe`（248,320 B）**不在** `asarUnpack`（`package.json:121-129`），而 `src/backend/wechat-data/src/query/voice.ts:94-110` 的 `silkDecoderBin()` 用 `existsSync` 从 `import.meta.url` 上溯找它 —— 打包态该路径落在 asar **内部**，Electron 的 `fs` 补丁让 `existsSync` 判为**真**（不像隔壁 `whisper.ts` 的 `bundledWhisperAssetsDir` 走了 `unpackedAware()`），随后 `voice.ts:132` 的 `spawnSync(bin, ...)` **无法执行 asar 内的 exe**。复审实测同一 exe：磁盘路径 `spawnSync status=1`（真执行）、asar 路径 `error.code=ENOENT`。影响面：`voice.ts:172` 的语音取 WAV、`voice-transcribe.ts:154` 的批量转写 —— 即打包版**语音播放与转写均不可用**（开发态正常，因为路径是真的）。 ⏳ 未开始 —— 修法两条都要做，缺一不可：① `asarUnpack` 加 `src/backend/wechat-data/resources/**`；② `silkDecoderBin()` 的结果过一遍路径改写（把 bundle 里已有的 `unpackedAware()` 抽成共享模块复用，而不是抄第二份）。验收：打包态下 `silkDecoderBin()` 返回 `app.asar.unpacked` 下的真实路径；单测覆盖「asar 路径 → 改写为 unpacked 且优先返回存在的那份」（可用临时目录模拟两种布局，无需 Electron）；`packaged-smoke` 断言该 exe 在 `app.asar.unpacked` 下存在。**未验证的边界**：真实微信语音数据的端到端解码（需要真实库与 whisper 模型）。 | 未开始 |
| N20 | 任务栏图标可能出双份：`main.js` 未设 AppUserModelID，而 NSIS 快捷方式写的是 appId（M19 复审发现） | NSIS 会给快捷方式写 `com.supertime.electron`（`appId`），但 `main.js` 全文没有 `app.setAppUserModelId(...)` —— Windows 用「窗口的 AppUserModelID + 快捷方式的」做任务栏分组与固定，两者不一致时固定后再启动可能出现**双图标**（或固定项显示成另一个图标）。与 M19 的窗口图标无关（那一条已证不缺）。 ⏳ 未开始 —— 在 `app.whenReady()` 之前调 `app.setAppUserModelId('com.supertime.electron')`（与 `appId` 同一个常量来源最好）。验收：安装后「固定到任务栏」再启动，任务栏不出现第二个图标。 | 未开始 |
| N21 | 干净检出直接 `npm run dist` 会打出**没有前端**的包（M19 复审发现；排掉 `ui-app` 后已无第二份兜底） | `src/client/ui-dist/**` 在 `.gitignore` 里（构建产物），而 `dist` 脚本是 `electron-builder --win nsis`、`pack` 是 `electron-builder --dir` —— **都没有前置 `build:ui`**（对比 `prestart`/`dev` 都有）。于是干净检出直接打包时，asar 里 `src/client/ui-dist/index.html` 不存在，`main.js:422` 的 `uiEntryHtml()` 会退回到演示页 `src/index.html`。**M19 之前**这一条还「温和」（`src/client/ui-app/**` 也在包里）；M19 把 `ui-app` 排掉后，包里已**没有第二份前端**。 ⏳ 未开始 —— 给 `pack`/`dist` 加前置 `build:ui`（与 `prestart` 同形）；M19 已加的那条断言（asar 必须含 `src/client/ui-dist/index.html`）会让这种包装出来后**在冒烟里立刻变红**，属先行兜底。验收：干净检出（`git clean -xdf` 后 `npm ci`）直接 `npm run dist`，打出的包启动后是真实前端而非演示页。 | 未开始 |
