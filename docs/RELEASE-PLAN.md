...
- **状态**：进行中（实现与验证完成，独立评审进行中）　**依赖**：H7　**预估**：2d
- **证据**：
...
- **本轮结果（2026-09-13）**：
  ...
  - [x] 命中集合不变：新增 6 项测试，含「跨过让出阈值（>2000 行）后仍完整」——
    覆盖「迭代中途 await 后迭代器状态正确」这一点
  - [x] 建索引不再秒级阻塞：`buildSearchIndex` 转 async，每 2000 行让出一次事件循环
    （node:sqlite 是同步 API，bigram 切分 + FTS 写入是纯 CPU，不让出会让承载 130+ 查询
    方法的 worker 停摆）；gateway 三处调用点相应 await。判别式测试：构建期间窗口内
    必须有其它宏任务被调度（全程同步则必然失败）
  - [ ] **搜索可中断未做**：兜底搜索本身仍是同步循环（只是行内不再物化），
    「切换面板即中断」需要 AbortSignal 贯通到查询层，属更大改造
  - 注：`search.ts` 里剩余的 `.all()` 都是元数据小表（sqlite_master / name2id /
    SessionNoContactInfo），量级与联系人/会话数同阶，无需改造
