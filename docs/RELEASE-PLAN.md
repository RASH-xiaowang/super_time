...
| 2026-09-22 | 实施 | M21 | 进行中（第二十三刀：Moments 第二刀之一 —— 导出对话框抽成组件） | 新建 `moments-portals.tsx`，先把**导出对话框**（74 行）从 `moments-panel.tsx` 里抽成 `MomentsExportDialog`（23 个道具）。**道具名与面板作用域里的名字完全相同** ⇒ JSX 逐行搬、面板侧一个对象字面量传进去；唯一的结构性改动是 `{exportOpen && (…)}` 换成组件内**早返回**（DOM 与属性一字未改）。`moments-panel.tsx` 1571 → **1505**（棘轮条目同步收紧）。门禁：`npm test` 2204 通过、typecheck 0（typecheck 就是这一刀的主要验证手段 —— props 少一个/多一个都会红）、`build:ui` 通过、`ui:smoke` 39 项。**下一刀**：图片查看器（100 行，21 道具，依赖 `cspSafeSrc` 来自 `utils/url.ts`）与单条动态详情（138 行，17 道具），再切卡片 map（315 行，39 道具）⇒ 面板 ~945 达标。 |
